...
static void hi3516ev300_io_config_spi_sfc(void)
{
...
static void hi3516ev300_io_config_spi_sdio(void)
{
    /* set pad ctrl reg for spi */
...
void hifmc_set_fmc_system_clock(struct spi_op *op, int clk_en)
{
    ...
    if (op && op->clock) {
        ...
        if(SPI_INPUT_SLE(sysstat_val)) {
            hi3516ev300_io_config_spi_sdio();
        } else {
            hi3516ev300_io_config_spi_sfc();
        }
    }
    if (clk_en) {
