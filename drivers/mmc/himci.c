...
static int hi_mci_setup_data(struct himci_host *host, struct mmc_data *data)
{
	...
	if (data->flags != MMC_DATA_READ){
		...
		invalidate_dcache_range((unsigned long)data->dest,(unsigned long)data->dest + blk_size);

	flush_cache((unsigned long)host->dma_des, sizeof(struct himci_dma_des)*(unsigned long)des_cnt);
...
tuning_retry:
	...
	err = mmc_send_tuning(mmc, opcode);
		...
		unsigned int max_clk, unsigned int min_clk)
{
	...
	cfg->host_caps |= MMC_MODE_HS | MMC_MODE_HS_52MHz | MMC_MODE_HS200_1_8V;
