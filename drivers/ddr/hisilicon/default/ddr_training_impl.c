...
unsigned int ddr_phy_get_dq_bdl(struct ddr_cfg_st *cfg)
{
	...
	dq &= 0x3;
	val = (REG_READ(cfg->cur_phy + offset) >> ((dq << 3) + PHY_BDL_DQ_BIT)) & PHY_BDL_MASK;
...
static void ddr_rdqs_sync_rdm(struct ddr_cfg_st *cfg, int offset)
{
	...
	REG_WRITE(rdqnbdl | ((unsigned int)rdm << PHY_RDM_BDL_BIT), cfg->cur_phy + DDR_PHY_DXNRDQNBDL2(cfg->rank_idx, cfg->cur_byte));
}
...
static void ddr_bdl_adj(struct ddr_cfg_st *cfg)
{
	...
	dq03 = REG_READ(base_phy + DDR_PHY_DXNRDQNBDL0(rank, byte_idx));
	dq47 = REG_READ(base_phy + DDR_PHY_DXNRDQNBDL1(rank, byte_idx));
	rdm = REG_READ(base_phy + DDR_PHY_DXNRDQNBDL2(rank, byte_idx));
	rdqs = REG_READ(base_phy + DDR_PHY_DXNRDQSDLY(byte_idx));
	...
	REG_WRITE(dq03, base_phy + DDR_PHY_DXNRDQNBDL0(rank, byte_idx));
	REG_WRITE(dq47, base_phy + DDR_PHY_DXNRDQNBDL1(rank, byte_idx));
	REG_WRITE(rdm, base_phy + DDR_PHY_DXNRDQNBDL2(rank, byte_idx));
	REG_WRITE(rdqs, base_phy + DDR_PHY_DXNRDQSDLY(byte_idx));
}
...
static int ddr_hw_training_process(struct ddr_cfg_st *cfg, unsigned int item)
{
	...
	if (count == 0xffffffff) {
		...
		ddr_training_stat(DDR_ERR_HW_RD_DATAEYE, base_phy, item, REG_READ(base_phy + DDR_PHY_PHYINITSTATUS)); /* TODO: */
		return -1;
	}
...
int ddr_hw_training_ctl(struct ddr_cfg_st *cfg)
{
	...
	unsigned int base_phy = cfg->cur_phy;
	unsigned int byte_num = cfg->phy[cfg->phy_idx].total_byte_num;
	unsigned int dvrft_ctrl = REG_READ(base_phy + DDR_PHY_DVRFTCTRL);
	...
	REG_WRITE(dvrft_ctrl & (~PHY_DVRFTCTRL_PDAEN_EN),
	...
	if (PHY_DRAMCFG_TYPE_DDR4 == cfg->phy[cfg->phy_idx].dram_type) {
		REG_WRITE(temp, base_phy + 0xe0); /* restore */
		temp = REG_READ(base_phy + 0x1e0);
		temp1 = REG_READ(base_phy + 0x2c);
		REG_WRITE(0x05555555, base_phy + 0x1e0); /* inti MR5 */
		REG_WRITE(temp1 | 0x00004000 , base_phy + 0x2c); /* write dm disable */
		result += ddr_hw_training_process(cfg, item & PHY_PHYINITCTRL_DRAM_INIT_EN);
		REG_WRITE(temp, base_phy + 0x1e0); /* restore */
		REG_WRITE(temp1, base_phy + 0x2c); /* restore */
	}
#endif
	ddr_phy_cfg_update(base_phy);
...
static int ddr_hw_training_by_phy(struct ddr_cfg_st *cfg)
{
	int result = 0;
	int i;
	struct rdqs_data_st rdqs_st;
	struct ddr_timing_st timing_st;
	unsigned int rank_num = cfg->phy[cfg->phy_idx].rank_num;

	cfg->res_st = (void *)&rdqs_st;
...
static int ddr_mpr_find_rdqs(struct ddr_cfg_st *cfg)
{
	...
	REG_WRITE(PHY_DQ_MIDDLE_VAL << PHY_BDL_DQ_BIT, base_phy + DDR_PHY_DXNRDQNBDL0(cfg->rank_idx, byte_index));
	REG_WRITE(PHY_DQ_MIDDLE_VAL << PHY_BDL_DQ_BIT, base_phy + DDR_PHY_DXNRDQNBDL1(cfg->rank_idx, byte_index));

	/* clear rdqs */
	delay    = REG_READ(base_phy + DDR_PHY_DXNRDQSDLY(byte_index)) >> PHY_RDQS_BDL_BIT;
	...
	REG_WRITE(delay | (rdqs_mid << PHY_RDQS_BDL_BIT), base_phy + DDR_PHY_DXNRDQSDLY(byte_index));
	ddr_phy_cfg_update(base_phy);
...
static void ddr_vref_set(struct ddr_cfg_st *cfg, unsigned int val)
{
	if (DDR_MODE_READ == cfg->cur_mode) { /* HOST vref */
		...
		unsigned int auto_ref_timing =
			REG_READ(cfg->cur_dmc + DDR_DMC_TIMING2);
		/* disable auto refresh */
	...
	struct ddr_delay_st *wdqs_new, struct ddr_delay_st *wdqs_old)
{
	...
	for (i = 0; i < byte_num; i++) {
		...
		phase_adj = 0;
		wdq_phase = (REG_READ(base_phy + DDR_PHY_DXNWDQDLY(rank_index, i))
					>> PHY_WDQ_PHASE_BIT)
					& PHY_WDQ_PHASE_MASK;
		wdm_bdl  = (REG_READ(base_phy + DDR_PHY_DXNWDQNBDL2(rank_index, i))
					>> PHY_WDM_BDL_BIT)
					& PHY_BDL_MASK;
		...
		REG_WRITE(wdm_bdl << PHY_WDM_BDL_BIT, base_phy + DDR_PHY_DXNWDQNBDL2(rank_index, i));
	}
	...
	struct ddr_delay_st *wdqs_new, struct ddr_delay_st *wdqs_old)
{
	...
	for (i = 0; i < byte_num; i++) {
		...
		/* bdl */
		tmp = REG_READ(base_phy + DDR_PHY_DXNOEBDL(rank_index, i));
...
int ddr_write_leveling(struct ddr_cfg_st *cfg)
{
	...
	/* init wdqs */
	for (i = 0; i < byte_num; i++) {
		tmp = REG_READ(base_phy + DDR_PHY_DXWDQSDLY(rank_index, i));
	...
	if (result) {
		...
		for (i = 0; i < byte_num; i++) {
			...
			REG_WRITE(tmp, base_phy + DDR_PHY_DXWDQSDLY(rank_index, i));
		}
		ddr_phy_cfg_update(base_phy);
		return -1;
	}
...
int ddr_lpca_training_func(struct ddr_cfg_st *cfg)
{
	...
	if (PHY_DRAMCFG_TYPE_LPDDR3 ==
		(REG_READ(cfg->cur_phy + DDR_PHY_DRAMCFG)
...
static void ddr_exit_sref_enter_pd(struct ddr_cfg_st *cfg, struct dmc_cfg_sref_st *cfg_sref)
{
	...
	for (i = 0; i < phy_st->dmc_num; i++) {
		...
		REG_WRITE((cfg_sref->val[i] & (~DMC_CFG_INIT_XSREF_PD_MASK)) | DMC_CFG_INIT_XSREF_PD,
...
/* Duty direction ctl */
static int ddr_dcc_ck_ctl(struct ddr_cfg_st *cfg, unsigned int ioctl21_def, int ctl_index)
{
...
/* Duty Correction */
static int ddr_dcc_correct_duty(struct ddr_cfg_st *cfg, int cur_duty, unsigned int duty_def)
{
	unsigned int ioctl21;
	if (PHY_DRAMCFG_TYPE_LPDDR4 == cfg->phy[cfg->phy_idx].dram_type) {
				...
				int rank_index, unsigned int ck0_win_min)
{
	...
	for (byte_index = 0; byte_index < (byte_num/2); byte_index++) {
		dcc_data->ck[ck_index].val[byte_index] = REG_READ(cfg->cur_phy + DDR_PHY_DXNRDBOUND(byte_index));
	}
				...
				int rank_index, unsigned int ck1_win_min)
{
	...
	for (byte_index = 2; byte_index < byte_num; byte_index++) {
		dcc_data->ck[ck_index].val[byte_index - 2] = REG_READ(cfg->cur_phy + DDR_PHY_DXNRDBOUND(byte_index));
	}
