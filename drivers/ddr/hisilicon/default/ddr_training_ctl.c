...
int ddr_sw_training_func(void *ddrtr_result)
{
	...
	acphyctl = REG_READ(base_phy + DDR_PHY_ACPHYCTL4);
	acphyctl = REG_READ(base_phy + DDR_PHY_ACPHYCTL4);
...
int ddr_pcode_training_func(void *ddrtr_result)
{
	struct ddr_cfg_st ddr_cfg;
	struct ddr_cfg_st *cfg = &ddr_cfg;
...
#ifdef DDR_HW_TRAINING_CONFIG
int ddr_hw_training_func(void *ddr_hw)
{
	struct ddr_cfg_st ddr_cfg;
	struct ddr_cfg_st *cfg = &ddr_cfg;
...
int ddr_hw_training_if(void *ddr_hw)
{
	return DDR_HW_TRAINING_FUNC(ddr_hw);
}

int ddr_pcode_training_if(void *ddrtr_result)
{
	return DDR_PCODE_TRAINING_FUNC(ddrtr_result);
}
