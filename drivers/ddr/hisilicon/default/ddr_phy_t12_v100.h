...
#define DDR_PHY_VREF_HOST_SET(base_phy, rank, bytenum, byte_index, val) \
	do { \
		...
		if (0 == rank) { \
			...
			REG_WRITE(hvreft | (val << PHY_VRFTRES_RXDIFFCAL_BIT), base_phy + DDR_PHY_HVREFT_STATUS(rank, byte_index)); \
			REG_WRITE(hvreft | (val << PHY_VRFTRES_RXDIFFCAL_BIT), base_phy + DDR_PHY_HVREFT_STATUS(rank, byte_index + 1)); \
		} \
	} while (0)
...
#define DDR_PHY_SWITCH_RANK(base_phy, val) \
	do { \
		REG_WRITE((REG_READ(base_phy + DDR_PHY_TRAINCTRL0) & (~PHY_TRAINCTRL0_MASK)) | val, base_phy + DDR_PHY_TRAINCTRL0); \
		REG_WRITE((REG_READ(base_phy + DDR_PHY_HVRFTCTRL) & (~(0x1 << PHY_HRXDIFFCAL_EN_BIT))) | (val << PHY_HRXDIFFCAL_EN_BIT), base_phy + DDR_PHY_HVRFTCTRL); \
	} while (0)
