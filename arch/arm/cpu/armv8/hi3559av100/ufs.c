...
static int wait_for_cmd_completion(uint32_t tag, uint32_t timeout_ms)
{
    ...
    if ((tag < 1) || (tag > MAX_TR_TASK)) {
        debug_printf("wait_for_cmd_completion: input tag invalid, tag = %d\n", tag);
        tag = 1;
    }
...
static void create_nop_out_upiu(uint32_t tag)
{
    ...
    if ((tag < 1) || (tag > MAX_TR_TASK)) {
        debug_printf("create_nop_out_upiu: input tag invalid, tag = %d\n", tag);
        tag = 1;
    }
...
static int read_nop_rsp(uint32_t tag)
{
    ...
    if ((tag < 1) || (tag > MAX_TR_TASK)) {
        debug_printf("read_nop_rsp: input tag invalid, tag = %d\n", tag);
        tag = 1;
    }
...
static int send_nop_out_cmd(void)
{
    ...
    debug_printf("in send nop out: read tag = %d\n", tag);
    for (i = 0; i < 3; i++) {
        if ((tag > MAX_TR_TASK) || (tag < 1)) {
            tag = 1;
        }
        debug_printf("create nop out: tag = %d\n", tag);
                       ...
                       uint32_t size, uint32_t tag)
{
    int ret = 0;
    if ((tag < 1) || (tag > MAX_TR_TASK)) {
        debug_printf("do_scsi_cmd: input tag invalid, tag = %d\n", tag);
        tag = 1;
    }
...
static int test_unit_ready(void)
{
    ...
    tag = readl(STATIC_UFS_TAG_ADDR);
    debug_printf("test_unit_ready: read tag = %d\n", tag);
    for (i = 0; i < 4; i++) {
        if ((tag > MAX_TR_TASK) || (tag < 1)) {
            tag = 1;
        }
        debug_printf("test_unit_ready: tag = %d\n", tag);
                              ...
                              uint32_t size)
{
    ...
    tag = readl(STATIC_UFS_TAG_ADDR);
    debug_printf("ufs_read_boot_data: read tag = %d\n", tag);
    if ((tag > MAX_TR_TASK) || (tag < 1)) {
        tag = 1;
    }
    debug_printf("ufs_read_boot_data: tag = %d\n", tag);
