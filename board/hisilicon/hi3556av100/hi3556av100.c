...
#ifdef CONFIG_TARGET_HI3556AV100
    UPGRADE_STATUS_E* penUpgradeStatus = (UPGRADE_STATUS_E*)UPGRADE_STATUS_REG_ADDR;
    if (*penUpgradeStatus == UPGRADE_STATUS_PROCESSING) {
...
#ifdef CONFIG_HIFMC_SPI_NAND              /* spi nand configure */
...
#elif CONFIG_HIFMC_SPI_NOR              /* spi nor configure */
...
static struct boot_medium_interface boot_intf[1] = {
#ifdef CONFIG_HIFMC_SPI_NAND
    {"nand",   NULL, nand_flash_read}
#elif CONFIG_HIFMC_SPI_NOR
...
int misc_init_r(void)
{
    ...
    if (auto_update_flag) {
        ...
        UPGRADE_STATUS_E* penUpgradeStatus = (UPGRADE_STATUS_E*)UPGRADE_STATUS_REG_ADDR;
        if ((*penUpgradeStatus == UPGRADE_STATUS_PROCESSING) && (result == 0)) {
            printf("update upgrade status: finish\n");
            *penUpgradeStatus = UPGRADE_STATUS_FINISH;
        }
...
int board_init(void)
{
    ...
    gd->bd->bi_arch_number = MACH_TYPE_HI3556AV100;
...
static int start_a53cpu1(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    hw_dec_init();
    ret = hw_dec_decompress(NULL, (unsigned char *)addr, &size_uncompressed,
                            NULL, ((unsigned char *)srcaddr + HIGZIP_HEAD_SIZE), size_compressed, NULL);
    if (ret) {
