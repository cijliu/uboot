...
int is_auto_update(void)
{
    ...
    if (((*penUpgradeStatus) & (UPGRADE_STATUS_PROCESSING)) != 0) {
...
int misc_init_r(void)
{
    ...
    if (auto_update_flag) {
        ...
        unsigned int* puval = readl(UPGRADE_STATUS_REG_ADDR);
        unsigned int stor_dev = 0;
        unsigned int stor_paration = 0;
        stor_dev = (*puval >> 16) & 0x03;
        stor_paration = (*puval >> 18) & 0x1f;
        if(((*puval) & (UPGRADE_STATUS_PROCESSING)) != 0) {
        ...
        UPGRADE_STATUS_E* penUpgradeStatus = (UPGRADE_STATUS_E*)UPGRADE_STATUS_REG_ADDR;
        if (((*penUpgradeStatus) & (UPGRADE_STATUS_PROCESSING)) && (update_flag == 0)) {
