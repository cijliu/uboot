...
#define BL33_LOAD_ADDR  (kernel_load_addr - 0x40)	/* kernel start addr - sizeof(header) */
...
/**
 * scan_flat_dt_save_bootargs - scan flattened tree blob and save bootargs to chosen args.
 * @data: context data pointer
 *
 * This function is used to scan the flattened device-tree, 
 * find the 'chosen' args, and save save bootargs to chosen args.
 */
int scan_flat_dt_save_bootargs(void *data)
{
...
int load_fip_amp(char *buf)
{
	...
	bl31_p->bl32_ep_info = (uint64_t)bl32_ep;
	bl31_p->bl33_ep_info = (uint64_t)bl33_ep;
	...
	for (index = 0; index < file_info_count; index++) {
		...
		if (compare_uuids(&files[index].name_uuid, &uuid_bl31) == 0) {
			...
			memmove((void *)BL31_BASE, files[index].image_buffer, files[index].size);
		...
		if (compare_uuids(&files[index].name_uuid, &uuid_bl33) == 0) {
			...
			if (image_check_arch ((image_header_t *)(files[index].image_buffer), IH_ARCH_ARM64)) {
				memmove((void *)BL33_LOAD_ADDR, files[index].image_buffer, files[index].size);
				hdr = (image_header_t *)(BL33_LOAD_ADDR);
				char *fdt = (char *)image_get_image_end(hdr);
				int fdt_size = files[index].size - ((unsigned long)fdt - (unsigned long)hdr);

				printf("kernel_size[0x%x] fdt_size[0x%x] fdt_addr[0x%p]\n", files[index].size, fdt_size, fdt);
				memmove((void *)FDT_LOAD_ADDR, fdt, fdt_size);
				...
				bl33_ep->pc = (uint64_t)image_get_data((const image_header_t *)BL33_LOAD_ADDR);
				bl33_ep->args.arg0 = (uint64_t)fdt;
...
int load_fip(char *buf)
{
	...
	scan_flat_dt_save_bootargs(getenv ("bootargs"));
	...
	if (boot_a53mp_flag && boot_a53mp_addr) {
		...
		(*(volatile uint64_t *)8) = (uint64_t)&amp_bl31_params;
	...
	/*boot up a53up*/
	(*(volatile uint64_t *)8) = (uint64_t)bl31_p;
	(*(volatile uint64_t *)16) = (uint64_t)NULL;

	void (*atf_entry)(void) = (void(*)(void))(bl31_pc);
	atf_entry();
