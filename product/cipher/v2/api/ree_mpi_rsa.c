...
HI_S32 RSA_Private(HI_UNF_CIPHER_RSA_PRI_KEY_S *pstPriKey, HI_UNF_CIPHER_CA_TYPE_E enCaType,  HI_U8 *pu8In, HI_U8 *pu8Out)
{
    ...
    INLET_PARAM_CHECK_POINT_NULL(pstPriKey);
    INLET_PARAM_CHECK_POINT_NULL(pstPriKey->pu8D);
    INLET_PARAM_CHECK_POINT_NULL(pstPriKey->pu8N);
    ...
    stRsaData.enCaType = HI_UNF_CIPHER_KEY_SRC_USER;
                             ...
                             HI_U8 *pu8Mask, HI_U32 u32MaskLen)
{
    ...
    HI_U8* u8Seed = HI_NULL;

    u8Cnt = (HI_U8*)memalign(ARCH_DMA_MINALIGN, 4);
    if (HI_NULL == u8Cnt)
    {
        HI_ERR_CIPHER("memalign for u8Cnt invalid.\n");
        return HI_ERR_CIPHER_INVALID_POINT;
    }

    u8Md = (HI_U8*)memalign(ARCH_DMA_MINALIGN, HASH_RESULT_MAX_LEN);
    if (HI_NULL == u8Md)
    {
        HI_ERR_CIPHER("memalign for u8Md invalid.\n");
        free(u8Cnt);
        u8Cnt = HI_NULL;
        return HI_ERR_CIPHER_INVALID_POINT;
    }

    u8Seed = (HI_U8*)memalign(ARCH_DMA_MINALIGN, u32Seedlen);
    if (HI_NULL == u8Seed)
    {
        HI_ERR_CIPHER("memalign for u8Seed invalid.\n");
        free(u8Cnt);
        u8Cnt = HI_NULL;
        free(u8Md);
        u8Md = HI_NULL;
    ...
    cipher_memset_s(u8Cnt, 4, 0, 4);
    cipher_memset_s(u8Md, HASH_RESULT_MAX_LEN, 0, HASH_RESULT_MAX_LEN);
    cipher_memcpy_s(u8Seed, u32Seedlen, pu8Seed, u32Seedlen);
    ...
    for (i = 0; u32Outlen < u32MaskLen; i++)
    {
        ...
        Ret = HI_MPI_CIPHER_HashInit(&stHashAttr, &HashHandle)
              || HI_MPI_CIPHER_HashUpdate(HashHandle, u8Seed, u32Seedlen)
              || HI_MPI_CIPHER_HashUpdate(HashHandle, u8Cnt, 4)
              || HI_MPI_CIPHER_HashFinal(HashHandle, u8Md);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_CIPHER("hash calculte for u8Md failed.\n");
            free(u8Cnt);
            u8Cnt = HI_NULL;
            free(u8Md);
            u8Md = HI_NULL;
                                        ...
                                        HI_U32 *pu32OutLen)
{
    ...
    *pu32OutLen = u32KeyLen;
    ...
    pu8M = (HI_U8*)memalign(ARCH_DMA_MINALIGN, u32Mlen);
    if (pu8M == HI_NULL)
    {
                                        ...
                                        HI_U32 *pu32OutLen)
{
    ...
    *pu32OutLen = u32HLen;
    ...
    case HI_UNF_CIPHER_HASH_TYPE_SHA1:
        if (u32Len != 35)
        {
            HI_ERR_CIPHER("RSA T len error: %u\n", u32Len);
            break;
        }
                                   ...
                                   HI_U8 *pu8Output, HI_U32 *pu32OutLen)
{
    ...
    ret = RSA_Private(&pstRsaDec->stPriKey, pstRsaDec->enCaType, pu8Input, u8EM);
    if (ret != HI_SUCCESS)
    {
                                    ...
                                    HI_U8 *pu8Output, HI_U32 *pu32OutLen)
{
    ...
    return RSA_Private(&pstRsaEnc->stPriKey, pstRsaEnc->enCaType, u8EM, pu8Output);
                             ...
                             HI_U8 *pu8OutSign, HI_U32 *pu32OutSignLen)
{
    ...
    HI_U8  u8EM[CIPHER_MAX_RSA_KEY_LEN] = {0};
    HI_S32 ret = HI_FAILURE;
    HI_UNF_CIPHER_HASH_TYPE_E enHashType;
    HI_UNF_CIPHER_HASH_ATTS_S stHashAttr;
    HI_HANDLE HashHandle;
    ...
    return RSA_Private(&pstRsaSign->stPriKey, pstRsaSign->enCaType, u8EM, pu8OutSign);
}
