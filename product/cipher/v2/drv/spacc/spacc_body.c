               ...
               unsigned long entry_phy_addr, void *entry_via_addr)
{
    ...
    for(i=CIPHER_PKGxN_CHAN_MIN; i<=CIPHER_PKGxN_CHAN_MAX; i++)
    {
        ...
        SPACC_WRITE(CHANn_CIPHER_IN_NODE_START_ADDR(i), GET_ULONG_LOW(page_phy));
        SPACC_WRITE(CHANN_CIPHER_IN_NODE_START_ADDR_HIGH(i), GET_ULONG_HIGH(page_phy));
        SPACC_DBG("CHANn_CIPHER_IN_NODE_CFG[%p]: \t0x%x, PHY: 0x%zx, VIA %p\n", CHANn_CIPHER_IN_NODE_CFG(i), cipher_in_cfg.u32, page_phy, page_via);
        ...
        SPACC_WRITE(CHANn_CIPHER_OUT_NODE_START_ADDR(i), GET_ULONG_LOW(page_phy));
        SPACC_WRITE(CHANN_CIPHER_OUT_NODE_START_ADDR_HIGH(i), GET_ULONG_HIGH(page_phy));
        SPACC_DBG("CHANn_CIPHER_OUT_NODE_CFG[%p]: \t0x%x, PHY: 0x%zx, VIA %p\n", CHANn_CIPHER_OUT_NODE_CFG(i), cipher_out_cfg.u32, page_phy, page_via);
        ...
        SPACC_WRITE(CHANn_HASH_IN_NODE_START_ADDR(i), GET_ULONG_LOW(page_phy));
        SPACC_WRITE(CHANN_HASH_IN_NODE_START_ADDR_HIGH(i), GET_ULONG_HIGH(page_phy));
        SPACC_DBG("CHANn_HASH_IN_NODE_CFG[%p]: \t0x%x, PHY: 0x%zx, VIA %p\n", CHANn_HASH_IN_NODE_CFG(i), hash_in_cfg.u32, page_phy, page_via);
...
int spacc_symc_gettag(unsigned int chn_num, unsigned char *tag)
{
    ...
    flush_cache (CIPHER_ALIGN_DOWN((HI_SIZE_T)info->entry_symc_out), CIPHER_SIZE_ALIGN((HI_SIZE_T)info->entry_symc_out, SPACC_PAGE_SIZE));
...
int spacc_symc_setiv(unsigned int chn_num, unsigned char *iv, unsigned int ivlen)
{
    ...
    cipher_memset_s(info->symc_IV, sizeof(info->symc_IV), 0, sizeof(info->symc_IV));
                         ...
                         unsigned int ctrl)
{
    ...
    flush_cache (CIPHER_ALIGN_DOWN(buf_phy), CIPHER_SIZE_ALIGN(buf_phy, buf_size));
...
int spacc_symc_start(unsigned int chn_num, unsigned int decrypt, unsigned int iv_set_flag)
{
    ...
    flush_cache (CIPHER_ALIGN_DOWN((HI_SIZE_T)info->entry_symc_in), CIPHER_SIZE_ALIGN((HI_SIZE_T)info->entry_symc_in, SPACC_PAGE_SIZE));
    flush_cache (CIPHER_ALIGN_DOWN((HI_SIZE_T)info->entry_symc_out), CIPHER_SIZE_ALIGN((HI_SIZE_T)info->entry_symc_out, SPACC_PAGE_SIZE));
                                ...
                                unsigned int *state)
{
    ...
    flush_cache (CIPHER_ALIGN_DOWN((HI_SIZE_T)info->entry_digest_in), CIPHER_SIZE_ALIGN((HI_SIZE_T)info->entry_digest_in, SPACC_PAGE_SIZE));
...
HI_S32 spacc_symc_proc_status(CIPHER_CHN_STATUS_S *pstCipherStatus)
{
    ...
    for(i = 0; i < 8; i++)
    {
        ...
        if(0 != i)
        {
            pu32RegAddr = CHANn_CIPHER_CTRL(i);
        }
        else
        {
            pu32RegAddr = CHAN0_CIPHER_CTRL;
        }

        stCtrl.u32 = SPACC_READ(pu32RegAddr);
        pstCipherStatus[i].bIsDecrypt = stCtrl.bits.sym_chn_decrypt;

        switch(stCtrl.bits.sym_chn_alg_sel)
        {
            ...
            {
                pstCipherStatus[i].ps8Alg = "SM1 ";
                break;
            }
            case HI_UNF_CIPHER_ALG_SM4:
            {
                pstCipherStatus[i].ps8Alg = "SM4 ";
                break;
            }
            case HI_UNF_CIPHER_ALG_DMA:
            {
                pstCipherStatus[i].ps8Alg = "DMA ";
                break;
            }
            default:
            {
        ...
        switch(stCtrl.bits.sym_chn_alg_mode)
        {
        ...
        if(stCtrl.bits.sym_chn_alg_sel == HI_UNF_CIPHER_ALG_AES)
        {
            switch(stCtrl.bits.sym_chn_key_length)
            {
        ...
        else if(stCtrl.bits.sym_chn_alg_sel == HI_UNF_CIPHER_ALG_DES)
        {
            pstCipherStatus[i].u32KeyLen = 64;
        }
        else if(stCtrl.bits.sym_chn_alg_sel == HI_UNF_CIPHER_ALG_3DES)
        {
            switch(stCtrl.bits.sym_chn_key_length)
            {
        ...
        else if(stCtrl.bits.sym_chn_alg_sel == HI_UNF_CIPHER_ALG_SM1)
        {
            pstCipherStatus[i].u32KeyLen = 384;
        }
        else if(stCtrl.bits.sym_chn_alg_sel == HI_UNF_CIPHER_ALG_SM4)
        {
            pstCipherStatus[i].u32KeyLen = 128;
        }
        else
        ...
        if(stCtrl.bits.sym_chn_key_sel)
        {
        ...
        if(0 != i)
        {
            pu32RegAddr = CHANn_CIPHER_IN_BUF_RPTR(i);
            pstCipherStatus[i].u32DataInAddr = SPACC_READ(pu32RegAddr);
        }
        else
        {
            pstCipherStatus[0].u32DataInAddr = CIPHER_CIPHER_REG_BASE_ADDR_PHY + 0x420;
        }

        /* get data out */
        if(0 != i)
        {
            pu32RegAddr = CHANn_CIPHER_OUT_BUF_RPTR(i);
            pstCipherStatus[i].u32DataOutAddr = SPACC_READ(pu32RegAddr);
        }
        else
        {
            pstCipherStatus[0].u32DataOutAddr = CIPHER_CIPHER_REG_BASE_ADDR_PHY + 0x80;
        }

        for(j=0; j<4; j++)
        {
            u32Val = SPACC_READ(CHANn_CIPHER_IVOUT(i) + j*4);
            HEX2STR(pstCipherStatus[i].u8IVString+j*8, (HI_U8)(u32Val & 0xFF));
            HEX2STR(pstCipherStatus[i].u8IVString+j*8+2, (HI_U8)((u32Val >> 8) & 0xFF));
            HEX2STR(pstCipherStatus[i].u8IVString+j*8+4, (HI_U8)((u32Val >> 16) & 0xFF));
            HEX2STR(pstCipherStatus[i].u8IVString+j*8+6, (HI_U8)((u32Val >> 24) & 0xFF));
        }

        /* get INT RAW status */
        stIntRaw.u32 = SPACC_READ(CIPHER_INT_RAW);
        pstCipherStatus[i].bInINTRaw = (stIntRaw.bits.cipher_chn_ibuf_raw >> i) & 0x1;
        pstCipherStatus[i].bOutINTRaw = (stIntRaw.bits.cipher_chn_obuf_raw >> i) & 0x1;

        /* get INT EN status */
        stIntEn.u32 = SPACC_READ(CIPHER_INT_EN);
        pstCipherStatus[i].bInINTEn= stIntEn.bits.cipher_nsec_int_en;
        pstCipherStatus[i].bOutINTEn= ( stIntEn.bits.cipher_chn_ibuf_en >> i) & 0x1;
        pstCipherStatus[i].bInINTAllEn= ( stIntEn.bits.cipher_chn_obuf_en >> i) & 0x1;

        /* get INT_OINTCFG */
        pu32RegAddr = CHANn_CIPHER_IN_NODE_CFG(i);
        stInNode.u32 = SPACC_READ(pu32RegAddr);
        pstCipherStatus[i].u32OutINTCount = stInNode.bits.cipher_in_node_mpackage_int_level;
    }
