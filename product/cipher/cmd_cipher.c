...
HI_S32 SHA1(HI_VOID)
{
    HI_U8* u8Hash = HI_NULL;
    HI_U32 i;
    HI_S32 ret = HI_FAILURE;
    HI_U8* u8Buf = HI_NULL;
    HI_HANDLE hHandle = 0xffffffff;
    HI_UNF_CIPHER_HASH_ATTS_S stHashAttr;

    ret = HI_UNF_CIPHER_Init();
    if (ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("Cipher init for %s sample test failed.\n", __FUNCTION__);
        return ret;
    }

    u8Hash = (HI_U8*)memalign(ARCH_DMA_MINALIGN, 20);
    if (u8Hash == HI_NULL)
    {
        HI_ERR_CIPHER("memalign for hash failed!\n");
        (HI_VOID)HI_UNF_CIPHER_DeInit();
        return HI_FAILURE;
    }

    u8Buf =(HI_U8*)memalign(ARCH_DMA_MINALIGN, 1024);
    if (u8Buf == HI_NULL)
    {
        HI_ERR_CIPHER("memalign for u8Buf failed!\n");
        free(u8Hash);
        u8Hash = HI_NULL;
        (HI_VOID)HI_UNF_CIPHER_DeInit();
        return HI_FAILURE;
    ...
    for(i = 0; i < 3; i++)
    {
        if(i == 2)
        {
            ...
            (HI_VOID)HI_UNF_CIPHER_HashUpdate(hHandle, u8Buf, 1024);
            (HI_VOID)HI_UNF_CIPHER_HashFinal(hHandle, u8Hash);
        }
        else
        {
            stHashAttr.eShaType = HI_UNF_CIPHER_HASH_TYPE_SHA1;
            (HI_VOID)HI_UNF_CIPHER_HashInit(&stHashAttr, &hHandle);
            memcpy(u8Buf, sha1_buf[i], sha1_buflen[i]);
            (HI_VOID)HI_UNF_CIPHER_HashUpdate(hHandle, u8Buf, sha1_buflen[i]);
            (HI_VOID)HI_UNF_CIPHER_HashFinal(hHandle, u8Hash);
        }

        (HI_VOID)Cipher_printBuffer("Sha1 result:", u8Hash, 20);

        if(memcmp(u8Hash, sha1_sum[i], 20) != 0)
        {
            HI_ERR_CIPHER("\033[0;31m" "SHA1 run failed, sample %d!\n" "\033[0m", i);
            (HI_VOID)Cipher_printBuffer("golden data:", sha1_sum[i], 20);
            free(u8Hash);
            u8Hash = HI_NULL;
            free(u8Buf);
            u8Buf = HI_NULL;
...
HI_S32 SHA256(HI_VOID)
{
    ...
    u8Hash = (HI_U8*)memalign(ARCH_DMA_MINALIGN, 32);
    if (u8Hash == HI_NULL)
    {
        HI_ERR_CIPHER("memalign for hash failed!\n");
        (HI_VOID)HI_UNF_CIPHER_DeInit();
    ...
    if (u8Buf == HI_NULL)
    {
        HI_ERR_CIPHER("memalign for buf failed!\n");
        free(u8Hash);
        u8Hash = HI_NULL;
        (HI_VOID)HI_UNF_CIPHER_DeInit();
        return HI_FAILURE;
    ...
    memset(u8Hash, 0, 32);
    memset(hHandle, 0xffffffff, sizeof(hHandle));

#ifdef CIPHER_MHASH_SUPPORT
    for(i = 0; i < 3; i++)
    {
        stHashAttr.eShaType = HI_UNF_CIPHER_HASH_TYPE_SHA256;
        (HI_VOID)HI_UNF_CIPHER_HashInit(&stHashAttr, &hHandle[i]);
    }
    ...
    for(i = 0; i < 3; i++)
    {
        if(i == 2)
        {
            ...
            (HI_VOID)HI_UNF_CIPHER_HashUpdate(hHandle[i], u8Buf, 1024);
        }
        else
        {
            (HI_VOID)HI_UNF_CIPHER_HashUpdate(hHandle[i], sha1_buf[i], sha1_buflen[i]);
        }
    }
    for(i = 0; i < 3; i++)
    {
        (HI_VOID)HI_UNF_CIPHER_HashFinal(hHandle[i], u8Hash);

        (HI_VOID)Cipher_printBuffer("Sha256 result:", u8Hash, 32);
        if(memcmp(u8Hash, sha2_sum[i], 32) != 0)
        {
            HI_ERR_CIPHER("\033[0;31m" "SHA256 run failed, sample %d!\n" "\033[0m", i);
            (HI_VOID)Cipher_printBuffer("golden data:", sha2_sum[i], 32);
            free(u8Hash);
            u8Hash = HI_NULL;
            free(u8Buf);
            u8Buf = HI_NULL;
    ...
    for(i = 0; i < 3; i++)
    {
        if(i == 2)
        {
            ...
            stHashAttr.eShaType = HI_UNF_CIPHER_HASH_TYPE_SHA256;
            (HI_VOID)HI_UNF_CIPHER_HashInit(&stHashAttr, &hHandle[0]);
            (HI_VOID)HI_UNF_CIPHER_HashUpdate(hHandle[0], u8Buf, 1024);
            (HI_VOID)HI_UNF_CIPHER_HashFinal(hHandle[0], u8Hash);
        }
        else
        {
            stHashAttr.eShaType = HI_UNF_CIPHER_HASH_TYPE_SHA256;
            (HI_VOID)HI_UNF_CIPHER_HashInit(&stHashAttr, &hHandle[0]);
            memcpy(u8Buf, sha1_buf[i], sha1_buflen[i]);
            (HI_VOID)HI_UNF_CIPHER_HashUpdate(hHandle[0], u8Buf, sha1_buflen[i]);
            (HI_VOID)HI_UNF_CIPHER_HashFinal(hHandle[0], u8Hash);
        }
...
HI_S32 HMAC_SHA1(HI_VOID)
{
    ...
    if (u8Hash == HI_NULL)
    {
        HI_ERR_CIPHER("memalign for u8Hash failed!\n");
        (HI_VOID)HI_UNF_CIPHER_DeInit();
        return HI_FAILURE;
    }
    ...
    u8HmacKey = (HI_U8*)memalign(ARCH_DMA_MINALIGN, 16);
    if (u8HmacKey == HI_NULL)
    {
        HI_ERR_CIPHER("memalign for u8HmacKey failed!\n");
        free(u8Hash);
        u8Hash = HI_NULL;
        free(u8Buf);
        u8Buf = HI_NULL;
    ...
    memset(u8Hash, 0, 20);
    memcpy(u8HmacKey, u8HMACSha1Key, 16);
    ...
    for(i = 0; i < 1; i++)
    {
        if(i == 2)
        {
            ...
            stHashAttr.pu8HMACKey = u8HmacKey;
        ...
        else
        {
            ...
            memcpy(u8Buf, sha1_buf[i], sha1_buflen[i]);
            ret = HI_UNF_CIPHER_HashUpdate(hHandle, u8Buf, sha1_buflen[i]);
            if(HI_SUCCESS != ret)
            {
                HI_ERR_CIPHER("hash update failed!\n");
                ret = HI_FAILURE;
        ...
        (HI_VOID)Cipher_printBuffer("hmac-Sha1 result:", u8Hash, 20);

        if(memcmp(u8Hash, hmac_sha1_sum[i], 20) != 0)
        {
            HI_ERR_CIPHER("\033[0;31m" "HMAC SHA1 run failed, sample %d!\n" "\033[0m", i);
            (HI_VOID)Cipher_printBuffer("golden data:", hmac_sha1_sum[i], 20);
            ret = HI_FAILURE;
            goto EXIT;
        }
    }
...
HI_S32 HMAC_SHA256(HI_VOID)
{
    ...
    memset(u8Hash, 0, 32);
    memcpy(u8HmacKey, u8HMACSha2Key, 16);
    ...
    for(i = 0; i < 3; i++)
    {
        if(i == 2)
        {
            ...
            for(j=0; j<707; j+=101)
            {
                ret = HI_UNF_CIPHER_HashUpdate(hHandle, &u8Buf[j], 101);
                if(HI_SUCCESS != ret)
                {
                    HI_ERR_CIPHER("hash update failed!\n");
                    goto EXIT;
            ...
            for(; j<1024; j++)
            {
                ret = HI_UNF_CIPHER_HashUpdate(hHandle, &u8Buf[j], 1);
                if(HI_SUCCESS != ret)
                {
                    HI_ERR_CIPHER("hash update failed!\n");
                    goto EXIT;
        ...
        (HI_VOID)Cipher_printBuffer("hmac-Sha256 result:", u8Hash, 32);

        if(memcmp(u8Hash, hmac_sha2_sum[i], 32) != 0)
        {
            HI_ERR_CIPHER("\033[0;31m" "HMAC SHA256 run failed, sample %d!\n" "\033[0m", i);
            (HI_VOID)Cipher_printBuffer("golden data:", hmac_sha2_sum[i], 32);
            ret = HI_FAILURE;
            goto EXIT;
        }
    }
...
static int test_hash(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
...
int test_hash_all(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    SHA1();
    SHA256();
    HMAC_SHA1();
...
static HI_S32 Hash_SHA1_Calc(HI_U8 *pu8Buffer, HI_U32 u32Len)
{
    ...
    (HI_VOID)Cipher_printBuffer("SHA1 result:", au8Hash, 20);
...
static HI_S32 Hash_SHA2_Calc(HI_U8 *pu8Buffer, HI_U32 u32Len)
{
    ...
    (HI_VOID)Cipher_printBuffer("SHA2 result:", au8Hash, 32);

    return HI_SUCCESS;
}

static int Hash_Calc(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    u32Addr = (HI_U8*)simple_strtoul(argv[2], NULL, 0);
    u32Len = (HI_U32)simple_strtoul(argv[3], NULL, 0);

    printf("u32HashType: 0x%x, Address: %p, Length: 0x%x\n", u32HashType, u32Addr, u32Len);
...
HI_S32 RSA_SIGN_VERIFY(HI_UNF_CIPHER_RSA_SIGN_SCHEME_E enScheme, HI_U32 option)
{
    HI_S32 ret = HI_SUCCESS;
    HI_U8  u8Sign[256];
    HI_U32 u32SignLen;
    ...
    HI_U32 time_start;
    HI_U8 *pu8HashDataSign = HI_NULL;
    HI_U8 *pu8HashDataVerify = HI_NULL;

    if (option == 0)
    {
        pu8HashDataSign = sha256_sum;
        pu8HashDataVerify = sha256_sum;
    }
    else if (option == 1)
    {
        pu8HashDataSign = HI_NULL;
        pu8HashDataVerify = HI_NULL;
    }
    else
    {
        pu8HashDataSign = sha256_sum;
    ...
    ret = HI_UNF_CIPHER_RsaSign(&stRsaSign, test_data, sizeof(test_data) - 1, pu8HashDataSign, u8Sign, &u32SignLen);
    if ( HI_SUCCESS != ret )
    {
    ...
    case HI_UNF_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_V15_SHA256:
        ...
        {
            HI_ERR_CIPHER("HI_UNF_CIPHER_RsaSign failed\n");
            (HI_VOID)Cipher_printBuffer("sign", u8Sign, u32SignLen);
            (HI_VOID)Cipher_printBuffer("golden", RES, sizeof(RES));
            (HI_VOID)HI_UNF_CIPHER_DeInit();
            return HI_FAILURE;
        }
        break;
    ...
    time_start = get_timer(0);
    ret = HI_UNF_CIPHER_RsaVerify(&stRsaVerify, test_data, sizeof(test_data) - 1, pu8HashDataVerify, u8Sign, u32SignLen);
    if ( HI_SUCCESS != ret )
    {
...
static int rsa_sign_test(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    HI_S32 ret = HI_SUCCESS;

    ret = RSA_SIGN_VERIFY(HI_UNF_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_V15_SHA256, 0);
    if (ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("RSA_SIGN_VERIFY_1 failed, ret:0x%x\n", ret);
        return ret;
    }

    ret = RSA_SIGN_VERIFY(HI_UNF_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_V15_SHA256, 1);
    if (ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("RSA_SIGN_VERIFY_2 failed, ret:0x%x\n", ret);
        return ret;
    }

    ret = RSA_SIGN_VERIFY(HI_UNF_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_V15_SHA256, 2);
    if (ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("RSA_SIGN_VERIFY_3 failed, ret:0x%x\n", ret);
        return ret;
    }
    ret = RSA_SIGN_VERIFY(HI_UNF_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_PSS_SHA256, 0);
    if (ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("RSA_SIGN_VERIFY_1 failed, ret:0x%x\n", ret);
        return ret;
    ...
    ret = RSA_SIGN_VERIFY(HI_UNF_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_PSS_SHA256, 1);
    if (ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("RSA_SIGN_VERIFY_2 failed, ret:0x%x\n", ret);
        return ret;
    ...
    ret = RSA_SIGN_VERIFY(HI_UNF_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_PSS_SHA256, 2);
    if (ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("RSA_SIGN_VERIFY_3 failed, ret:0x%x\n", ret);
        return ret;
...
HI_S32 PKCS_PUB_ENC(HI_UNF_CIPHER_RSA_ENC_SCHEME_E enScheme, HI_U8 *pu8Expect)
{
    ...
    if(pu8Expect != HI_NULL)
    {
        ...
        {
            HI_ERR_CIPHER("HI_UNF_CIPHER_RsaPublicEnc failed\n");
            (HI_VOID)Cipher_printBuffer("enc", u8Enc, u32OutLen);
            (HI_VOID)Cipher_printBuffer("expect", pu8Expect, 256);
            (HI_VOID)HI_UNF_CIPHER_DeInit();
            return HI_FAILURE;
        }
    }
    ...
    if (ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("HI_UNF_CIPHER_RsaPrivateDecrypt failed\n");
        (HI_VOID)HI_UNF_CIPHER_DeInit();
        return HI_FAILURE;
    }
    ...
    if(enScheme == HI_UNF_CIPHER_RSA_ENC_SCHEME_NO_PADDING)
    {
        ...
        {
            HI_ERR_CIPHER("HI_UNF_CIPHER_RsaPrivateDec memcmp failed\n");
            (HI_VOID)Cipher_printBuffer("enc", u8Enc, u32OutLen);
            (HI_VOID)Cipher_printBuffer("dec", u8Dec, u32OutLen);
            (HI_VOID)Cipher_printBuffer("expect", NO_PADDING, 256);
            (HI_VOID)HI_UNF_CIPHER_DeInit();
            return HI_FAILURE;
        }
    }
    else
    {
        if((sizeof(test_data) - 1) != u32OutLen)
        {
            ...
            printf("dec: 0x%x, expect: %u\n", u32OutLen, (HI_U32)(sizeof(test_data) - 1));
            (HI_VOID)HI_UNF_CIPHER_DeInit();
            return HI_FAILURE;
        }
        if(memcmp(u8Dec, test_data, sizeof(test_data) - 1) != 0)
        {
            ...
            (HI_VOID)Cipher_printBuffer("enc", u8Enc, u32OutLen);
            (HI_VOID)Cipher_printBuffer("expect", test_data, sizeof(test_data) - 1);
            (HI_VOID)HI_UNF_CIPHER_DeInit();
            return HI_FAILURE;
        }
    }
...
HI_S32 PKCS_PRI_ENC(HI_UNF_CIPHER_RSA_ENC_SCHEME_E enScheme)
{
    ...
    memset(&stRsaEnc, 0, sizeof(stRsaEnc));
    memset(&stRsaDec, 0, sizeof(stRsaDec));
    stRsaEnc.enScheme = enScheme;
    stRsaDec.enScheme = enScheme;
    ...
    case HI_UNF_CIPHER_RSA_ENC_SCHEME_BLOCK_TYPE_1:
        if(memcmp(u8Hash, sha256_sum, u32HLen) != 0)
        {
            ...
            (HI_VOID)Cipher_printBuffer("sign", u8Sign, u32SignLen);
            (HI_VOID)Cipher_printBuffer("sha256_sum", sha256_sum, sizeof(sha256_sum));
            (HI_VOID)HI_UNF_CIPHER_DeInit();
            return HI_FAILURE;
        }
        break;
...
static int rsa_enc_test(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    HI_S32 ret = HI_FAILURE;
    ...
    if (ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("%s call PKCS_PUB_ENC with NO_PADDING failed.\n", __FUNCTION__);
        return ret;
    }
    ...
    if (ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("%s call PKCS_PUB_ENC with BLOCK_TYPE_2 failed.\n", __FUNCTION__);
        return ret;
    }

    ret = PKCS_PUB_ENC(HI_UNF_CIPHER_RSA_ENC_SCHEME_RSAES_OAEP_SHA1, HI_NULL);
    if (ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("%s call PKCS_PUB_ENC with RSAES_OAEP_SHA1 failed.\n", __FUNCTION__);
        return ret;
    }
    ...
    if (ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("%s call PKCS_PUB_ENC with RSAES_OAEP_SHA256 failed.\n", __FUNCTION__);
        return ret;
    }
    ...
    if (ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("%s call PKCS_PUB_ENC with RSAES_PKCS1_V1_5 failed.\n", __FUNCTION__);
        return ret;
    }
    ...
    if (ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("%s call PKCS_PRI_ENC with BLOCK_TYPE_1 failed.\n", __FUNCTION__);
        return ret;
    }
                                                ...
                                                const HI_U8 u8KeyBuf[16], const HI_U8 u8IVBuf[16])
{
    ...
    CipherCtrl.enCaType = (HI_UNF_CIPHER_CA_TYPE_E)bKeyByCA;
           ...
           const unsigned char u8Src[16], const unsigned char u8Dst[16])
{
    ...
    if(HI_SUCCESS != s32Ret)
    {
        HI_ERR_CIPHER("Error: HI_UNF_CIPHER_Init failed!\n");
        return HI_FAILURE;
    }
    ...
    input = (HI_SIZE_T)memalign(ARCH_DMA_MINALIGN, 16);
    if (HI_NULL == (HI_U8 *)input)
    {
        HI_ERR_CIPHER("Error: memalign for input failed!\n");
        (HI_VOID)HI_UNF_CIPHER_DestroyHandle(hTestchnid);
        (HI_VOID)HI_UNF_CIPHER_DeInit();
        return HI_FAILURE;
    }
    output =(HI_SIZE_T)memalign(ARCH_DMA_MINALIGN, 16);
    if (HI_NULL == (HI_U8 *)input)
    {
        HI_ERR_CIPHER("Error: memalign for output failed!\n");
        free((HI_U8 *)input);
        input = 0;
        (HI_VOID)HI_UNF_CIPHER_DestroyHandle(hTestchnid);
        (HI_VOID)HI_UNF_CIPHER_DeInit();
    ...
    s32Ret = HI_UNF_CIPHER_Encrypt(hTestchnid, (HI_SIZE_T)input, (HI_SIZE_T)output, 16);
    if(0 != s32Ret)
    {
    ...
    if(memcmp((HI_U8*)output, u8Dst, 16))
    {
        HI_ERR_CIPHER("Cipher encrypt failed. hTestchnid=0x%x\n", hTestchnid);
        s32Ret = -1;
        goto __CIPHER_EXIT__;
    ...
    memcpy((HI_U8*)input, u8Dst, 16);
    s32Ret = HI_UNF_CIPHER_Decrypt(hTestchnid, (HI_SIZE_T)input, (HI_SIZE_T)output, 16);
    if(0 != s32Ret)
    {
    ...
    if(memcmp((HI_U8*)output, u8Src, 16))
    {
...
static int AES_CBC128(int argc, char* const argv[])
{
...
/* encrypt data using special chn*/
static int AES_CTR128(int argc, char* const argv[])
{
...
/* encrypt data using special chn*/
static int AES_CFB128(int argc, char* const argv[])
{
...
static int cipher_test(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
...
static int rng_test(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    HI_S32 i;
    HI_U32 u32Random = 0;
    ...
    for(i=0; i<16; i++)
    {
        (HI_VOID)HI_UNF_CIPHER_GetRandomNumber(&u32Random);
        if(u32Random == 0)
        {
            i--;
            continue;
...
static HI_U8 aes_128_enc_key_with_128bit_otp_zero_key[16]  =
    {0xec,0x8c,0xa4,0x6e,0x7c,0xd9,0x09,0x74,0x85,0xaf,0x67,0xff,0xbc,0x91,0xa0,0x23};
static HI_U8 aes_128_enc_key_with_256bit_otp_zero_key[16]  =
    {0xc9,0xbe,0x57,0x5a,0xa3,0xeb,0x19,0x9a,0xd2,0x41,0x1c,0xec,0xcb,0x45,0x61,0xe8};

/* use 32 bytes 0x11 otp key */
static HI_U8 aes_128_enc_key_with_256bit_otp_gold_key[16]  =
    {0xed,0x53,0xa8,0x0a,0xa1,0x68,0x3d,0xf4,0x95,0x59,0xc4,0x1a,0x54,0x13,0xa4,0xf9};
...
static int klad_test(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    (HI_VOID)Cipher_printBuffer("encrypt key", u8Key, 16);

    if ((memcmp(u8Key, aes_128_enc_key, 16) != 0)
        && (memcmp(u8Key, aes_128_enc_key_with_128bit_otp_zero_key, 16) != 0)
        && (memcmp(u8Key, aes_128_enc_key_with_256bit_otp_zero_key, 16) != 0)
        && ((memcmp(u8Key, aes_128_enc_key_with_256bit_otp_gold_key, 16) != 0)))
    {
    ...
    u8InBuf = (HI_SIZE_T)memalign(ARCH_DMA_MINALIGN, 16);
    if (HI_NULL == (HI_U8 *)u8InBuf)
    {
        HI_ERR_CIPHER("memalign for u8InBuf failed!\n");
        HI_UNF_CIPHER_DestroyHandle(hTestchnid);
        HI_UNF_CIPHER_DeInit();
        return HI_FAILURE;
    }
    u8OutBuf =(HI_SIZE_T)memalign(ARCH_DMA_MINALIGN, 16);
    if (HI_NULL == (HI_U8 *)u8OutBuf)
    {
        HI_ERR_CIPHER("memalign for u8OutBuf failed!\n");
        free((HI_U8 *)u8InBuf);
        u8InBuf = 0;
        HI_UNF_CIPHER_DestroyHandle(hTestchnid);
        HI_UNF_CIPHER_DeInit();
    ...
    memset((HI_U8*)u8InBuf, 0x0, u32TestDataLen);
    memcpy((HI_U8*)u8InBuf, aes_128_src_buf, u32TestDataLen);
    (HI_VOID)Cipher_printBuffer("clear text:", (HI_U8*)u8InBuf, u32TestDataLen);

    memset((HI_U8*)u8OutBuf, 0x0, u32TestDataLen);

    s32Ret = HI_UNF_CIPHER_Encrypt(hTestchnid, (HI_SIZE_T)u8InBuf, (HI_SIZE_T)u8OutBuf, u32TestDataLen);
    if(s32Ret != HI_SUCCESS)
    {
    ...
    if ( 0 != memcmp((HI_U8*)u8OutBuf, aes_128_dst_buf, u32TestDataLen) )
    {
   ...
   /* For decrypt */
    memcpy((HI_U8*)u8InBuf, aes_128_dst_buf, u32TestDataLen);
    memset((HI_U8*)u8OutBuf, 0x0, u32TestDataLen);
    ...
    s32Ret = HI_UNF_CIPHER_Decrypt(hTestchnid, (HI_SIZE_T)u8InBuf, (HI_SIZE_T)u8OutBuf, u32TestDataLen);
    if(s32Ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("Cipher decrypt failed.\n");
        s32Ret = HI_FAILURE;
    ...
    /* compare */
    if ( 0 != memcmp((HI_U8*)u8OutBuf, aes_128_src_buf, u32TestDataLen) )
    {
        HI_ERR_CIPHER("Memcmp failed!\n");
        s32Ret = HI_FAILURE;
        goto __CIPHER_EXIT__;
