...
#define OSAL_STRCPY(pdes, psrc, len)        strncpy(pdes, psrc, len)
#define OSAL_STRCMP(pdes, psrc, len)        strncmp(pdes, psrc, len)
...
#define OSAL_SCHED_LOCK                     sched_clock
...
#define OSAL_STRCPY(pdes, psrc, len)        osal_strncpy(pdes, psrc, len)
#define OSAL_STRCMP(pdes, psrc, len)        osal_strncmp(pdes, psrc, len)
...
#define OSAL_SMSTRTOUL(str, end, base)      simple_strtoul(str, end, base)
#define OSAL_SCHED_LOCK                     osal_sched_clock
...
#define HDMI_MUTEX_LOCK(a)  \
do{\
    HI_S32 s32Ret = osal_down((HDMI_MUTEX *)&(a));\
    if (0 != s32Ret)\
        {HDMI_ERR("osal_down failed! s32Ret=%d\n", s32Ret);}\
}while(0)

#define HDMI_MUTEX_UNLOCK(a)         	osal_up((HDMI_MUTEX *)&(a))
...
#define IS_STR_EQUAL(p1,p2) ({\
    ( p1 && p2 && (OSAL_STRLEN(p1)==OSAL_STRLEN(p2)) && (HDMI_STRNCMP(p1,p2,OSAL_STRLEN(p1))==0) ) ? HI_TRUE : HI_FALSE;\
})
