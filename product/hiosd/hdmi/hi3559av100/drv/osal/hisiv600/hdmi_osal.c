...
struct file* HDMI_OSAL_FileOpen(const HI_S8* ps8FileName, OSAL_FILE_FLAG_S eFlags)
{
    ...
    pFile = HDMI_FILP_OPEN(ps8FileName, eFlags, HDMI_FILE_MODE);
...
HI_S32 HDMI_OSAL_FileRead(struct file * pFile,  HI_U8* ps8Buf, HI_U32 u32Len)
{
    ...
    s32ReadLen = HDMI_FILP_READ(ps8Buf, u32Len, pFile);
...
HI_S32 HDMI_OSAL_FileWrite(struct file* pFile, HI_S8* ps8Buf, HI_U32 u32Len)
{
    ...
    s32WriteLen = HDMI_FILP_FWRITE(ps8Buf, u32Len, pFile);
...
{
    return HDMI_STRNCPY(pszDest, pszSrc, ulLen);
}
...
{
    return HDMI_STRNCAT(pszDest, pszSrc, ulLen);
}
...
/** vsnprintf pszSrc
**/
HI_S32 HDMI_OSAL_Vsnprintf(HI_CHAR *pszStr, HI_SIZE_T ulLen, const HI_CHAR *pszFormat, HDMI_VA_LIST stVAList)
{
    return HDMI_VSNPRINTF(pszStr, ulLen, pszFormat, stVAList);
}
