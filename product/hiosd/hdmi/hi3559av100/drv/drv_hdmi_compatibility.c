...
#define HDMI_MAX_DELAY_TIME     (5000)
...
HI_S32 FormatDelayGet(HDMI_DEVICE_S* pstHdmiDev, HI_U32 *pu32DelayTime)
{
    ...
    if(HDMI_MAX_DELAY_TIME < u32FmtDelay)
    {
        u32FmtDelay = HDMI_MAX_DELAY_TIME;
        HDMI_INFO("Fmt delay too long, force change to %dms \n", u32FmtDelay);
    }
...
HI_S32 AvMuteDelayGet(HDMI_DEVICE_S* pstHdmiDev, HI_U32 *pu32DelayTime)
{
    ...
    if(HDMI_MAX_DELAY_TIME < u32MuteDelay)
    {
        u32MuteDelay = HDMI_MAX_DELAY_TIME;
        HDMI_INFO("Mute delay too long, force change to %dms \n", u32MuteDelay);
    }
