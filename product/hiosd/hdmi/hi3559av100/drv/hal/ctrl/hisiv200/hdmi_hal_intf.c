...
HI_VOID HAL_HDMI_PhyOutputEnableSet(struct hiHDMI_HAL_S *pstHdmiHal, HI_BOOL bEnable)
{
    if (HI_TRUE == bEnable)
    {
        HAL_HDMI_CtrlSrsSet(pstHdmiHal->stHalCtx.u32HdmiID);
    }
#ifndef HDMI_FPGA_SUPPORT
...
#ifdef HDMI_FRL_SUPPORT
    HDMI_MEMSET(&stStatus, 0, sizeof(FRL_STATUS_S));
#endif
...
#ifdef HDMI_FRL_SUPPORT
    HAL_HDMI_FrlGetStatus(pstHdmiHal->stHalCtx.u32HdmiID, &stStatus);
    pstHwStatus->stFrlStatus.bFrlStart  = stStatus.stTrainInfo.stRunStatus.bStart;
    pstHwStatus->stFrlStatus.bWorkEn    = stStatus.bWorkEn;
    pstHwStatus->stFrlStatus.enWorkMode = stStatus.stTrainInfo.stResult.enWorkMode;
#endif
    return ;
}
...
#endif
#ifdef HDMI_FRL_SUPPORT
        case HDMI_DEBUG_CMD_FRL:
        {
            FRL_DEBUG_S stDebugFrl;
            stDebugFrl.u8Rate = *(HI_U8 *)pData;
            HAL_HDMI_FrlDebug(&stDebugFrl);
        }
            break;
#endif
        default:
...
HI_S32 HAL_HDMI_FrlGetTrainStatus(struct hiHDMI_HAL_S *pstHdmiHal, HDMI_FRL_TRAIN_STATUS_S *pstFrlStatus)
{
    HI_U32       i = 0;
    HI_S32       s32Ret = HI_SUCCESS;
    FRL_STATUS_S stFrlStatus = {0};

    INTF_NULL_CHK(pstHdmiHal);
    INTF_NULL_CHK(pstFrlStatus);

    s32Ret = HAL_HDMI_FrlGetStatus(pstHdmiHal->stHalCtx.u32HdmiID, &stFrlStatus);
    HALTRAINSTATUS_2_INTFTRAINSTATUS(stFrlStatus.stTrainInfo.stResult.enTrainStatus,
                                     pstFrlStatus->enFrlTrainStatus);
    HALTRAINFAIL_2_INTFTRAINFAIL(stFrlStatus.stTrainInfo.stResult.enFailRes,
                                 pstFrlStatus->enTrainFailRes);

    for (i = 0; i < HDMI_FRL_LANE_MAX_NUM; i++)
    {
        HALTRAINPTN_2_INTFTRAINPTN(stFrlStatus.stTrainInfo.stRunStatus.enPattern[i],
                                   pstFrlStatus->enTrainPattern[i]);
    }

    return s32Ret;
}

HI_S32 HAL_HDMI_FrlTrainConfig(struct hiHDMI_HAL_S *pstHdmiHal, HDMI_FRL_TRAIN_CONFIG_S * pstFrlConfig)
{
    FRL_CFG_S stFrlCfg = {0};

    INTF_NULL_CHK(pstHdmiHal);
    INTF_NULL_CHK(pstFrlConfig);

    stFrlCfg.stDFM.bSWDFM = pstFrlConfig->bSWDFM;
    stFrlCfg.u8FrlRate = pstFrlConfig->u8FrlRate;
    stFrlCfg.u8FFELevels = pstFrlConfig->u8FFELevels;
    stFrlCfg.stTimer.u32FltStepTimeOut = pstFrlConfig->stTimer.u32FltStepTimeOut;
    stFrlCfg.stTimer.u32FltTimeOut = pstFrlConfig->stTimer.u32FltTimeOut;
    stFrlCfg.stTimer.u32FltUpdateTimeOut = pstFrlConfig->stTimer.u32FltUpdateTimeOut;
    stFrlCfg.stTimer.u32FltWaitConfigMs = pstFrlConfig->stTimer.u32FltWaitConfigMs;
    stFrlCfg.stTimer.u32FltWaitResultMs = pstFrlConfig->stTimer.u32FltWaitResultMs;
    stFrlCfg.stDFM.u32AvMaxLen = pstFrlConfig->stDFMOut.s32AvMaxLen;
    stFrlCfg.stDFM.u32AvGapLen = pstFrlConfig->stDFMOut.s32AvGapLen;
    stFrlCfg.stDFM.u32BlankMaxLen = pstFrlConfig->stDFMOut.s32BlankMaxLen;
    stFrlCfg.stDFM.u32BlankGapLen = pstFrlConfig->stDFMOut.s32BlankGapLen;
    stFrlCfg.stDFM.u32HblankCycles = pstFrlConfig->stDFMOut.s32HblankCycles;
    stFrlCfg.stDFM.u32HactiveTime = pstFrlConfig->stDFMOut.s32HactiveTime;
    stFrlCfg.stDFM.u32ExtraBlankMaxLen = pstFrlConfig->stDFMOut.s32ExtraBlankMaxLen;
    stFrlCfg.stDFM.u32ExtraBlankGapLen = pstFrlConfig->stDFMOut.s32ExtraBlankGapLen;
    stFrlCfg.stDFM.u32Vactive = pstFrlConfig->stDFMOut.s32Vactive;
    stFrlCfg.stDFM.u32Vblank1Lines = pstFrlConfig->stDFMOut.s32Vblank1Lines;// hsync+hback;
    stFrlCfg.stDFM.bExtraNoGapFlags = pstFrlConfig->stDFMOut.bExtraNoGapFlags;
    stFrlCfg.stDFM.bIsExtraMode = pstFrlConfig->stDFMOut.bIsExtraMode;

    return HAL_HDMI_FrlConfig(pstHdmiHal->stHalCtx.u32HdmiID, &stFrlCfg);
}

HI_S32 HAL_HDMI_FrlTrainStart(struct hiHDMI_HAL_S *pstHdmiHal)
{
    INTF_NULL_CHK(pstHdmiHal);

    return HAL_HDMI_FrlStartTraining(pstHdmiHal->stHalCtx.u32HdmiID);
}

HI_S32 HAL_HDMI_FrlEnable(struct hiHDMI_HAL_S *pstHdmiHal, HI_BOOL bEnable)
{
    INTF_NULL_CHK(pstHdmiHal);

    return HAL_HDMI_FrlEnableWorkEn(pstHdmiHal->stHalCtx.u32HdmiID, bEnable);
}
...
#ifdef HDMI_FRL_SUPPORT
    HAL_HDMI_FrlInit(pstHdmiHal->stHalCtx.u32HdmiID, pstHalInit);
#endif
...
#ifdef HDMI_FRL_SUPPORT
    pstHdmiHal->HAL_HDMI_FrlGetTrainStatus       = HAL_HDMI_FrlGetTrainStatus;
    pstHdmiHal->HAL_HDMI_FrlTrainConfig          = HAL_HDMI_FrlTrainConfig;
    pstHdmiHal->HAL_HDMI_FrlTrainStart           = HAL_HDMI_FrlTrainStart;
    pstHdmiHal->HAL_HDMI_FrlEnable               = HAL_HDMI_FrlEnable;
#endif
...
#ifdef HDMI_FRL_SUPPORT
	HAL_HDMI_FrlDeInit(pstHdmiHal->stHalCtx.u32HdmiID);
#endif
