...
HI_VOID FrlTraningInterrupt(HI_VOID *pParam)
{
    ...
    pstResult = &pstParam->pstInfo->stStatus.stTrainInfo.stResult;
    bFrlNoTimeout = pstParam->pstInfo->stCFG.bFrlNoTimeout;
...
static HI_U32 FrlLMDataGet(LM_TABLE_CONFIG_S *LM_TableConfig)
{
    ...
    osal_printk("read:addr(0x%x), 0x%x \n", HDMIRegReadU32(0x11143200), HDMIRegReadU32(0x11143208));
...
static HI_S32 FrlDfmSet(FRL_DFM_S *pstDFM)
{
    FRL_NULL_CHECK(pstDFM);

    /*When writing to Buffer 1 CMD corresponds to the \
    maximum AV data length in 3-byte u64AvMaxLen*/
    HDMI_AV_MAX_LEN_av_max_lenSet(pstDFM->u32AvMaxLen);

    /*AV slices need to insert between\ the number of Gap  u64AvGapLen*/
    HDMI_AV_GAP_LEN_av_gap_lenSet(pstDFM->u32AvGapLen);

    /*When writing to Buffer 1 CMD corresponds to the \
    maximum blank data length in 3-byte u64BlankMaxLen*/
    HDMI_BLK_MAX_LEN_blk_max_lenSet(pstDFM->u32BlankMaxLen);

    /*BLANK slices need to insert between the number of Gap u64BlankGapLen*/
    HDMI_BLK_GAP_LEN_blk_gap_lenSet(pstDFM->u32BlankGapLen);

    /*blank length u64HblankCycles*/
    HDMI_HBLANK_hblankSet(pstDFM->u32HblankCycles);

    /*active video lengthu64HactiveTime*/
    HDMI_HACTIVE_hactiveSet(pstDFM->u32HactiveTime);

    /*In extra mode,when writing to Buffer corresponds to the maximum \
    blank length  u64ExtraBlankMaxLen*/
    HDMI_EXTRA_BLANK_MAX_LEN_extra_blank_max_lenSet(pstDFM->u32ExtraBlankMaxLen);

    /*In extra mode,BLANK slice need to insert between the number of \
    Gap u64ExtraBlankGapLen*/
    HDMI_EXTRA_BLANK_GAP_LEN_extra_blank_gap_lenSet(pstDFM->u32ExtraBlankGapLen);

    /*extra_mode_en 1:en,0:not enable	bIsExtraMode*/
    HDMI_EXTRA_MODE_EN_extra_mode_enSet(pstDFM->bIsExtraMode);

    /*vsync+vback	 */
    HDMI_VBLANK1_LINES_vblank1_linesSet(pstDFM->u32Vblank1Lines);

    /*VACTIVE   0x312c     u32Vactive */
    HDMI_VACTIVE_vactiveSet(pstDFM->u32Vactive);
    HDMI_EXTRA_NO_GAP_FLAG_extra_no_gap_flagSet(pstDFM->bExtraNoGapFlags);

#if 0
    HDMI_INFO("Flow config bExtraNoGapFlags = %d \r\n",pstDFM->bExtraNoGapFlags);
    HDMI_INFO("Flow config bIsExtraMode = %d \r\n",pstDFM->bIsExtraMode);
    HDMI_INFO("Flow config u32AvGapLen = %d \r\n",pstDFM->u32AvGapLen);
    HDMI_INFO("Flow config u32AvMaxLen = %d \r\n",pstDFM->u32AvMaxLen);
    HDMI_INFO("Flow config u32BlankGapLen = %d \r\n",pstDFM->u32BlankGapLen);
    HDMI_INFO("Flow config u32BlankMaxLen = %d \r\n",pstDFM->u32BlankMaxLen);
    HDMI_INFO("Flow config u32ExtraBlankGapLen = %d \r\n",pstDFM->u32ExtraBlankGapLen);
    HDMI_INFO("Flow config u32ExtraBlankMaxLen = %d \r\n",pstDFM->u32ExtraBlankMaxLen);
    HDMI_INFO("Flow config u32HactiveTime = %d \r\n",pstDFM->u32HactiveTime);
    HDMI_INFO("Flow config u32HblankCycles = %d \r\n",pstDFM->u32HblankCycles);
    HDMI_INFO("Flow config u32Vactive = %d \r\n",pstDFM->u32Vactive);
    HDMI_INFO("Flow config u32Vblank1Lines = %d \r\n",pstDFM->u32Vblank1Lines);
#endif
    return HI_SUCCESS;
}
...
static HI_S32 FrlFltStatusInfoGet(HDMI_DEVICE_ID_E enHdmi)
{
	...
	HI_U32 u32FltLtp = 0,u32FrlModeAct = 0;
    FRL_TRAIN_RUN_STATUS_S stRunStatus;

    HDMI_MEMSET(&stRunStatus,0,sizeof(stRunStatus));
...
HI_S32 HAL_HDMI_FrlConfig(HDMI_DEVICE_ID_E enHdmi, FRL_CFG_S *pstCfg)
{
    ...
    HDMI_TX_CHANNEL_SEL_flbk_selSet(pstCfg->stDFM.bSWDFM);

    if (HI_TRUE == pstCfg->stDFM.bSWDFM)
    {
        if (HI_SUCCESS != FrlDfmSet(&pstCfg->stDFM))
        {
        	HDMI_ERR("Data flow set error!\r\n");
        	return HI_FAILURE;
        }
    }
