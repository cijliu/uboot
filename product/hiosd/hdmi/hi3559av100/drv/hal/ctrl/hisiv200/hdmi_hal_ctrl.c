...
static HI_VOID CtrlVideoPathDeepClrSet(HI_BOOL bEnalbeGcp,HDMI_DEEP_COLOR_E enDeepColorBit)
{
    ...
    if (HI_TRUE == HDMI_TX_CHANNEL_reg_vid_bypass_selGet())
    {
        ...
        u32Reg = HDMIRegReadU32(0x11141800);
        u32Reg &= 0xFFFFFFFC;
        u32Reg |= enDeepColorBit;
        HDMIRegWriteU32(0x11141800, u32Reg);
    }
    else
    {
...
static HI_VOID CtrlVideoColorDwsmVertSet(HI_BOOL bDwsmVert)
{
    ...
    if (HI_TRUE == HDMI_TX_CHANNEL_reg_vid_bypass_selGet())
    {
        ...
        u32Reg = HDMIRegReadU32(0x11140980);
        u32Reg &= (~(1 << 3));
        u32Reg |= ((!bDwsmVert) << 3);
        HDMIRegWriteU32(0x11140980, u32Reg);
    }
    else
    {
...
HI_VOID HAL_HDMI_CtrlSrsSet(HDMI_DEVICE_ID_E enHdmi)
{
    ...
    while((HDMI_OSAL_GetTimeInMs() - u32StartTime) < 30)
    {
        ...
        if ((u32HTotalCur >= 300) && (osal_abs(((HI_S32)u32HTotalCur - u32HTotalPre)) < 2))
        {
    ...
    while(u32StartTime < 30)
    {
        ...
        u32HTotalCur = HDMI_HTOTAL_HW_htotal_hwGet();
        if ((abs(((HI_S32)u32HTotalCur - u32HTotalPre)) < 2))
        {
            break;
        }
