...
static HI_S32 CecMsgRx(CEC_MSG_S *pstMsg)
{
	...
	pstMsg->stRawData.u8Length = HDMI_CEC_RX_STATE_cec_reg_rx_cmd_byte_cntGet();
	...
	s32Ret = CecOperationCodeGet(pstMsg->stRawData.u8Data);
...
static HI_S32 CecMsgTx(CEC_MSG_S *pstMsg)
{
...
#endif
    {
        ...
        HDMI_CEC_TX_CTRL_cec_reg_tx_cmd_cntSet(pstMsg->stRawData.u8Length);
        CecOperationCodeSet(pstMsg->stRawData.u8Data);
        /* clear txDone Intr */
        HDMI_TX_AON_INTR_STATE_aon_intr_stat11Set(HI_TRUE);
        /* transimit lunch */
        CecManualCmdSet();
...
static HI_U8 CecMapPriorityPlayBack(CEC_LOGIC_ADDR_U un16MapAddr)
{
    HI_U8 u8LogicAddr = CEC_LOGICALADD_BROADCAST;

    if( !un16MapAddr.bits.bit01_playback_dev1 )
    {
        u8LogicAddr = CEC_LOGICALADD_PLAYDEV_1;
    }
    else if( !un16MapAddr.bits.bit01_playback_dev2 )
    {
        u8LogicAddr = CEC_LOGICALADD_PLAYDEV_2;
    }
    else if( !un16MapAddr.bits.bit01_playback_dev3 )
    {
        u8LogicAddr = CEC_LOGICALADD_PLAYDEV_3;
    }
    else if( !un16MapAddr.bits.bit01_tuner1 )
    {
        u8LogicAddr = CEC_LOGICALADD_TUNER_1;
    }
    else if( !un16MapAddr.bits.bit01_tuner2 )
    {
        u8LogicAddr = CEC_LOGICALADD_TUNER_2;
    }
    else if( !un16MapAddr.bits.bit01_tuner3 )
    {
        u8LogicAddr = CEC_LOGICALADD_TUNER_3;
    }
    else if( !un16MapAddr.bits.bit01_tuner4 )
    {
        u8LogicAddr = CEC_LOGICALADD_TUNER_4;
    }
    else
    {
        u8LogicAddr = CEC_LOGICALADD_BROADCAST;
    }
...
static HI_U8 CecMapPriorityTuner(CEC_LOGIC_ADDR_U un16MapAddr)
{
    HI_U8 u8LogicAddr = CEC_LOGICALADD_BROADCAST;
...
static HI_S32 CecPingMapAddrGet(HDMI_DEVICE_ID_E enHdmiId)
{
    ...
    pstCecInfo->stDrvInfo.u8LogicalAddr = CecMapPriorityPlayBack(un16MapAddr);
#else
    if (DRV_HDMI_CompatCecMapPlaydev(enHdmiId))
    {
        pstCecInfo->stDrvInfo.u8LogicalAddr = CecMapPriorityPlayBack(un16MapAddr);
    }
    else
    {
        pstCecInfo->stDrvInfo.u8LogicalAddr = CecMapPriorityTuner(un16MapAddr);
    }
#endif
...
HI_S32 CecBroadAddr(HDMI_DEVICE_ID_E enHdmiId)
{
    ...
    stMsg.stRawData.u8Data[0] = ((pstCecInfo->stDrvInfo.au8PhysicalAddr[0] << 4) & 0xf0) | (pstCecInfo->stDrvInfo.au8PhysicalAddr[1] & 0x0f);
    stMsg.stRawData.u8Data[1] = ((pstCecInfo->stDrvInfo.au8PhysicalAddr[2] << 4) & 0xf0) | (pstCecInfo->stDrvInfo.au8PhysicalAddr[3] & 0x0f);
    stMsg.stRawData.u8Data[2] = pstCecInfo->stDrvInfo.u8LogicalAddr;
    stMsg.stRawData.u8Length  = 3;
...
static HI_S32 CecRxPoolWrite(CEC_INFO_S *pstCecInfo,CEC_MSG_S *pstMsg)
{
    ...
    HDMI_WAKE_UP(&pstCecInfo->stRxWaitQueue);
...
HI_S32 HAL_HDMI_CecInit(HDMI_DEVICE_ID_E enHdmiId, HDMI_HAL_INIT_S *pstHalInit)
{
    ...
    CEC_NULL_CHK(pstCecInfo);
    CEC_NULL_CHK(pstHalInit);

    s32Ret |= HDMI_TX_S_tx_cec_reg_REGS_TYPE_Init(pstHalInit->pcBaseAddr);
    ...
    if (!pstCecInfo->bInit)
    {
        ...
        HDMI_WAIT_QUEUE_INIT(pstCecInfo->stRxWaitQueue);
...
HI_S32 HAL_HDMI_CecDeInit(HDMI_DEVICE_ID_E enHdmiId)
{
    ...
    HDMI_DESTORY_MUTEX(pstCecInfo->stTxMute);
    HDMI_DESTORY_MUTEX(pstCecInfo->stRxMute);
    HDMI_WAIT_QUEUE_DESTORY(pstCecInfo->stRxWaitQueue);
...
HI_S32 HAL_HDMI_CecMsgCallback(HI_VOID *pParam)
{
    CEC_INFO_S *pstCecInfo = (CEC_INFO_S *)pParam;
    ...
    s32Result = (   (HI_TRUE == pstCecInfo->bRxWakeUpFlag)
                 || (pstCecInfo->u32RxPoolRdAbleCnt > 0));
...
HI_S32 HAL_HDMI_CecMsgRx(HDMI_DEVICE_ID_E enHdmiId,CEC_MSG_S *pstMsg)
{
    ...
    {
        HDMI_WAIT_EVENT_INTERRUPT(pstCecInfo->stRxWaitQueue, HAL_HDMI_CecMsgCallback, (HI_VOID *)pstCecInfo);
    }
