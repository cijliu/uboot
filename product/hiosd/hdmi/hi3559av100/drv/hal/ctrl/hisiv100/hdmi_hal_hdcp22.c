...
HI_S32  HAL_HDMI_Hdcp22Init(HDMI_DEVICE_ID_E enHdmiId, HDMI_HAL_INIT_S *pstHalInit)
{
    ...
    {
        s32Ret = HDMI_TX_S_tx_hdcp_reg_REGS_TYPE_Init(pstHalInit->pcBaseAddr);
        HDCP22_CHK_FAILURE(s32Ret);

        s32Ret = HDMI_TX_S_mcu_cpu_reg_REGS_TYPE_Init(pstHalInit->pcBaseAddr);
        HDCP22_CHK_FAILURE(s32Ret);

        g_Hdcp22McuDramVirAddr = (volatile HI_U32 *)(pstHalInit->pcBaseAddr + HDMI_TX_BASE_ADDR_MCU_DRAM);
...
HI_VOID HAL_HDMI_Hdcp22DeInit(HDMI_DEVICE_ID_E enHdmiId)
{
    ...
    if (HI_NULL != g_Hdcp22McuDramVirAddr)
    {
        HDMI_OSAL_IOUNMAP((HI_VOID *)g_Hdcp22McuDramVirAddr);
    }
