...
typedef struct
{
    HI_U32              u32SscAmptd; // 1/1M ppm(Spread rate range)
    HI_U32              u32SscFreq;  // 1Hz(Spread the frequency)
} PHY_SSC_CFG_S;
...
{
    HI_U32              u32TmdsClk;     /* TMDS colck,in KHz */
    HI_U8                u8FrlDatRat;   /* FRL_DATA_RATA*/
...
#endif
    HI_U32              u32TraceLen;    /* PCB trace length, olny used on 59A */
}HDMI_PHY_TMDS_CFG_S;

typedef struct
{
    HI_BOOL             bInit;          /* 1:init 0:deinit */
