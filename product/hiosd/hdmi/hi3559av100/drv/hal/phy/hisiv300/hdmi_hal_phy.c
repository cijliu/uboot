...
/*x--lane,y--TXFFE*/
PHY_TXFFE_RANGE_S  s_stPhyHWTXFFEValue[4][4] =
{
...
{
    /* strobe manual mode set */
#if 0
	HDMI_STBOPT_req_lengthSet(0x0);
	HDMI_STBOPT_stb_delay2Set(0x0);
	HDMI_STBOPT_stb_delay1Set(0x0);
	HDMI_STBOPT_stb_delay0Set(0x0);
...
static HI_S32 CtrlClkSourceSet(HDMI_PHY_TMDS_CFG_S *pstHdmiCfg)
{
    ...
    {
#ifdef RX21_TMDS
        //only for RX2.1 to TMDS
        HDMI_TX_PWD_RST_CTRL_audpath_acr_clk_selSet(0x01);
        HDMI_TX_PWD_RST_CTRL_clk_tmds_source_selSet(0x00);
        HDMI_TX_PWD_RST_CTRL_clk_frl_source_selSet(0x01);
        REG_HDMI_CRG_mpll_refclk_cksel_Set(0x1);
#else
...
static HI_S32 PhyHWInitset(HDMI_PHY_TMDS_CFG_S *stHdmiSpecCfg)
{
    HI_U32 u32RefDiv = 0;

    PHY_NULL_CHK(stHdmiSpecCfg);

    HDMI_INFO("enModeCfg = %d\r\n", stHdmiSpecCfg->enModeCfg);
	if (HDMI_PHY_MODE_CFG_TMDS == stHdmiSpecCfg->enModeCfg)
    {
        // DDC QEC Settings
        ...
        if (stHdmiSpecCfg->u32PixelClk < 37500)
        {
            u32RefDiv = 0;
        }
        else if (37500 <= stHdmiSpecCfg->u32PixelClk && stHdmiSpecCfg->u32PixelClk < 75000)
        {
            u32RefDiv = 1;
        }
        else if (75000 <= stHdmiSpecCfg->u32PixelClk && stHdmiSpecCfg->u32PixelClk < 150000)
        {
            u32RefDiv = 2;
        }
        else if (150000 <= stHdmiSpecCfg->u32PixelClk && stHdmiSpecCfg->u32PixelClk < 300000)
        {
            u32RefDiv = 3;
        }
        else if (300000 <= stHdmiSpecCfg->u32PixelClk && stHdmiSpecCfg->u32PixelClk < 600000)
        {
            u32RefDiv = 4;
        }

        PhyHWWriteStb1Byte(APHY_CS_8, APHY_OFFSET_2, u32RefDiv);
...
{
    /* HS FIFO Disable (8260) */
	HDMI_HSSET_p_hssetSet(0x0);
    /* TX FIFO Disable for H20 (8210) */
	HDMI_TXFIFOSET0_p_pr_en_h20Set(0x0);
    /* Data up_sampler FIFO Disable for H20 (8210) */
	HDMI_TXFIFOSET0_p_enable_h20Set(0x0);
	HDMI_TXFIFOSET0_txfifoset0_unusedSet(0X0);
    /* STB_RESET reset (8010) */
	...
	HDMI_SWRESET_sw_reset_frl_clockSet(0);
    HDMI_SWRESET_swreset_unusedSet(0);
	HDMI_SWRESET_global_resetSet(1);

    /* SPPDHTG2 Software Set 8310 */
...
******************************************************/
static HI_VOID PHYSpaceCfg100M(HI_VOID)
{
    //#Data Drv Settings
	...
	PhyHWWriteStb1Byte(0x007, 0x05,	0x1C);
	...
	PhyHWWriteStb1Byte(0x008, 0x05,	0x21);
...
******************************************************/
static HI_VOID PHYSpaceCfg165M(HI_VOID)
{
    //#Data Drv Settings
	PhyHWWriteStb1Byte(0x007, 0x00,	0x87);
	PhyHWWriteStb1Byte(0x007, 0x01,	0x3E);
	PhyHWWriteStb1Byte(0x007, 0x02,	0x3E);
	PhyHWWriteStb1Byte(0x007, 0x03,	0xDF);
	PhyHWWriteStb1Byte(0x007, 0x05,	0x24);
	PhyHWWriteStb1Byte(0x007, 0x06,	0x00);
	...
	PhyHWWriteStb1Byte(0x008, 0x01,	0x3E);
	PhyHWWriteStb1Byte(0x008, 0x02,	0x3E);
	PhyHWWriteStb1Byte(0x008, 0x03,	0xDF);
...
******************************************************/
static HI_VOID PHYSpaceCfg165M_340M(HI_VOID)
{
    //#Data Drv Settings
	PhyHWWriteStb1Byte(0x007, 0x00,	0x87);
	PhyHWWriteStb1Byte(0x007, 0x01,	0x3E);
	PhyHWWriteStb1Byte(0x007, 0x02,	0x3E);
	PhyHWWriteStb1Byte(0x007, 0x03,	0xDF);
	PhyHWWriteStb1Byte(0x007, 0x05,	0x30);        // MAIN_ICAL<4:0> : Data Swing Control
...
******************************************************/
static HI_VOID PHYSpaceCfg340M_600M(HI_VOID)
{
    //Data Drv Settings
    PhyHWWriteStb1Byte(0x007, 0x0, 0x87);
    PhyHWWriteStb1Byte(0x007, 0x1, 0x3E);
    PhyHWWriteStb1Byte(0x007, 0x2, 0x3E);
    PhyHWWriteStb1Byte(0x007, 0x3, 0xDF);
    PhyHWWriteStb1Byte(0x007, 0x5, 0x3D);
    ...
    PhyHWWriteStb1Byte(0x008, 0x3, 0xDF);
    PhyHWWriteStb1Byte(0x008, 0x5, 0x23);
    PhyHWWriteStb1Byte(0x008, 0x6, 0x00);
    PhyHWWriteStb1Byte(0x008, 0x8, 0x4B);
...
static HI_VOID PHYSpaceCfg3G(HI_VOID)
{
	...
	PhyHWWriteStb1Byte(0x008, 0x1, 0xBC);
	PhyHWWriteStb1Byte(0x00F, 0x2, 0x05);
	PhyHWWriteStb1Byte(0x00F, 0x3, 0x81);
	PhyHWWriteStb1Byte(0x00F, 0x5, 0x00);
	PhyHWWriteStb1Byte(0x00F, 0x6, 0x00);
	PhyHWWriteStb1Byte(0x00F, 0xB, 0x50);
	PhyHWWriteStb1Byte(0x007, 0x8, 0x64);
	PhyHWWriteStb1Byte(0x008, 0x8, 0x44);
	PhyHWWriteStb1Byte(0x00F, 0x9, 0x8c);
	PhyHWWriteStb1Byte(0x100, 0x3, 0x1F);
...
{
	PhyHWWriteStb1Byte(0x007, 0x1, 0x5E);
	PhyHWWriteStb1Byte(0x008, 0x1, 0x9E);
	PhyHWWriteStb1Byte(0x00F, 0x2, 0x05);
	PhyHWWriteStb1Byte(0x00F, 0x3, 0x81);
	PhyHWWriteStb1Byte(0x00F, 0x5, 0x0F);
	PhyHWWriteStb1Byte(0x00F, 0x6, 0x00);
	PhyHWWriteStb1Byte(0x00F, 0xB, 0x40);
	PhyHWWriteStb1Byte(0x007, 0x8, 0x70);
	PhyHWWriteStb1Byte(0x008, 0x8, 0x70);
	PhyHWWriteStb1Byte(0x00F, 0x9, 0x8c);
	PhyHWWriteStb1Byte(0x100, 0x3, 0x0A);
...
{
	PhyHWWriteStb1Byte(0x007, 0x0, 0x87); //add from goden TJ
	...
	PhyHWWriteStb1Byte(0x007, 0x8, 0x70); //add from goden TJ
	PhyHWWriteStb1Byte(0x007, 0x9, 0x81); //add from goden TJ
	PhyHWWriteStb1Byte(0x007, 0xB, 0x10); //add from goden TJ

    //CLK Drv Settings
	PhyHWWriteStb1Byte(0x008, 0x0, 0x87); //add from goden TJ
	...
	PhyHWWriteStb1Byte(0x008, 0x8, 0x70); //add from goden TJ
	PhyHWWriteStb1Byte(0x008, 0x9, 0x81); //add from goden TJ
	PhyHWWriteStb1Byte(0x008, 0xB, 0x10); //add from goden TJ
//	PhyHWWriteStb1Byte(0x008, 0xD, 0xFF);// aphy clk ---xiaoshu fenpin youhua(0xFF--200ppm, cdr stable /1600ppm before, cdr fail)
...
{
    //Data Drv Settings
	PhyHWWriteStb1Byte(0x007, 0x0, 0xE7); //0x87->0xe7,0xe7 from TJ
	...
	PhyHWWriteStb1Byte(0x007, 0x7, 0x12); // add from TJ
	PhyHWWriteStb1Byte(0x007, 0x8, 0x70);
	PhyHWWriteStb1Byte(0x007, 0x9, 0x81);
	PhyHWWriteStb1Byte(0x007, 0xB, 0x10); //0x50

    //CLK Drv Settings
	PhyHWWriteStb1Byte(0x008, 0x0, 0xE7); //0x87
	PhyHWWriteStb1Byte(0x008, 0x1, 0xFF); //0x3E
	PhyHWWriteStb1Byte(0x008, 0x2, 0xFF); //0x3E
	PhyHWWriteStb1Byte(0x008, 0x3, 0xFF);
	PhyHWWriteStb1Byte(0x008, 0x4, 0xEF); // add from TJ
	PhyHWWriteStb1Byte(0x008, 0x5, 0x3F); //0x23
	PhyHWWriteStb1Byte(0x008, 0x6, 0x32); //0x00'
	PhyHWWriteStb1Byte(0x008, 0x7, 0x12); // add from TJ
	PhyHWWriteStb1Byte(0x008, 0x8, 0x70); //0x4B 300ohm termination_b
	PhyHWWriteStb1Byte(0x008, 0x9, 0x81);
...
{
    //Data Drv Settings
	PhyHWWriteStb1Byte(0x007, 0x0, 0xE7); //add from goden TJ
	...
	PhyHWWriteStb1Byte(0x007, 0x5, 0x3F); //add from goden TJ
	PhyHWWriteStb1Byte(0x007, 0x6, 0x32); //add from goden TJ
	PhyHWWriteStb1Byte(0x007, 0x7, 0x12); //add from goden TJ
	PhyHWWriteStb1Byte(0x007, 0x8, 0x70); //add from goden TJ
	PhyHWWriteStb1Byte(0x007, 0x9, 0x81); //add from goden TJ
	PhyHWWriteStb1Byte(0x007, 0xB, 0x10); //add from goden TJ

    //CLK Drv Settings
	PhyHWWriteStb1Byte(0x008, 0x0, 0xE7); //add from goden TJ
	PhyHWWriteStb1Byte(0x008, 0x1, 0xFF); //add from goden TJ
	PhyHWWriteStb1Byte(0x008, 0x2, 0xFF); //add from goden TJ
	PhyHWWriteStb1Byte(0x008, 0x3, 0xFF); //add from goden TJ
	PhyHWWriteStb1Byte(0x008, 0x4, 0xEF); //add from goden TJ
	PhyHWWriteStb1Byte(0x008, 0x5, 0x3F); //add from goden TJ
	PhyHWWriteStb1Byte(0x008, 0x6, 0x32); //add from goden TJ
	PhyHWWriteStb1Byte(0x008, 0x7, 0x12); //add from goden TJ
	PhyHWWriteStb1Byte(0x008, 0x8, 0x70); //add from goden TJ
	PhyHWWriteStb1Byte(0x008, 0x9, 0x81); //add from goden TJ
	PhyHWWriteStb1Byte(0x008, 0xB, 0x10); //add from goden TJ
//   PhyHWWriteStb1Byte(0x008, 0xD, 0xFF);// aphy clk ---xiaoshu fenpin youhua(0xFF--200ppm, cdr stable /1600ppm before, cdr fail)
...
static HI_S32 PhyHWFconCfgParaGet(PHY_FCON_PARA_S* pstFconMode, HI_U32 u32TmdsClk,
                                                HDMI_PHY_TMDS_CFG_S *pstHdmiTmdsCfg)
{
	...
	if(pstHdmiTmdsCfg->enModeCfg == HDMI_PHY_MODE_CFG_TMDS)
    {
        /***************************
             PLL_VCO_FRE(MHz)
...
#ifdef HDMI_FRL_SUPPORT
	else if(pstHdmiTmdsCfg->enModeCfg == HDMI_PHY_MODE_CFG_FRL)
    {
    	switch(pstHdmiTmdsCfg->enRate)
        {
        	case FRL_WORK_MODE_3L3G:
        	case FRL_WORK_MODE_3L6G:
        	case FRL_WORK_MODE_4L6G:
	...
	HDMI_INFO("ModeCfg: %d, I_REF_CNT: %x\r\n", pstHdmiTmdsCfg->enModeCfg, pstFconMode->u32IRefCnt);

	return HI_SUCCESS;
}

static HI_VOID PhyHWSpecTmdsSet(HDMI_PHY_TMDS_CFG_S *pstHdmiSpecCfg)
{
    PHY_NULL_CHK_NO_RET(pstHdmiSpecCfg);

    HDMI_INFO("pstHdmiSpecCfg->u32TmdsClk = %d \r\n", pstHdmiSpecCfg->u32TmdsClk);
    if(pstHdmiSpecCfg->u32TmdsClk < 100000)
    {
        PHYSpaceCfg100M();
    }
	else if(pstHdmiSpecCfg->u32TmdsClk >= 100000 && pstHdmiSpecCfg->u32TmdsClk < 165000)
    {
        PHYSpaceCfg165M();
    }
	else if(pstHdmiSpecCfg->u32TmdsClk >= 165000 && pstHdmiSpecCfg->u32TmdsClk < 340000)
    {
        PHYSpaceCfg165M_340M();
    }
	else if(pstHdmiSpecCfg->u32TmdsClk >= 340000 && pstHdmiSpecCfg->u32TmdsClk < 600000)
    {
        PHYSpaceCfg340M_600M();
    }
...
static HI_VOID PhyHWClockTmdsSet(PHY_CLK_SET_S *pstPhyClkSet)
{
    ...
    HDMI_INFO("pstPhyClkSet->stTmdsCfg.u32TmdsClk: %d\n", pstPhyClkSet->stTmdsCfg.u32TmdsClk);
...
static HI_VOID PhyHWClockFrlSet(FRL_WORK_MODE_E  enRate)
{
    switch (enRate)
    {
        case FRL_WORK_MODE_3L3G:
            ...
            //N/X parameter
            HDMI_FDIVSTAT1_i_fdiv_inSet((s_stPhyHWPllRefClk[0].stPhyPllRefClkRange.u8Fracn << 24) \
        ...
        case FRL_WORK_MODE_4L6G:
            ...
            //N/X parameter
            HDMI_FDIVSTAT1_i_fdiv_inSet((s_stPhyHWPllRefClk[1].stPhyPllRefClkRange.u8Fracn << 24) \
                                        | (s_stPhyHWPllRefClk[0].stPhyPllRefClkRange.u32Fracx & 0x00ffffff));
            //FDIV init
            HDMI_FDIVSET0_initSet(0x1);
            HDMI_UDELAY(1);
       ...
       case FRL_WORK_MODE_4L8G:
            ...
            //N/X parameter
            HDMI_FDIVSTAT1_i_fdiv_inSet((s_stPhyHWPllRefClk[2].stPhyPllRefClkRange.u8Fracn << 24) \
                                         | (s_stPhyHWPllRefClk[2].stPhyPllRefClkRange.u32Fracx & 0x00ffffff));
            //FDIV init
            HDMI_FDIVSET0_initSet(0x1);
            HDMI_UDELAY(1);
            HDMI_FDIVSET0_initSet(0x0);
        ...
        case FRL_WORK_MODE_4L10G:
            ...
            //N/X parameter
            HDMI_FDIVSTAT1_i_fdiv_inSet((s_stPhyHWPllRefClk[3].stPhyPllRefClkRange.u8Fracn << 24) \
        ...
        case FRL_WORK_MODE_4L12G:
            ...
            //N/X parameter
            HDMI_FDIVSTAT1_i_fdiv_inSet((s_stPhyHWPllRefClk[4].stPhyPllRefClkRange.u8Fracn << 24) \
...
static HI_S32 PhyHWFCON(PHY_FCON_PARA_S stFconMode)
{
	...
	if(stFconMode.enFconMode == PHY_FCON_MODE_AUTO)
    {
    	...
    	HDMI_FCCNTR0_i_ref_cnt_lenSet(stFconMode.u32IRefCnt_len);
    	...
    	HDMI_FCCNTR1_i_ref_cntSet(stFconMode.u32IRefCnt & (0x0000ffff));
...
static HI_S32 PhyHWPostSetup(HDMI_PHY_TMDS_CFG_S *pstHdmiTmdsCfg)
{
    PHY_NULL_CHK(pstHdmiTmdsCfg);
...
#ifdef HDMI_FRL_SUPPORT
	if (pstHdmiTmdsCfg->enModeCfg == HDMI_PHY_MODE_CFG_FRL)
    {
        /* HDMI2.1 input selection */
    	HDMI_TXDATAOUTSEL_p_ch_out_selSet(0x1);

        if((pstHdmiTmdsCfg->enRate == FRL_WORK_MODE_3L3G)
           ||(pstHdmiTmdsCfg->enRate == FRL_WORK_MODE_3L6G))
        {
        ...
        else if((pstHdmiTmdsCfg->enRate == FRL_WORK_MODE_4L6G)
            || (pstHdmiTmdsCfg->enRate == FRL_WORK_MODE_4L8G)
            || (pstHdmiTmdsCfg->enRate == FRL_WORK_MODE_4L10G)
            || (pstHdmiTmdsCfg->enRate == FRL_WORK_MODE_4L12G))
        {
            /*PHY data serializer reset/set*/
            PhyHWWriteStb(APHY_CS_012|APHY_CS_3, APHY_OFFSET_0, 1, 1, 0x0);
...
{
	HI_U32 u32vcoFreq = 0;
	HI_U32 u32PixelClk = pstTmdsCfg->u32PixelClk;
    ...
    if((pstFcgPara->u32p_fig_div <= 0x04) && (pstFcgPara->u32p_fig_div > 0x00))
    {
    ...
    else if((pstFcgPara->u32p_fig_div <= 0x18) && (pstFcgPara->u32p_fig_div > 0x0C))
       {
        ...
        {
        	pstFcgPara->u8p_fig_divm = (pstFcgPara->u32p_fig_div&(0xF))>>0x1;
        }
    }
    else if((pstFcgPara->u32p_fig_div > 0x18) && (pstFcgPara->u32p_fig_div <= 0x30))
    {
        ...
        {
        	pstFcgPara->u8p_fig_divm = (pstFcgPara->u32p_fig_div&(0x0F))>>0x2;
        }
    }
    else if((pstFcgPara->u32p_fig_div > 0x30) && (pstFcgPara->u32p_fig_div <= 0x60))
    {
        ...
        {
        	pstFcgPara->u8p_fig_divm = (pstFcgPara->u32p_fig_div&(0x0F))>>0x3;
        }
    }
    else
    {
	...
	HDMI_INFO("FcgPara->u8p_fig_divm= %x, \r\n", pstFcgPara->u8p_fig_divm);
	HDMI_INFO("FcgPara->u32p_fig_div= %x, \r\n", pstFcgPara->u32p_fig_div);
	HDMI_INFO("FcgPara->u8p_fig_divp= %x, \r\n", pstFcgPara->u8p_fig_divp);
	HDMI_INFO("FcgPara->u32ptmdscntval= %x, \r\n", pstFcgPara->u32ptmdscntval);
                                    ...
                                    PHY_FCG_PARA_S *pstFcgPara)
{
	...
	HDMI_INFO("pstFcgPara->u8Pki = %x ,u32Plockth = %x \r\n", pstFcgPara->u8Pki, u32Plockth);
...
static HI_S32 PhyHWSpecSet(HDMI_PHY_TMDS_CFG_S *pstHdmiSpecCfg)
{
    PHY_NULL_CHK(pstHdmiSpecCfg);

    switch(pstHdmiSpecCfg->enModeCfg)
    {
        case HDMI_PHY_MODE_CFG_TMDS:
            PhyHWSpecTmdsSet(pstHdmiSpecCfg);
            break;
#ifdef HDMI_FRL_SUPPORT
        case HDMI_PHY_MODE_CFG_FRL:
            PhyHWSpecFrlSet(pstHdmiSpecCfg->enRate);
            break;
#endif
        default:
            break;
...
HI_S32 HAL_HDMI_PhyTmdsSet(HDMI_PHY_TMDS_CFG_S *pstHdmiTmdsCfg)
{
	...
	if( (pstHdmiTmdsCfg->enModeCfg == HDMI_PHY_MODE_CFG_TMDS) ||
        (pstHdmiTmdsCfg->enModeCfg == HDMI_PHY_MODE_CFG_FRL))
    {
#ifdef HDMI_FRL_SUPPORT
        // Clock source select
        if(HI_SUCCESS != CtrlClkSourceSet(pstHdmiTmdsCfg))
        {
        ...
        HDMI_INFO("TMDS mode: %d\n", pstHdmiTmdsCfg->enModeCfg);
        HDMI_INFO("u32PixelClk: %d\n", pstHdmiTmdsCfg->u32PixelClk);
        HDMI_INFO("u32TmdsClk: %d\n", pstHdmiTmdsCfg->u32TmdsClk);
        HDMI_INFO("enDeepColor: %d\n", pstHdmiTmdsCfg->enDeepColor);
#ifdef HDMI_FRL_SUPPORT
        HDMI_INFO("enColorSpace: %d\n", pstHdmiTmdsCfg->enColorSpace);
#endif
        HDMI_INFO("tarce len: %d, u32PixelClk: %d\n", pstHdmiTmdsCfg->u32TraceLen, pstHdmiTmdsCfg->u32PixelClk);
...
//            return HI_FAILURE;
        ...
        PhyHWInitset(pstHdmiTmdsCfg);

        stPhyClkSet.stTmdsCfg.enModeCfg = pstHdmiTmdsCfg->enModeCfg;
...
#ifdef HDMI_FRL_SUPPORT
        stPhyClkSet.stTmdsCfg.enRate = pstHdmiTmdsCfg->enRate;
#endif
        ...
        /* calculate FCON */
        PhyHWFconCfgParaGet(&stFconMode, stPhyClkSet.stTmdsCfg.u32TmdsClk, pstHdmiTmdsCfg);
        ...
        /* inital:index */
     	PhyHWSpecSet(pstHdmiTmdsCfg);

        /*data path enable*/
    	PhyHWPostSetup(pstHdmiTmdsCfg);
...
#ifdef HDMI_FRL_SUPPORT
    else if(pstHdmiTmdsCfg->enModeCfg == HDMI_PHY_MODE_CFG_TXFFE)
    {
        /* TxFFE */
        PhyTxFFESet(pstHdmiTmdsCfg->aenTxFFE);
    }
#endif
