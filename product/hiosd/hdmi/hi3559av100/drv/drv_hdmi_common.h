...
{
    HI_U32              u32TraceLength; /**< The length of HDMI traces on circuit boards. Units: inch*10 */
}HDMI_MOD_PARAM_S;

#define CEAVideoCodesMax        155
...
#define HDMI_HAL_CALL_RET(pfunc,param...) ({\
    HI_S32 s32Ret = HI_FAILURE;\
    if (pstHdmiDev && pstHdmiDev->pstHdmiHal && pstHdmiDev->pstHdmiHal->pfunc)\
        s32Ret = pstHdmiDev->pstHdmiHal->pfunc(param);\
    else\
...
#define HDMI_HAL_CALL(pfunc,param...) ({\
    if (pstHdmiDev && pstHdmiDev->pstHdmiHal && pstHdmiDev->pstHdmiHal->pfunc)\
        pstHdmiDev->pstHdmiHal->pfunc(param);\
    else\
        HDMI_WARN("%s null pointer! \n",#pfunc);\
})
