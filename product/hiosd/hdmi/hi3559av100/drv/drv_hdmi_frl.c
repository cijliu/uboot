...
static HI_S32 FrlTxFFESet(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    FRL_CHECK_POINTER(pstHdmiDev->pstHdmiHal);
...
HI_U8 FrlGetSinkVersion(HDMI_DEVICE_ID_E enHdmiID)
{
    ...
    if (pstHdmiDev == HI_NULL || pstHdmiDev->pstHdmiHal == HI_NULL)
    {
        return 0;
    }
    ...
    pstHdmiHal->HAL_HDMI_ScdcProcess(pstHdmiDev->pstHdmiHal,
                                     SCDC_CMD_GET_SINK_VER, &u8SinkVer);
...
static HI_BOOL FrlGetFltUpdate(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    HDMI_HAL_S* pstHal = HI_NULL;

    FRL_CHECK_POINTER_RET(pstHdmiDev);
    FRL_CHECK_POINTER_RET(pstHdmiDev->pstHdmiHal);

    pstHal = pstHdmiDev->pstHdmiHal;

    pstHal->HAL_HDMI_ScdcProcess(pstHdmiDev->pstHdmiHal,
                                  SCDC_CMD_GET_FLT_UPDATE, &bFltUpdate);
...
static HI_VOID FrlSetFltUpdate(HDMI_DEVICE_S* pstHdmiDev, HI_BOOL bFltUpdate)
{
    ...
    FRL_CHECK_POINTER_NO_RET(pstHdmiDev->pstHdmiHal);

    pstHal = pstHdmiDev->pstHdmiHal;

    pstHal->HAL_HDMI_ScdcProcess(pstHdmiDev->pstHdmiHal,
                                  SCDC_CMD_SET_FLT_UPDATE, &bFltUpdate);
...
static HI_BOOL FrlGetFltStart(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    pstHal->HAL_HDMI_ScdcProcess(pstHdmiDev->pstHdmiHal,
                                  SCDC_CMD_GET_FLT_START, &bFltStart);
...
static HI_VOID FrlSetFltStart(HDMI_DEVICE_S* pstHdmiDev, HI_BOOL bFltStart)
{
    ...
    pstHal->HAL_HDMI_ScdcProcess(pstHdmiDev->pstHdmiHal,
                                  SCDC_CMD_SET_FLT_START, &bFltStart);
...
static HI_VOID FrlGetTestConfig(HDMI_DEVICE_S* pstHdmiDev, SCDC_TEST_CONFIG_S* pstTestCfg)
{
    ...
    pstHal->HAL_HDMI_ScdcProcess(pstHdmiDev->pstHdmiHal,
                                  SCDC_CMD_GET_TEST_CONFIG, pstTestCfg);
...
static HI_BOOL FrlGetFltReady(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    pstHal->HAL_HDMI_ScdcProcess(pstHdmiDev->pstHdmiHal,
                                  SCDC_CMD_GET_FLT_READY, &bFltReady);
...
static HI_BOOL FrlCheckFormat(HDMI_FRL_INFO_S* pstFrlInfo, HDMI_ATTR_S* pstAttr)
{
    ...
    for (i = u8MaxRate; i > 0; i--)
    {
        ...
        if (DRV_HDMI_DFMFormatCanbeSupport(&stDFMIn) == HI_TRUE)
        {
            ...
            /* DFM parameters only calculating when use software DFM. */
            if (HI_TRUE == pstFrlInfo->stDFM.bSWDFM)
            {
                DRV_HDMI_DFMGetData(&stDFMIn, &pstDFM->stDFMOut[i]);
            }
...
static HI_VOID FrlConfigAndStartTrain(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    stTrainConfig.bSWDFM = pstFrlInfo->stDFM.bSWDFM;

    if (HI_TRUE == stTrainConfig.bSWDFM)
    {
        HDMI_MEMCPY(&stTrainConfig.stDFMOut, &pstFrlInfo->stDFM.stDFMOut[u8FrlRate-1],
                                                                sizeof(DFM_OUT_S));
    }
    ...
    pstHal->HAL_HDMI_FrlTrainConfig(pstHdmiDev->pstHdmiHal, &stTrainConfig);
    pstHal->HAL_HDMI_FrlEnable(pstHdmiDev->pstHdmiHal, HI_FALSE);
    pstHal->HAL_HDMI_FrlTrainStart(pstHdmiDev->pstHdmiHal);
                                 ...
                                 HDMI_FRL_TRAIN_STATUS_S* pstTrainStatus)
{
    ...
    pstHdmiDev->pstHdmiHal->HAL_HDMI_FrlGetTrainStatus(pstHdmiDev->pstHdmiHal, pstTrainStatus);
...
static HI_VOID FrlEnableOutput(HDMI_DEVICE_S* pstHdmiDev , HI_BOOL bEnable)
{
    ...
    pstHdmiDev->pstHdmiHal->HAL_HDMI_FrlEnable(pstHdmiDev->pstHdmiHal, bEnable);
    return;
}
...
HI_VOID DRV_HDMI_FrlMachInit(HDMI_DEVICE_ID_E enHdmiID)
{
    ...
    /* Use FIFO DFM default. */
    pstFrlInfo->stDFM.bSWDFM = HI_FALSE;
...
HI_S32 DRV_HDMI_FrlModeChange(HDMI_DEVICE_ID_E enHdmiID, HDMI_FRL_DEBUG_S *pstDebug)
{
    ...
    {
        pstFrlInfo->stDFM.bSWDFM = pstDebug->bSWDFM;
        ...
        if (HI_TRUE == stTrainConfig.bSWDFM)
        {
            HDMI_MEMCPY(&stTrainConfig.stDFMOut,
                        &pstFrlInfo->stDFM.stDFMOut[pstDebug->enRate - 1], sizeof(DFM_OUT_S));
        }
        ...
        pstHal->HAL_HDMI_FrlTrainConfig(pstHdmiDev->pstHdmiHal, &stTrainConfig);
        /* phy config and oe enable */
        FrlPhySet(pstHdmiDev);

        /* LM config & Channel config */
        pstHal->HAL_HDMI_Debug(pstHdmiDev->pstHdmiHal, HDMI_DEBUG_CMD_FRL, &stTrainConfig.u8FrlRate);

        /* worken */
        pstHal->HAL_HDMI_FrlEnable(pstHdmiDev->pstHdmiHal, HI_TRUE);
    }
    else
    {
        /* TMDS mode */
