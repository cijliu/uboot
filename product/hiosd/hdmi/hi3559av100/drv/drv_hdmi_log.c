...
do{ \
    if (HI_NULL == pLogCtx){\
        HDMI_PRINTK("func: %s line: %d pLogCtx is NULL!!!\n", __FUNCTION__, __LINE__);\
        return;\
    } \
}while(0)
...
do{ \
    if (HI_SUCCESS != ret){\
        HDMI_PRINTK("call %s failed ret: %d!!!\n", pfunc, ret);\
    } \
}while(0)
...
HI_S32 LogCreate(LOG_CTX_S *pLogCtx)
{
    ...
    pstMem->pVirAddr = HDMI_VMALLOC(pstMem->u32MemSize);
    if (HI_NULL == pstMem->pVirAddr)
    {
        HDMI_PRINTK("alloc memory for log failed!!\n");
        return HI_FAILURE;
    }
    HDMI_MEMSET(pstMem->pVirAddr, 0x0, pstMem->u32MemSize);
...
HI_VOID LogInit(HI_VOID)
{
    ...
    {
        HDMI_PRINTK("g_pstLogCtx is inited!!!\n");
        return;
    }

    g_pstLogCtx = HDMI_VMALLOC(sizeof(LOG_CTX_S));
    LOG_CHECK_CTX_NULL(g_pstLogCtx);

    HDMI_MEMSET(g_pstLogCtx, 0x0, sizeof(LOG_CTX_S));
    HDMI_SPIN_LOCK_INIT(g_pstLogCtx->stSpinLock);
...
HI_VOID LogDeInit(HI_VOID)
{
    ...
    if (HI_NULL == LogGetCtx())
    {
        HDMI_PRINTK("g_pstLogCtx have not be inited!!!\n");
        return;
    }

    s32Ret = LogDestroy(g_pstLogCtx);
    LOG_CHECK_RET("LogDestroy", s32Ret);
    HDMI_SPIN_DESTORY(g_pstLogCtx->stSpinLock);

    g_pstLogCtx->bInited = HI_FALSE;
    HDMI_VFREE(g_pstLogCtx);
    g_pstLogCtx = HI_NULL;
...
HI_S32 LogRecordToMem(LOG_LEVEL_E eLevel, LOG_TYPE_FUNC_E eFuncType, HI_U8 *pParam)
{
    ...
    if (HI_NULL == pParam)
    {
        HDMI_PRINTK("pParam = null is invalid!!!\n");
        return HI_FAILURE;
    }
    ...
    {
        HDMI_PRINTK("memory of log buffer is output now!!!\n");
    ...
    {
        HDMI_PRINTK("pVirAddr = %p, pPos = %p is invalid!!!\n", pstMem->pVirAddr, pstMem->pPos);
        return HI_FAILURE;
    }

    /* len do not include '\0' */
    len = HDMI_STRLEN(pParam);
    if (LOG_MEM_BUF_SIZE < len)
    {
        HDMI_PRINTK("len = %d of pParam is invalid!!!\n", len);
        return HI_FAILURE;
    }
    ...
    HDMI_SPIN_LOCK(pLogCtx->stSpinLock);
    ...
    HDMI_MEMCPY(pstMem->pPos, temp, left);
    if(right)
    {
        HDMI_MEMCPY(pstMem->pVirAddr, temp + left, right);
    ...
    {
        pstMem->pPos += left;
        if(pstMem->pPos == (pstMem->pVirAddr + pstMem->u32MemSize))
        {
            pstMem->pPos = pstMem->pVirAddr;
        }
    }
    *pstMem->pPos = '\0';
    ...
    HDMI_SPIN_UNLOCK(pLogCtx->stSpinLock);
...
static HI_S32 LogMode(LOG_CTX_S *pLogCtx, HI_U8 *buf_out)
{
    ...
    switch (pLogCtx->eMode)
    {
        ...
        case LOG_PRINT_TO_ALL:
            HDMI_PRINTK("%s", buf_out);
            if(HI_NULL != pLogCtx->pFile)
            {
                s32Ret = HDMI_OSAL_FileWrite(pLogCtx->pFile, buf_out, HDMI_STRLEN(buf_out));
                if (s32Ret <= 0)
                {
                    HDMI_PRINTK("HDMI_OSAL_FileWrite ret=%d!\n", s32Ret);
                }
            }
            break;
...
HI_S32 LogPrintOutOfMem(HI_VOID)
{
    ...
    while(left >= 0)
    {
        ...
        else
        {
            ...
            if(!pos)
            {
                HDMI_PRINTK("can not Find '>##' in LogMem!!!\n");
                HDMI_SPIN_LOCK(pLogCtx->stSpinLock);
                pLogCtx->bReadFlag = HI_FALSE;
                HDMI_SPIN_UNLOCK(pLogCtx->stSpinLock);
                return HI_FAILURE;
            ...
            {
                HDMI_PRINTK("wrong pos value %p, start %p\n", pos, pstMem->pVirAddr);
                return HI_FAILURE;
            }
            pmem = pos;
        }

        u32FuncNum = HDMI_SMSTRTOUL(u8FuncNumBuf, NULL, 10);
        u8DbgLevel = u8DbgLevel - '0';
        u32StrLen  = HDMI_SMSTRTOUL(u8StrLenBuf, NULL, 10);
...
HI_S32 LogSetLevel(LOG_LEVEL_E eLevel)
{
    ...
    {
        HDMI_PRINTK("eLevel: %d is invalid!!!\n", eLevel);
        return HI_FAILURE;
    }
...
HI_S32 LogSetMaskFunc(LOG_TYPE_FUNC_E eFuncType)
{
    ...
    {
        HDMI_PRINTK("eFunc: %d is invalid!!!\n", eFuncType);
        return HI_FAILURE;
    }
...
HI_VOID LogCat(HI_VOID)
{
    ...
    {
        HDMI_OSAL_FileWrite(pLogCtx->pFile, pLogStart, HDMI_STRLEN(pLogStart));
    }

    s32Ret = LogPrintOutOfMem();
    if (HI_SUCCESS != s32Ret)
    {
        HDMI_PRINTK("call func:LogPrintOutOfMem failed!!\n");
    }

    if ((HI_NULL != pLogCtx->pFile) &&
        ((LOG_PRINT_TO_FILE == pLogCtx->eMode) || (LOG_PRINT_TO_ALL == pLogCtx->eMode)))
    {
        HDMI_OSAL_FileWrite(pLogCtx->pFile, pLogEnd, HDMI_STRLEN(pLogEnd));
    }
...
HI_S32 LogSetMode(LOG_PRINT_MODE_E eMode, HI_CHAR *pFilePath)
{
    ...
    switch (eMode)
    {
        ...
        case LOG_PRINT_TO_ALL:
            ...
            {
                if(!(HDMI_STRLEN(pFilePath) < LOG_FILEE_NAME_SIZE))
                {
            ...
            {
                HDMI_PRINTK("Please set a right file path and name!!!\n");
                return HI_FAILURE;
            }
            ...
            if (HI_NULL == pLogCtx->pFile)
            {
                ...
                if (HI_NULL == pLogCtx->pFile)
                {
                    HDMI_PRINTK("can not open the log file: %s ...\n", LogFile);
                    return HI_FAILURE;
                }
            }
            break;
        default:
            HDMI_PRINTK("eMode: %d is invalid!!!, please set right mode...\n", eMode);
            return HI_FAILURE;
    }
...
HI_S32 DRV_HDMI_LogCat(LOG_PRINT_MODE_E eMode, HI_CHAR *pFilePath)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HDMI_PRINTK("Call func LogSetMode failed s32Ret: %d!!!\n", s32Ret);
        return HI_FAILURE;
    }
    LogCat();
...
HI_S32 DRV_HDMI_LogSetLevel(LOG_LEVEL_E eLevel)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HDMI_PRINTK("Call func LogSetLevel failed s32Ret: %d!!!\n", s32Ret);
        return HI_FAILURE;
    }
...
HI_S32 DRV_HDMI_LogSetMask(LOG_TYPE_FUNC_E eFuncType)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HDMI_PRINTK("Call func LogSetMaskFunc failed s32Ret: %d!!!\n", s32Ret);
        return HI_FAILURE;
    }
