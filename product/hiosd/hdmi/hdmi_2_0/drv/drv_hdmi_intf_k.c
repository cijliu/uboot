...
HI_S32 HI_DRV_HDMI_Suspend(PM_BASEDEV_S *pdev, pm_message_t state)
{
    ...
    pstHdmiDev->stDebug.stIntfStatus.u32SuspendInTime = HDMI_OSAL_GetTimeInMs();
    ...
    pstHdmiDev->stDebug.stIntfStatus.u32SuspendOutTime = HDMI_OSAL_GetTimeInMs();
...
HI_S32 HI_DRV_HDMI_Resume(PM_BASEDEV_S *pdev)
{
    ...
    pstHdmiDev = GetHdmiDevice(enHdmi);
    pstHdmiDev->stDebug.stIntfStatus.u32ResumeInTime = HDMI_OSAL_GetTimeInMs();
    ...
    while (enHdmi < HDMI_DEVICE_ID_BUTT)
    {
        ...
        HDMI_INFO("CMD_HDMI_OPEN ok! \n");

        HDMI_MEMSET(&stHDMIAttr, 0, sizeof(stHDMIAttr));
    ...
    pstHdmiDev->stDebug.stIntfStatus.u32ResumeOutTime = HDMI_OSAL_GetTimeInMs();
...
HI_S32 HI_DRV_HDMI_SoftResume(HDMI_VIDEO_ATTR_S *pstVideoAttr)
{
    ...
    while (enHdmi < HDMI_DEVICE_ID_BUTT)
    {
        ...
        HDMI_INFO("CMD_HDMI_SET_VO_ATTR ok! \n");
...
HI_S32 HI_DRV_HDMI_KernelEventCallback(HI_VOID* data, HDMI_EVENT_E enEvent)
{
    ...
    enHdmiID = *(HDMI_DEVICE_ID_E*)data;
...
HI_S32 HI_DRV_HDMI_GetSinkCapability(HI_DRV_HDMI_ID_E enHdmi, HI_DRV_HDMI_SINK_CAPABILITY_S *pstSinkCap)
{
	...
	pstDrvCap = HDMI_VMALLOC(sizeof(DRV_HDMI_SINK_CAPABILITY_S));
	if (pstDrvCap == HI_NULL)
	{
		return HI_FAILURE;
	}

    HDMI_MEMSET(pstDrvCap, 0, sizeof(DRV_HDMI_SINK_CAPABILITY_S));
...
HI_S32 HI_DRV_HDMI_SetAOAttr(HI_UNF_HDMI_ID_E enHdmi, HDMI_AUDIO_ATTR_S *pstAudioAttr)
{
    ...
    HDMI_MEMSET(&stDrvAoAttr, 0, sizeof(DRV_HDMI_AO_ATTR_S));
    stDrvAoAttr.enHdmiID = enHdmi;
...
HI_S32 HI_DRV_HDMI_PreFormat(HI_UNF_HDMI_ID_E enHdmi)
{
    ...
    pstHdmiDev = GetHdmiDevice(enHdmi);
    pstHdmiDev->stDebug.stIntfStatus.u32PreformatInTime = HDMI_OSAL_GetTimeInMs();
    ...
    HDMI_INFO("...\n");
    pstHdmiDev->stDebug.stIntfStatus.u32PreformatOutTime = HDMI_OSAL_GetTimeInMs();
...
HI_S32 HI_DRV_HDMI_SetFormat(HI_UNF_HDMI_ID_E enHdmi, HDMI_VIDEO_ATTR_S *pstVideoAttr)
{
    ...
    pstHdmiDev = GetHdmiDevice(enHdmi);
    pstHdmiDev->stDebug.stIntfStatus.u32SetformatInTime = HDMI_OSAL_GetTimeInMs();

    HDMI_MEMSET(&stDrvVoAttr, 0, sizeof(DRV_HDMI_VO_ATTR_S));
    ...
    pstHdmiDev->stDebug.stIntfStatus.u32SetformatOutTime = HDMI_OSAL_GetTimeInMs();
...
HI_S32 HI_DRV_HDMI_Detach(HI_UNF_HDMI_ID_E enHdmi)
{
    ...
    pstHdmiDev = GetHdmiDevice(enHdmi);
    pstHdmiDev->stDebug.stIntfStatus.u32DeAttachInTime = HDMI_OSAL_GetTimeInMs();
    ...
    pstHdmiDev->stDebug.stIntfStatus.u32DeAttachOutTime = HDMI_OSAL_GetTimeInMs();
...
HI_S32 HI_DRV_HDMI_Attach(HI_UNF_HDMI_ID_E enHdmi, HDMI_VIDEO_ATTR_S *pstVideoAttr)
{
    ...
    pstHdmiDev = GetHdmiDevice(enHdmi);
    pstHdmiDev->stDebug.stIntfStatus.u32AttachInTime = HDMI_OSAL_GetTimeInMs();
    ...
    pstHdmiDev->stDebug.stIntfStatus.u32AttachOutTime = HDMI_OSAL_GetTimeInMs();
...
static HI_VOID Disp2HdmidrAttr(HDMI_DEVICE_S* pstHdmiDev, HDMI_HDR_ATTR_S *pstDrvHdrAttr, HI_DRV_HDMI_HDR_ATTR_S *pstHdrAttr)
{
    ...
    HDMI_MEMCPY(&(pstDrvHdrAttr->unDescriptor), &(pstHdrAttr->unDescriptor), sizeof(HDMI_META_DESCRIPTOR_UN));
}
...
HI_S32 HI_DRV_HDMI_SetHdrMode(HI_UNF_HDMI_ID_E enHdmi, HI_DRV_HDMI_HDR_USERMODE_E enUserHdrMode)
{
    ...
    if(HDMI_HDR_USERMODE_DOLBY == enHdrMode)
    {
           ...
           || HDMI_COLORSPACE_YCbCr444 != pstAppAttr->enOutColorSpace)
        {
            ...
            if (s32Ret != HI_SUCCESS)
            {
        	    HDMI_ERR("HI_DRV_HDMI_Stop fail\n");
        	    return s32Ret;
            }
...
HI_S32 HI_DRV_HDMI_SetHdrAttr(HI_UNF_HDMI_ID_E enHdmi, HI_DRV_HDMI_HDR_ATTR_S *pstHdrAttr)
{
    ...
    HDMI_MEMSET(&stDrvHdr, 0, sizeof(DRV_HDMI_HDR_S));
...
#elif defined(CONFIG_HDMI_BVT_SDK)
static HI_VOID Disp2HdmidrAttr(HDMI_DEVICE_S* pstHdmiDev, HDMI_HDR_ATTR_S *pstDrvHdrAttr, HI_HDMI_HDR_ATTR_S *pstHdrAttr)
{
    CHECK_HDMI_PTR_NULL_NO_RET(pstHdmiDev);
    CHECK_HDMI_PTR_NULL_NO_RET(pstDrvHdrAttr);
    CHECK_HDMI_PTR_NULL_NO_RET(pstHdrAttr);
...
HI_S32 HI_DRV_HDMI_Stop(HI_HDMI_DEV_ID_E enHdmi)
{
    HI_S32         s32Ret     = HI_FAILURE;

    HDMI_INFO("in...\n");

    s32Ret = DRV_HDMI_CmdProcess(CMD_HDMI_STOP, (HI_VOID *)(&enHdmi), HI_FALSE);
    if (s32Ret != HI_SUCCESS)
    {
	    HDMI_ERR("stop hdmi err!:0x%x\n", s32Ret);
	    return s32Ret;
...
HI_S32 HI_DRV_HDMI_SetHdrAttr(HI_HDMI_DEV_ID_E enHdmi, HI_HDMI_HDR_ATTR_S *pstHdrAttr)
{
    HI_S32          s32Ret = HI_FAILURE;
    DRV_HDMI_HDR_S  stDrvHdr;
    HDMI_HDR_ATTR_S *pDrvHdrAttr = HI_NULL;
    HDMI_DEVICE_S   *pstHdmiDev = HI_NULL;
    ...
    if(HI_HDMI_HDR_MODE_BUTT == pstHdrAttr->enHdrMode)
    {
        return HI_FAILURE;
    }
