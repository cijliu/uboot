...
HI_S32 DRV_HDMI_TeeOpen(HDMI_DEVICE_ID_E enHdmiId)
{
    ...
    HDMI_MEMSET(&(pstTeeInfo->stTeecContext), 0, sizeof(pstTeeInfo->stTeecContext));
    HDMI_MEMSET(&(pstTeeInfo->stTeecOperation), 0, sizeof(pstTeeInfo->stTeecOperation));
    HDMI_MEMSET(&(pstTeeInfo->stTeecSession), 0, sizeof(pstTeeInfo->stTeecSession));
    ...
    pstTeeInfo->stTeecOperation.params[3].tmpref.size = HDMI_STRLEN(TEE_HDMI_TA_NAME) + 1;
...
HI_S32 DRV_HDMI_TeeCmdSend(HDMI_DEVICE_ID_E enHdmiId, HDMI_TEE_PARAM_S *pstParam)
{
    ...
    if(   (pstParam->enTeeCmd >= HI_HDMI_COMM_BUTT)
       ...
       || (pstTeeInfo->bTeeOpen != HI_TRUE))
    {
        HDMI_ERR("Tee command(%d) is invaild or tee is not open(%d). \n", pstParam->enTeeCmd, pstTeeInfo->bTeeOpen);
        return HI_FAILURE;
    }
    ...
    switch(pstParam->enTeeCmd)
    {
        ...
        case HI_HDMI_HDCP14_INIT ... HI_HDMI_HDCP14_EVENT :
            ...
            HDMI_MEMCPY(pstTeeInfo->au8TeeDataBuf,(HI_VOID *)&pstParam->enTeeCmd,sizeof(HI_U32));
            if (pstParam->u32Size > (HDMI_TEE_DATA_SIZE - sizeof(HI_U32)))
            {
                HDMI_ERR("TEE param is invalid!\n")
                return HI_FAILURE;
            }
            HDMI_MEMCPY(pstTeeInfo->au8TeeDataBuf+sizeof(HI_U32),pstParam->pvData,pstParam->u32Size);
