...
{
    HAL_CALL(HAL_HDMI_PhyOutputEnableSet, bPhyOutputEnable);
...
static HI_S32 HdmiScrambleEnableSet(HDMI_DEVICE_S* pstHdmiDev, HI_BOOL bEnable)
{
    ...
    HDMI_HAL_CALL(HAL_HDMI_ScdcStatusGet, &stScdcStatus);
    ...
    HDMI_HAL_CALL(HAL_HDMI_VideoMuteSet, HI_TRUE);
    HDMI_HAL_CALL(HAL_HDMI_ScdcConfig, &stScdcConfig);
    HDMI_HAL_CALL(HAL_HDMI_VideoMuteSet, HI_FALSE);
...
static HI_S32 HdmiColorSpaceCheck(HDMI_DEVICE_S* pstHdmiDev,HDMI_APP_ATTR_S *pstUserApp)
{
    ...
    HDMI_HAL_CALL(HAL_HDMI_TxCapabilityGet, &stTxCap);
    ...
    if (!bSupportClrSpace)
    {
        ...
        {
            HDMI_WARN("fail,clr(%d) adapt to RGB!\n", pstUserApp->enOutColorSpace);
...
static HI_S32 HdmiDeepColorCheck(HDMI_DEVICE_S* pstHdmiDev, HDMI_APP_ATTR_S *pstUserApp, HI_U32 u32MaxTmdsClk)
{
    ...
    else
    {
        ...
        {
            HDMI_WARN("fail,deepclr(%d) adapt to 8bit!\n",pstUserApp->enDeepColorMode);
            pstUserApp->enDeepColorMode = HDMI_DEEP_COLOR_24BIT;
        }
        else
        {
...
static HI_S32 HdmiColorAndBitStrategy(HDMI_DEVICE_S* pstHdmiDev, HDMI_APP_ATTR_S *pstUserApp)
{
    ...
    /* DVI mode, must set RGB & DeepColor Off */
...
static HI_S32 HdmiCecNetWorkSet(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    {
        s32Ret = HAL_CALL_RET(HAL_HDMI_CecNetPing, pstHdmiDev->stCecInfo.au8PhysicalAddr);
    }
...
HI_S32 HdmiModeStrategy(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    if (!pstAppAttr->bEnableHdmi)
    {
        ...
        if (   (pstVoAttr->u32HdmiAdaptPixClk > HDMI_EDID_MAX_HDMI14_TMDS_RATE)
            && (!pstAppAttr->bEnableClrSpaceAdapt))
        {
...
#else
    enTmdsMode = (HI_TRUE == pstAppAttr->bEnableHdmi) ? HDMI_TMDS_MODE_HDMI_1_4 : HDMI_TMDS_MODE_DVI;
#endif

    HDMI_INFO("Tmds Mode %u->%u.\n",pstHdmiDev->enTmdsMode, enTmdsMode);
    pstHdmiDev->enTmdsMode = enTmdsMode;

    HDMI_HAL_CALL(HAL_HDMI_TmdsModeSet, pstHdmiDev->enTmdsMode);
    ...
    {
        HDMI_HAL_CALL(HAL_HDMI_ScdcStatusGet, &stScdcGet);
            ...
            || (HI_TRUE == pstHdmiDev->stAttr.stAppAttr.bAuthMode))
        {
            ...
            HDMI_HAL_CALL(HAL_HDMI_ScdcStatusSet, &stScdcStatus);
        }
    }
    else
    {
...
static HI_S32 HdcpSet(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    HDMI_HAL_CALL(HAL_HDMI_HotPlugStatusGet, &bHotPlug);
    ...
    if (bHotPlug)// hot plug in
    {
        if (pstHdmiDev->stHdcpInfo.bHdcpIsOn && !pstHdmiDev->stHdcpInfo.bHdcpWasOn)
        {
            ...
            HDMI_HAL_CALL(HAL_HDMI_HdcpEnableSet, HI_TRUE);
        ...
        else if (!pstHdmiDev->stHdcpInfo.bHdcpIsOn && pstHdmiDev->stHdcpInfo.bHdcpWasOn)
        {
            ...
            HDMI_HAL_CALL(HAL_HDMI_HdcpEnableSet, HI_FALSE);
            HDMI_INFO("Stop hdcp\n");
        }
    }
    else// hot plug out
...
static HI_S32 HdcpStrategy(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    if (pstHdmiDev->stHdcpInfo.bHdcpIsOn)
    {
        ...
        if (HI_TRUE == bHdcp22Mode)
        {
            ...
            s32Ret = HAL_CALL_RET(HAL_HDMI_HdcpMcuCodeLoad, pstHdmiDev->stHdcpInfo.enHdcpMode);
            if(HI_SUCCESS != s32Ret)
            {
        ...
        s32Ret = HAL_CALL_RET(HAL_HDMI_HdcpModeSet, pstHdmiDev->stHdcpInfo.enHdcpMode);
        if (s32Ret != HI_SUCCESS)
        {
...
{
    HDMI_HARDWARE_STATUS_S   stHwStatus   = {0};
    HDMI_AO_ATTR_S*          pstAudioAttr = HI_NULL;
    ...
    CHECK_POINTER(pstAppAttr);
    CHECK_POINTER(pstAudioAttr);
    CHECK_POINTER(pstVideoAttr);

    HDMI_MEMSET(&stHwStatus, 0, sizeof(HDMI_HARDWARE_STATUS_S));

    HDMI_HAL_CALL(HAL_HDMI_HardwareStatusGet, &stHwStatus);
...
#ifdef HDMI_HDCP_SUPPORT
            HDMI_HAL_CALL(HAL_HDMI_HdcpSinkCapabilityGet, &(pstHdmiDev->stEdidInfo.stCapability.stHdcpSupport));
#endif
            if (s32Ret == HI_SUCCESS)
            {
...
#ifdef HDMI_CEC_SUPPORT
    if (HI_TRUE == pstHdmiDev->stCecInfo.bCecEnable)
    {
...
static HI_S32 HdmiEventCallback(HI_VOID* pData , HDMI_EVENT_E enEvent)
{
    ...
    switch (enEvent)
    {
        ...
        case HDMI_EVENT_HDCP_FAIL:
            {
                DRV_HDMI_BlackDataSet(pstHdmiDev, HI_TRUE);
                HDMI_HAL_CALL(HAL_HDMI_AudioPathEnableSet, HI_FALSE);
                pstHdmiDev->stHdcpInfo.bHdcpAuthSuc = HI_FALSE;
#ifdef HDMI_TEE_SUPPORT
        ...
        case HDMI_EVENT_HDCP_SUCCESS:
            {
                DRV_HDMI_BlackDataSet(pstHdmiDev, HI_FALSE);
                HDMI_HAL_CALL(HAL_HDMI_AudioPathEnableSet, HI_TRUE);
...
{
    if (   pstHdmiDev
        && pstHdmiDev->pstHdmiHal
        && pstHdmiDev->pstHdmiHal->HAL_HDMI_DispFmt2HdmiTiming)
    {
...
static HI_S32 HdmiDeviceInit(HDMI_DEVICE_S * pstHdmiDev)
{
    ...
    HDMI_MEMSET(&stBaseParam, 0, sizeof(stBaseParam));
#endif
    ...
    pstHdmiDev->stDelay.u32FmtDelay = 500;
    pstHdmiDev->stDelay.u32MuteDelay= 120;
    ...
    pstHdrAttr->enColorimetry       = HDMI_HDR_COLORIMETRY_ITU_601;

    pstAppAttr->bEnableDRMInfoFrame = HI_TRUE;
    pstHdmiDev->stHdrMutex.bAttrBusy                    = HI_FALSE;
    pstHdmiDev->stHdrMutex.bAttrSet                     = HI_FALSE;
    ...
    if (pstHdmiDev->pstHdmiHal && pstHdmiDev->pstHdmiHal->HAL_HDMI_DispFmtGet)
    {
        ...
        HDMI_INFO("DispFmt=%u,PixClk=%u,enDeepColorMode=%u,enVidOutMode=%u\n",
    ...
    pstHdmiDev->stModParam.enTraceLen = HDMI_TRACE_LEN_2;
#else
    pstHdmiDev->stModParam.enTraceLen = HDMI_TRACE_LEN_1;
#endif
...
HI_S32 DRV_HDMI_Open(HDMI_DEVICE_S* pstHdmiDev, HI_BOOL bUser)
{
    ...
    HDMI_HAL_CALL(HAL_HDMI_PhyOutputEnableGet, &bHdmiOn);
    ...
    if (HI_FALSE == bHdmiOn)
    {
        ...
        pstHdmiDev->enTransitionState = HDMI_TRANSITION_NONE;
        s32Ret = HAL_CALL_NOPARAM(HAL_HDMI_HardwareInit);
        if (HI_SUCCESS != s32Ret)
        {
            return s32Ret;
        }
    ...
    HDMI_HAL_CALL(HAL_HDMI_IrqEnableSet, HDMI_INT_TYPE_HPD_RSEN, HI_TRUE);
...
HI_S32 DRV_HDMI_Start(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    HDMI_HAL_CALL(HAL_HDMI_AvMuteSet, HI_FALSE);
...
static HI_VOID HdmiDebugDelay(HDMI_DEVICE_S* pstHdmiDev, HI_CHAR *pcInfo)
{
    CHECK_POINTER_NO_RET(pcInfo);
	CHECK_POINTER_NO_RET(pstHdmiDev);

    HDMI_MSLEEP(pstHdmiDev->stDebug.u32StopDelay);
    if(pstHdmiDev->stDebug.u32StopDelay > 0)
    {
        HDMI_INFO("%s, delay %dms...\n", pcInfo, pstHdmiDev->stDebug.u32StopDelay);
    }
}
...
HI_S32 DRV_HDMI_Stop(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    DRV_HDMI_CompatStopDelay(pstHdmiDev->u32HdmiDevId);
    HdmiDebugDelay(pstHdmiDev, "stop delay");
    HDMI_INFO("<<< out...\n");
...
HI_VOID DRV_HDMI_AVMuteSet(HDMI_DEVICE_S* pstHdmiDev, HI_BOOL bAvMute)
{
    ...
    if(HI_TRUE != pstHdmiDev->stAttr.stAppAttr.bAuthMode)
    {
        ...
        if (!bAvMute)
        {
            ...
            HDMI_MSLEEP(stDelay.u32FmtDelay);
        }
    }

    HAL_CALL(HAL_HDMI_AvMuteSet, bAvMute);
    ...
    if(HI_TRUE != pstHdmiDev->stAttr.stAppAttr.bAuthMode)
    {
        ...
        if (bAvMute)
        {
            ...
            HDMI_MSLEEP(stDelay.u32MuteDelay);
        }
    }
...
HI_VOID DRV_HDMI_BlackDataSet(HDMI_DEVICE_S* pstHdmiDev, HI_BOOL bEnable)
{
    ...
    HAL_CALL(HAL_HDMI_BlackDataSet, &stBlackInfo);
...
HI_S32 DRV_HDMI_EmiStatusGet(HDMI_DEVICE_S* pstHdmiDev, HDMI_EMI_STATUS_S *pstEmiStatus)
{
    ...
    HAL_CALL(HAL_HDMI_EmiStatusGet, pstEmiStatus);
...
HI_S32 DRV_HDMI_EmiSet(HDMI_DEVICE_S* pstHdmiDev, HDMI_VO_ATTR_S* pstHwVOAttr, HDMI_APP_ATTR_S* pstHwAPPAttr)
{
    ...
    HDMI_HAL_CALL(HAL_HDMI_EmiSet, &stEmiConfig);
...
HI_S32 DRV_HDMI_DeepColorSet(HDMI_DEVICE_S* pstHdmiDev, HDMI_DEEP_COLOR_E enDeepColor)
{
    ...
    HDMI_MEMSET(&stAttr, 0, sizeof(HDMI_ATTR_S));
...
HI_S32 DRV_HDMI_AOAttrSet(HDMI_DEVICE_S* pstHdmiDev, HDMI_AO_ATTR_S *pstAOAttr)
{
    ...
    HDMI_HAL_CALL(HAL_HDMI_AudioPathEnableSet, HI_FALSE);
    HDMI_HAL_CALL(HAL_HDMI_AudioPathSet, &stAudioCfg);
    DRV_HDMI_AudioInfoFrameSend(&pstHdmiDev->stInfoFrame, (pstAppAttr->bEnableAudio && pstAppAttr->bEnableAudInfoFrame));
    HDMI_HAL_CALL(HAL_HDMI_AudioPathEnableSet, stAudioCfg.bEnableAudio);
...
{
    HAL_CALL(HAL_HDMI_AudioPathEnableSet, bEnable);
...
HI_S32 DRV_HDMI_AudioOutputEnableSet(HDMI_DEVICE_S* pstHdmiDev, HI_BOOL bEnable)
{
    ...
    HDMI_HAL_CALL(HAL_HDMI_AudioMuteSet, bMute);
...
static HI_S32 CheckVideoAttr(HDMI_VO_ATTR_S* pstVOAttr)
{
    ...
    {
        HDMI_ERR("video PictureAspect(%d) is wrong\n", pstVOAttr->enPictureAspect);
        return HI_FAILURE;
    }
    ...
    {
        HDMI_ERR("video InBitDepth(%d) is wrong\n", pstVOAttr->enInBitDepth);
        return HI_FAILURE;
    }
...
static HI_S32 HdmiCscSet(HDMI_DEVICE_S* pstHdmiDev, HDMI_VO_ATTR_S* pstVOAttr)
{
    ...
    HDMI_HAL_CALL(HAL_HDMI_CscParamSet, &stVideoCfg);
...
static HI_S32 HdmiVideoPathSet(HDMI_DEVICE_S* pstHdmiDev, HDMI_VO_ATTR_S* pstVOAttr)
{
    ...
    HDMI_MEMSET(&stVideoCfg, 0, sizeof(HDMI_VIDEO_CONFIG_S));
    stVideoCfg.bEmiEnable   = pstHdmiDev->bEmiEnable;

    HdmiAttrToVidcfg(pstVOAttr, pstAppAttr, &stVideoCfg);

    HDMI_HAL_CALL(HAL_HDMI_VideoPathSet, &stVideoCfg);
...
static HI_S32 CheckAppAttr(HDMI_APP_ATTR_S* pstAppAttr)
{
    ...
    {
        HDMI_ERR("OutColorSpace=%d is wrong\n", pstAppAttr->enOutColorSpace);
        s32Ret |=  HI_FAILURE;
    }
    ...
    {
        HDMI_ERR("DeepClrMode=%d is wrong\n", pstAppAttr->enDeepColorMode);
        s32Ret |=  HI_FAILURE;
    }
...
static HI_VOID HdrModeCheck(HDMI_DEVICE_S* pstHdmiDev, HDMI_VO_ATTR_S *pstVOAttr, HDMI_APP_ATTR_S *pstAppAttr)
{
    ...
    pstHdrAttr = &(pstHdmiDev->stAttr.stHdrAttr);
    CHECK_POINTER_NO_RET(pstHdrAttr);
...
HI_S32 DRV_HDMI_VOAttrSet(HDMI_DEVICE_S* pstHdmiDev, HDMI_VO_ATTR_S* pstVOAttr)
{
    ...
    pstHdmiDev->stHdrMutex.bAttrSet = HI_TRUE;
#endif
...
HI_S32 HdmiHdrAttrLock(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    HDMI_SPIN_LOCK(pstHdmiDev->stHdrMutex.stAttrSpin);

    if(   (HI_TRUE == pstHdmiDev->stHdrMutex.bAttrBusy)
       || (HI_FALSE == pstHdmiDev->stHdrMutex.bAttrSet))
    {
        HDMI_SPIN_UNLOCK(pstHdmiDev->stHdrMutex.stAttrSpin);
        HDMI_WARN("Attr busy(%d), Attr set(%d)!\n", pstHdmiDev->stHdrMutex.bAttrBusy, pstHdmiDev->stHdrMutex.bAttrSet);
        goto EXIT;
    }

    pstHdmiDev->stHdrMutex.bAttrBusy = HI_TRUE;
    HDMI_SPIN_UNLOCK(pstHdmiDev->stHdrMutex.stAttrSpin);
    s32Ret = HI_SUCCESS;
...
HI_S32 HdmiAttrLock(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    HDMI_SPIN_LOCK(pstHdmiDev->stHdrMutex.stAttrSpin);
    if(HI_FALSE == pstHdmiDev->stHdrMutex.bAttrBusy)
    {
        pstHdmiDev->stHdrMutex.bAttrBusy = HI_TRUE;
        HDMI_SPIN_UNLOCK(pstHdmiDev->stHdrMutex.stAttrSpin);
        goto EXIT;
...
static HI_VOID HdmiZeroDrmIfTimerSet(HDMI_DEVICE_S* pstHdmiDev, HDMI_TIMER_CONFIG_S* pstTimer)
{
    ...
    HDMI_TIMER_CONFIG_S  stTimerCfg = {0};
    HI_S32               s32Ret     = HI_SUCCESS;
    ...
    s32Ret = HAL_CALL(HAL_HDMI_HdrTimerSet, &stTimerCfg);
    if (HI_SUCCESS != s32Ret)
    {
        return;
    }
    ...
    HAL_CALL(HAL_HDMI_HdrTimerSet, pstTimer);
    ...
    switch(pstHdrAttr->enHdrMode)
    {
        ...
        case HDMI_HDR_MODE_CEA_861_3:
            ...
            if(HDMI_HDR_DEBUG_MODE_NONE != pstHdrInfo->enHdrDebugMode)
            {
                ...
                HDMI_HAL_CALL(HAL_HDMI_AvMuteSet, HI_TRUE);
        ...
        case HDMI_INFOFRAME_TYPE_AVI:
            {
                ...
                if (HI_TRUE == pstHdmiDev->stAttr.stAppAttr.bAuthMode)
                {
                    ...
                    (HI_VOID)DRV_HDMI_VOAttrSet(pstHdmiDev, pVoAttr);
                }
            }
...
HI_S32 DRV_HDMI_CecCmdSend(HDMI_DEVICE_S* pstHdmiDev,  HDMI_CEC_CMD_S  *pstCecCmd)
{
    ...
    s32Ret = HAL_CALL_RET(HAL_HDMI_CecMsgSend, pstCecCmd);

    HDMI_INFO("<<< out...u8Opcode=0x%x,src=%d,dst=%d \n",pstCecCmd->u8Opcode,pstCecCmd->enSrcAdd,pstCecCmd->enDstAdd);
...
HI_S32 DRV_HDMI_CecCmdReceive(HDMI_DEVICE_S* pstHdmiDev, HDMI_CEC_CMD_S  *pstCecCmd)
{
    ...
    s32Ret = HAL_CALL_RET(HAL_HDMI_CecMsgReceive, pstCecCmd);
...
HI_S32 DRV_HDMI_CecStatusGet(HDMI_DEVICE_S* pstHdmiDev, HDMI_CEC_INFO_S  *pstCecStatus)
{
    ...
    s32Ret = HAL_CALL_RET(HAL_HDMI_CecStatusGet, &pstHdmiDev->stCecInfo);
    if (HI_SUCCESS != s32Ret)
    {
        return s32Ret;
    }
...
HI_S32 DRV_HDMI_CecEnableSet(HDMI_DEVICE_S* pstHdmiDev, HI_BOOL bEnable)
{
    ...
    s32Ret = HAL_CALL_RET(HAL_HDMI_CecEnableSet,  bEnable);
...
HI_S32 DRV_HDMI_KeyLoad(HDMI_DEVICE_S* pstHdmiDev, HDMI_LOAD_KEY_S *pstLoadKey, HI_BOOL bFromUser)
{
    ...
    HDMI_MEMSET(&stFlashEncrytedHdcpKey, 0, sizeof(HI_DRV_CIPHER_FLASH_ENCRYPT_HDCPKEY_S));
     ...
     &&(HI_NULL != g_stCipherExtFunc->pfnCipherLoadHdcpKey))
    {
		...
		if(bFromUser)
		{
			s32Ret = HDMI_COPY_FROM_USER(stFlashEncrytedHdcpKey.u8Key, (void __user *)pstLoadKey->pu8InputEncryptedKeyAddr, pstLoadKey->u32KeyLength);
		}
		else
		{
...
HI_S32 DRV_HDMI_DelaySet(HDMI_DEVICE_S* pstHdmiDev, HDMI_DELAY_S *pHdmiDelay)
{
    ...
    HDMI_INFO("bForceMuteDelay: %d, u32MuteDelay: %d\n", pHdmiDelay->bForceMuteDelay, pHdmiDelay->u32MuteDelay);
    HDMI_INFO("bForceFmtDelay:  %d, u32FmtDelay:  %d\n", pHdmiDelay->bForceFmtDelay, pHdmiDelay->u32FmtDelay);
...
HI_S32 DRV_HDMI_ModParamSet(HDMI_DEVICE_S* pstHdmiDev, DRV_HDMI_MOD_PARAM_S *pstModParam)
{
    ...
    HDMI_INFO("Set trace length: %d->%d\n", pstParam->enTraceLen, pstModParam->enTraceLen);
...
HI_S32 DRV_HDMI_ModParamGet(HDMI_DEVICE_S* pstHdmiDev, DRV_HDMI_MOD_PARAM_S *pstModParam)
{
    ...
    pstModParam->enTraceLen = pstParam->enTraceLen;
...
HI_S32 DRV_HDMI_CmdProcess(unsigned int cmd, HI_VOID *arg, HI_BOOL bUser)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_HDMI_GET_SINK_CAPABILITY:
        {
            ...
            HDMI_MEMSET(&pstDrvSinkCap->stCap, 0, sizeof(HDMI_SINK_CAPABILITY_S));

            HDMI_MUTEX_LOCK(s_HdmiMutex);

            if (   HI_NULL == pstHdmiDev->pstHdmiHal
                || HI_NULL == pstHdmiDev->pstHdmiHal->HAL_HDMI_TxCapabilityGet)
            {
                HDMI_ERR("Null pointer!\n");
                s32Ret = HI_ERR_HDMI_NUL_PTR;
                HDMI_MUTEX_UNLOCK(s_HdmiMutex);
        ...
        case CMD_HDMI_GET_ATTR:
        {
            ...
            HDMI_MEMSET(&stHDMIAttr, 0, sizeof(HDMI_ATTR_S));
            HDMI_MEMSET(&pstDrvAppAttr->stAPPAttr, 0, sizeof(HDMI_APP_ATTR_S));
		...
		case CMD_HDMI_GET_VO_ATTR:
        {
            ...
            HDMI_MEMSET(&pstDrvVOAttr->stVOAttr, 0, sizeof(HDMI_VO_ATTR_S));
        ...
        case CMD_HDMI_GET_CEC:
        {
            ...
            HDMI_MEMSET(&pstCecCmd->CECCmd, 0, sizeof(HDMI_CEC_CMD_S));
        ...
        case CMD_HDMI_GET_CECSTATUS:
        {
            ...
            HDMI_MEMSET(&pstCecStatus->stCecInfo, 0, sizeof(HDMI_CEC_INFO_S));
        ...
        case CMD_HDMI_UPDATE_EDID:
        {
            ...
            HDMI_MEMSET(&pstDrvEdidRaw->stEdidRaw, 0, sizeof(HDMI_EDID_RAWDATA_S));
...
#ifdef HDMI_HDCP_SUPPORT
                if (   HI_NULL == pstHdmiDev->pstHdmiHal
                    || HI_NULL == pstHdmiDev->pstHdmiHal->HAL_HDMI_HdcpSinkCapabilityGet)
                {
                    HDMI_ERR("Null pointer!\n");
                    s32Ret = HI_ERR_HDMI_NUL_PTR;
                    HDMI_MUTEX_UNLOCK(s_HdmiMutex);
        ...
        case CMD_HDMI_GET_AO_ATTR:
        {
            ...
            HDMI_MEMSET(&pstDrvAOAttr->stAOAttr, 0, sizeof(HDMI_AO_ATTR_S));
        ...
        case CMD_HDMI_GET_STATUS:
        {
            ...
            HDMI_MEMSET(&pstStatus->stHdmiStatus, 0, sizeof(HDMI_STATUS_S));
        ...
        case CMD_HDMI_GET_AUDIO_CAPABILITY:
        {
            ...
            HDMI_MEMSET(&pstDrvAudioCap->stAudioCap, 0, sizeof(HDMI_AUDIO_CAPABILITY_S));
        ...
        case CMD_HDMI_GET_DELAY:
        {
            ...
            HDMI_MEMSET(&pstDelay->stDelay, 0, sizeof(HDMI_DELAY_S));
        ...
        case CMD_HDMI_GET_VIDEO_CAPABILITY:
        {
            ...
            HDMI_MEMSET(&pstDrvVideoCap->stVideoCap, 0, sizeof(HDMI_VIDEO_CAPABILITY_S));
...
#ifdef HDMI_HDR_SUPPORT
            HDMI_SPIN_LOCK_INIT(pstHdmiDev->stHdrMutex.stAttrSpin);
#endif
        }
    }
...
#ifdef HDMI_PRODUCT_HI3516CV500
    g_pstHdmiReg = (HI_CHAR *)HDMI_IO_MAP(BASE_ADDR_tx_dphy, HDMI_TX_REG_LEN);
#else
...
#ifdef HDMI_HDR_SUPPORT
            HDMI_SPIN_DESTORY(pstHdmiDev->stHdrMutex.stAttrSpin);
#endif
