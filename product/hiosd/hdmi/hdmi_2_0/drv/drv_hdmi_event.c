											...
											HDMI_EVENT_E *penEvent)
{
    ...
    *penEvent = HDMI_EVENT_BUTT;
    ...
    if(i >= HDMI_EVENT_POOL_CNT)
    {
        HDMI_ERR("The proc(%d) is not find\n",u32PoolID);
        HDMI_MSLEEP(500);
        return HI_FAILURE;
    }
    ...
    s32Ret = HDMI_WAIT_EVENT_TIMEOUT_INTERRUPT(pstEvtInfo->wtQueue, DRV_HDMI_EventCallback, (HI_VOID *)&stCallback,100);
    if (s32Ret <= 0)
    {
        //HDMI_INFO("Wait event time out\n");
        return HI_SUCCESS;
    }
