...
/* Global Data                                                                */
/******************************************************************************/
const static HI_CHAR *s_aDitherModeStr[] = {
    "12_10",            //HDMI_VEDIO_DITHER_12_10	    = 0x00,
    "12_8",             //HDMI_VEDIO_DITHER_12_8	    = 0x01,
    "10_8",             //HDMI_VEDIO_DITHER_10_8	    = 0x02,
    "disable",          //HDMI_VEDIO_DITHER_DISALBE	= 0x03,
};
...
const static HI_CHAR *s_eHdrEotfType2Str[] = {
    ...
    "SMPTE_ST_2084",    // HDMI_HDR_EOTF_SMPTE_ST_2084 = 2,
    "HLG",              // HDMI_HDR_EOTF_HLG = 3,
...
const static HI_CHAR *s_eExtColorimetry2Str[] = {
	...
	"ADDITIONAL",           // HDMI_EXTENDED_COLORIMETRY_Additional
...
const static HI_CHAR *s_eDispFMT2Str[] = {
       ...
       "576P@50",     //HI_HDMI_VIDEO_FMT_576P_50,
       ...
       "7680X4320P@30",    // HI_HDMI_VIDEO_FMT_7680X4320P_30
...
const static HI_CHAR *s_eBitDepth2Str[] = {
    ...
    "08 Bit",      //HDMI_VIDEO_BITDEPTH_OFF,
    "UNKNOWN"      //HDMI_VIDEO_BITDEPTH_BUTT
};

const static HI_CHAR *s_eDeepColorStr[] = {
    "24 Bit",      //HDMI_VIDEO_BITDEPTH_8,
    "30 Bit",      //HDMI_VIDEO_BITDEPTH_10,
    "36 Bit",      //HDMI_VIDEO_BITDEPTH_12,
    "48 Bit",      //HDMI_VIDEO_BITDEPTH_16,
    "24 Bit(OFF)", //HDMI_VIDEO_BITDEPTH_OFF,
    "UNKNOWN"      //HDMI_VIDEO_BITDEPTH_BUTT
};
...
const static HI_CHAR *s_eTrainMach2Str[] = {
	"ReadyCheck",   // HDMI_FRL_TRAIN_STEP_READY_CHECK
    "TrainStart",   // HDMI_FRL_TRAIN_STEP_TRAIN_START
    "ResultChk",    // HDMI_FRL_TRAIN_STEP_RESULT_CHECK
    "RateChange",   // HDMI_FRL_TRAIN_STEP_RATE_CHANGE
    "ResultHan",    // HDMI_FRL_TRAIN_STEP_RESULT_HANDLE
    "RetrainChk",   // HDMI_FRL_TRAIN_STEP_RETRAIN_CHECK
    "Stop",         // HDMI_FRL_TRAIN_STEP_STOP
    "BUTT",         // HDMI_FRL_TRAIN_STEP_BUTT
};

const static HI_CHAR *s_eTrainStatus2Str[] = {
	"NONE",     // HDMI_FRL_TRAIN_NONE
    "FAIL",     // HDMI_FRL_TRAIN_FAIL
    "SUCCESS",  // HDMI_FRL_TRAIN_SUCCESS
    "BUSY",     // HDMI_FRL_TRAIN_BUSY
    "BUTT",     // HDMI_FRL_TRAIN_BUTT
};

const static HI_CHAR *s_eRateSel2Str[] = {
	"LITTLE",   // HDMI_FRL_RATE_SELECT_LITTLE
    "BIG",      // HDMI_FRL_RATE_SELECT_BIG
    "BUTT",     // HDMI_FRL_RATE_SELECT_BUTT
};

const static HI_CHAR *s_eFrlStrategy2Str[] = {
	">600M",    // HDMI_FRL_STRATEGY_MODE_1
    ">340M",    // HDMI_FRL_STRATEGY_MODE_2
    "N-INTR",   // HDMI_FRL_STRATEGY_MODE_3
    "BUTT",     // HDMI_FRL_STRATEGY_MODE_BUTT
};

const static HI_CHAR *s_eWorkMode2Str[] = {
	"NONE",    // FRL_WORK_MODE_NONE
    "3L3G",    // FRL_WORK_MODE_3L3G
    "3L6G",    // FRL_WORK_MODE_3L6G
    "4L6G",    // FRL_WORK_MODE_4L6G
    "4L8G",    // FRL_WORK_MODE_4L8G
    "4L10G",   // FRL_WORK_MODE_4L10G
    "4L12G",   // FRL_WORK_MODE_4L12G
    "BUTT",    // FRL_WORK_MODE_BUTT
};

const static HI_CHAR *s_eTrainSel2Str[] = {
	"Software", // HDMI_FRL_TRAIN_SEL_SW
    "Hardware", // HDMI_FRL_TRAIN_SEL_HW
    "BUTT",     // HDMI_FRL_TRAIN_SEL_BUTT
};

const static HI_CHAR *s_eFrlRate2Str[] = {
	"NONE",    // 0, not support
    "3L3G",    // 1
    "3L6G",    // 2
    "4L6G",    // 3
...
static const HDMI_FMT_NAME_PARAM_S s_stFmtParam[]= {
	...
	/********************VIC,PixFfeq  , HACT ,VACT,FldRate,    Fmt************************/
	{						31, 148500,  1920,1080, 5000 ,	"1080P_50 16:9" 		},		// 31(1) 1920 x 1080p
	{						32, 74250 ,  1920,1080, 2400 ,	"1080P_24 16:9" 		},		// 32(2) 1920 x 1080p
	{						33, 74250 ,  1920,1080, 2500 ,	"1080P_25 16:9" 		},		// 33(3) 1920 x 1080p
	{						34, 74250 ,  1920,1080, 3000 ,	"1080P_30 16:9" 		},		// 34(4) 1920 x 1080p
...
**/
static HI_VOID Proc_SWVersion(HDMI_PROC_ENTRY *pFile)
{
    CHECK_POINTER(pFile);
	PROC_PRINT(pFile, "%s\n", g_pszVersion);
...
**/
static HI_VOID Proc_AppAttrInfo(HDMI_PROC_ENTRY *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
...
#ifdef HDMI_HDCP_SUPPORT
    PROC_PRINT(pFile, "%-15s: %-20s  ", "HDCPEnable", pstAppAttr->bHDCPEnable? STR_YES: STR_NO);
#endif
    PROC_PRINT(pFile, "\n");
    /* line 5 */
...
/** print software status
**/
static HI_VOID Proc_SWStatus(HDMI_PROC_ENTRY *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
	HDMI_DEVICE_S *pstHdmiDev = HI_NULL;
	HI_U32 i = 0;
	HI_U32 u32Index = 0;
#ifdef HDMI_FRL_SUPPORT
    HDMI_FRL_INFO_S *pstFrl   = HI_NULL;
#endif
	...
	CHECK_POINTER(pstHdmiDev);
#ifdef HDMI_FRL_SUPPORT
    pstFrl = &pstHdmiDev->stFrlInfo;
    CHECK_POINTER(pstFrl);
#endif
	...
	/* line 2 */
#ifdef HDMI_FRL_SUPPORT
    u32Index = (HDMI_FRL_MODE_FRL == pstFrl->enMode) ? \
        HDMI_TMDS_MODE_HDMI_2_1 : pstHdmiDev->enTmdsMode;

    CHECK_MAX_VALUE(u32Index, u32Index, HDMI_TMDS_MODE_BUTT);
#else
    ...
    /* line 6 */
    PROC_PRINT(pFile, "%-15s: %-20s  ", "EmiEnable", pstHdmiDev->bEmiEnable ? STR_YES : STR_NO);
    PROC_PRINT(pFile, "%-15s: %-20d \n", "PcbLen", pstHdmiDev->stModParam.enTraceLen);
#ifndef CONFIG_HDMI_BVT_SDK
    /* line 7 */
    PROC_PRINT(pFile, "%-15s: %-20d  ", "AttachInTime", pstHdmiDev->stDebug.stIntfStatus.u32AttachInTime);
    PROC_PRINT(pFile, "%-15s: %-20d \n", "AttachOutTime", pstHdmiDev->stIntfStatus.u32AttachOutTime);

    /* line 8 */
    PROC_PRINT(pFile, "%-15s: %-20d  ", "DeattachInTime", pstHdmiDev->stDebug.stIntfStatus.u32DeAttachInTime);
    PROC_PRINT(pFile, "%-15s: %-20d \n", "DeattachOutTime", pstHdmiDev->stDebug.stIntfStatus.u32DeAttachOutTime);

    /* line 9 */
    PROC_PRINT(pFile, "%-15s: %-20d  ", "PreformatInTime", pstHdmiDev->stDebug.stIntfStatus.u32PreformatInTime);
    PROC_PRINT(pFile, "%-15s: %-20d \n", "PreformatOutTime", pstHdmiDev->stDebug.stIntfStatus.u32PreformatOutTime);

    /* line 10 */
    PROC_PRINT(pFile, "%-15s: %-20d  ", "SetformatInTime", pstHdmiDev->stDebug.stIntfStatus.u32SetformatInTime);
    PROC_PRINT(pFile, "%-15s: %-20d \n", "SetformatOutTime", pstHdmiDev->stDebug.stIntfStatus.u32SetformatOutTime);

    /* line 11 */
    PROC_PRINT(pFile, "%-15s: %-20d  ", "ResumeInTime", pstHdmiDev->stDebug.stIntfStatus.u32ResumeInTime);
    PROC_PRINT(pFile, "%-15s: %-20d \n", "ResumeOutTime", pstHdmiDev->stDebug.stIntfStatus.u32ResumeOutTime);

    /* line 12 */
    PROC_PRINT(pFile, "%-15s: %-20d  ", "SuspendInTime", pstHdmiDev->stDebug.stIntfStatus.u32SuspendInTime);
    PROC_PRINT(pFile, "%-15s: %-20d \n", "SuspendOutTime", pstHdmiDev->stDebug.stIntfStatus.u32SuspendOutTime);
#endif
...
**/
#ifdef HDMI_CEC_SUPPORT
static HI_VOID Proc_CECStatus(HDMI_PROC_ENTRY *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    HI_U32 i = 0;
...
**/
#ifdef HDMI_HDCP_SUPPORT
static HI_VOID Proc_HDCPStatus(HDMI_PROC_ENTRY *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    HDMI_DEVICE_S *pstHdmiDev = HI_NULL;
...
#if defined HDMI_HDR_SUPPORT
static HI_VOID Proc_HDRStatus(HDMI_PROC_ENTRY *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    HDMI_DEVICE_S           *pstHdmiDev     = HI_NULL;
...
#ifdef HDMI_FRL_SUPPORT
static HI_VOID Proc_FRLStatus(HDMI_PROC_ENTRY *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    HDMI_DEVICE_S           *pstHdmiDev      = HI_NULL;
    HDMI_FRL_INFO_S         *pstFrl          = HI_NULL;
    ...
    pstFrl = &pstHdmiDev->stFrlInfo;
    CHECK_POINTER(pstFrl);
    pstMachInfo = &pstFrl->stStateMachInfo;
    CHECK_POINTER(pstMachInfo);

    /* Line 1*/
    PROC_PRINT(pFile, "%-15s: %-20s  ", "MachRun", pstMachInfo->bStartMach ? STR_YES : STR_NO);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "FRLEnable", pstFrl->bStart ? STR_YES : STR_NO);

    /* Line 2*/
    CHECK_MAX_VALUE(u32Index, pstFrl->enTrainSel, HDMI_FRL_TRAIN_SEL_BUTT);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "TrainSel", s_eTrainSel2Str[u32Index]);

    /* Line 3*/
    PROC_PRINT(pFile, "%-15s: %-20s  ", "DscEnable", pstFrl->bDscEnable ? STR_YES : STR_NO);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "FrlCtsEnable", pstFrl->bCTSMode ? STR_YES : STR_NO);

    /* Line 4*/
    CHECK_MAX_VALUE(u32Index, pstMachInfo->enTrainStatus, HDMI_FRL_TRAIN_STEP_BUTT);
    PROC_PRINT(pFile, "%-15s: %-20s  ", "MachStatus", s_eTrainMach2Str[u32Index]);
    CHECK_MAX_VALUE(u32Index, pstFrl->stTrainStatus.enFrlTrainStatus, HDMI_FRL_TRAIN_BUTT);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "TrainStatus", s_eTrainStatus2Str[u32Index]);

    /* Line 5*/
    CHECK_MAX_VALUE(u32Index, pstFrl->enRateSelect, HDMI_FRL_RATE_SELECT_BUTT);
    PROC_PRINT(pFile, "%-15s: %-20s  ", "RateSelect", s_eRateSel2Str[u32Index]);
    CHECK_MAX_VALUE(u32Index, pstFrl->enStrategyMode, HDMI_FRL_STRATEGY_MODE_BUTT);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "FrlStrategy", s_eFrlStrategy2Str[u32Index]);

    /* Line 6*/
    PROC_PRINT(pFile, "%-15s: %-20d  ", "MaxFailTime", pstFrl->u32MaxFailTimes);
    PROC_PRINT(pFile, "%-15s: %-20d \n", "WaitHandTime", pstMachInfo->u32WaitHandleMs);

    /* Line 7*/
    PROC_PRINT(pFile, "%-15s: %-20d  ", "WaitReadyTime", pstMachInfo->u32WaitReadyMs);
    PROC_PRINT(pFile, "%-15s: %-20d \n", "WaitRetrainTime", pstMachInfo->u32WaitRetrainMs);
...
**/
static HI_VOID Proc_HWStatus(HDMI_PROC_ENTRY *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    HDMI_DEVICE_S           *pstHdmiDev   = HI_NULL;
    static HDMI_HARDWARE_STATUS_S   stHwStatus   = {0};
...
#ifdef HDMI_FRL_SUPPORT
    HDMI_FRL_STATUS_S       *pstFrlStatus = HI_NULL;
#endif
    ...
    HAL_CALL(HAL_HDMI_HardwareStatusGet, &stHwStatus);
    ...
    /* line 3 */
#ifdef HDMI_FRL_SUPPORT
    pstFrlStatus = &stHwStatus.stFrlStatus;

    PROC_PRINT(pFile, "%-15s: %-20s  ",	"WorkEn", pstFrlStatus->bWorkEn ? STR_YES: STR_NO);
    CHECK_MAX_VALUE(u32Index, pstFrlStatus->enWorkMode, FRL_WORK_MODE_BUTT);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "WorkMode", s_eWorkMode2Str[u32Index]);
    /* TMDSMode is HDMI2.1 when phy work at FRL mode.*/
    if(HI_TRUE == pstFrlStatus->bFrlStart)
    {
        pstComStatus->enTmdsMode = HDMI_TMDS_MODE_HDMI_2_1;
    }
#endif
    ...
    HAL_CALL(HAL_HDMI_ScdcStatusGet, &stScdcStatus);
    ...
    /* line 6 */
    PROC_PRINT(pFile, "%-15s: 0x%-20  x", "CLKSwing", pstPhyConfig->u8CLKSwing);
    ...
    HAL_CALL(HAL_HDMI_EmiStatusGet, &stEmiStatus);
...
static HI_VOID Proc_IFRawData(HDMI_PROC_ENTRY *pFile, HI_U32 u32Len,
...
**/
static HI_VOID Proc_VideoInfo(HDMI_PROC_ENTRY *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    HI_U32 i = 0, len = 0, u32Line = 8;
    HI_U8 *paData = HI_NULL;
    HDMI_DEVICE_S *pstHdmiDev = HI_NULL;
    HDMI_HAL_S    *pstHdmiHal = HI_NULL;
    static HDMI_HARDWARE_STATUS_S stHwStatus;
    ...
    i = DRV_HDMI_VicToIndex(u32VideoCodeIndex);

    PROC_PRINT(pFile, "%-15s: %-20s |", "VideoTiming", s_stCEAVideoCodesDes[i].pcFmtStr);
    ...
    if (format == HDMI_VIDEO_FORMAT_4K)
    {
        ...
        {
            pVSIFTmpFmt = s_stH14bHdmiVideoCodes_4K[enHdmiVic - 1].pcFmtStr; // s_stH14bHdmiVideoCodes_4K index = enHdmiVic - 1
        }
    }
    ...
    {
        // normal
        i = DRV_HDMI_VicToIndex(stHwStatus.stInfoFrameStatus.u8AVI[7]);
        pTmpFmt = s_stCEAVideoCodesDes[i].pcFmtStr;
    }
	...
	if (HDMI_COLORSPACE_YCbCr422 == stHwStatus.stVideoStatus.enOutColorSpace)
	{
		u32Index = HDMI_VIDEO_BITDEPTH_12;
	}
	...
	/* line 25 */
    CHECK_MAX_VALUE(u32Index, stHwStatus.stVideoStatus.enDither, HDMI_VIDEO_DITHER_DISALBE);
    PROC_PRINT(pFile,"%-15s: %-20s |", "Dither",s_aDitherModeStr[u32Index]);
    Proc_IFRawData(pFile, len, &i, &u32Line, paData);
    CHECK_MAX_VALUE(u32Index, stHwStatus.stVideoStatus.enOutBitDepth, HDMI_VIDEO_BITDEPTH_BUTT);
    PROC_PRINT(pFile,"%-15s: %-20s |\n",	"DeepColorMode", s_eDeepColorStr[u32Index]);
...
**/
#ifdef HDMI_HDR_SUPPORT
static HI_VOID Proc_HDRInfo(HDMI_PROC_ENTRY *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    HI_U8            i          = 0;
    HI_U32           len        = 0;
    HI_U8           *paData     = HI_NULL;
...
**/
static HI_VOID Proc_AudioInfo(HDMI_PROC_ENTRY *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
    HI_U32 i = 0, len = 0;
    HI_U8 *paData = HI_NULL;
    HDMI_DEVICE_S *pstHdmiDev = HI_NULL;
...
**/
static HI_VOID Proc_EventInfo(HDMI_PROC_ENTRY *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
	HI_U32 i=0,j=0;
	HI_U32 u32TotalPool = 0;
 	HI_S32 s32Ret = HI_SUCCESS;
	HDMI_DEVICE_S *pstHdmiDev = HI_NULL;
	HDMI_EVENT_RUN_CNT_S stRunCnt = {0};
	HDMI_EVENT_RUN_CTRL_S stCtrl = {0};
    ...
    HDMI_MEMSET(&stCtrl, 0, sizeof(HDMI_EVENT_RUN_CTRL_S));
    HDMI_MEMSET(&stRunCnt, 0, sizeof(HDMI_EVENT_RUN_CNT_S));

	s32Ret = DRV_HDMI_EventPoolTotalGet(enHdmiID,&u32TotalPool);
	CHECK_RET_SUCCESS(s32Ret);

    if (u32TotalPool >= HDMI_EVENT_POOL_CNT)
    {
        HDMI_ERR("u32TotalPool(%d) must less than %d.\n", u32TotalPool, HDMI_EVENT_POOL_CNT);
        return ;
    }
...
**/
static HI_VOID Proc_EdidRawData(HDMI_PROC_ENTRY *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
	...
	pu8EdidRaw = (HI_U8 *)HDMI_VMALLOC(HDMI_EDID_TOTAL_SIZE);
	CHECK_POINTER(pu8EdidRaw);

    HDMI_MEMSET(pu8EdidRaw, 0, HDMI_EDID_TOTAL_SIZE);
...
**/
static HI_VOID Proc_EdidStatus(HDMI_PROC_ENTRY *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
	HI_S32 s32Ret;
	...
	pstEdidInfo = &pstHdmiDev->stEdidInfo;
    HDMI_MEMSET(&stStatus, 0x0, sizeof(HDMI_EDID_STATUS_S));

    s32Ret = DRV_HDMI_EdidStatusGet(pstEdidInfo, &stStatus);
    if (HI_SUCCESS != s32Ret)
    {
        HDMI_ERR("DRV_HDMI_EdidStatusGet failed!!!\n");
        return;
    }
...
**/
static HI_VOID Proc_SinkBasicCap(HDMI_PROC_ENTRY* pFile, HDMI_SINK_CAPABILITY_S *pstCap)
{
    HI_S32 i = 0;
    ...
    HDMI_MEMSET(StrTmpArray, '\0', sizeof(StrTmpArray));
    HDMI_STRNCPY(StrTmpArray, pstCap->stMfrsInfo.cMfrsName, 4);
    ...
    for (i = 0; i < ARRAY_SIZE(pstCap->bSupportAudioSpeaker); i++)
    {
        if (i < ARRAY_SIZE(s_aStr_Speaker))
        {
    ...
    /* line 10 */
    PROC_PRINT(pFile,"%-20s: %-20s ", "RgbQuanSelectable", pstCap->bRgbQrangeSelectable ? STR_YES: STR_NO);
    PROC_PRINT(pFile,"%-20s: %-20s \n", "YccQuanSelectable", pstCap->bYccQrangeSelectable ? STR_YES: STR_NO);
...
**/
static HI_VOID Proc_SinkVideoCap(HDMI_PROC_ENTRY *pFile, HDMI_SINK_CAPABILITY_S *pstCap)
{
    HI_S32 i = 0;
    HI_CHAR *pTmpFmt = HI_NULL;
    ...
    for (i = 0; (i < pstCap->u32SupportY420VICNum) && (i < HDMI_EDID_MAX_VIC_COUNT); i++)
    {
    ...
    for (i = 0; (i < pstCap->u32OnlySupportY420VICNum) && (i < HDMI_EDID_MAX_VIC_COUNT); i++)
    {
...
/** print the video capability of sink
**/
static HI_VOID Proc_SinkFormatCap(HDMI_PROC_ENTRY *pFile, HDMI_SINK_CAPABILITY_S *pstCap)
{
    HI_S32 i = 0, j = 0;
    HI_CHAR *pTmpFmt = HI_NULL;
	...
	for (i = 0; (i < pstCap->u32SupportVICNum) && (i < HDMI_EDID_MAX_VIC_COUNT); i++)
	{
	...
	for (i = 0; i < ARRAY_SIZE(pstCap->stStdTiming); i++)
	{
		...
		{
			int n = 0;
			HDMI_MEMSET(aTmpArray, '\0', sizeof(aTmpArray));
			n = HDMI_OSAL_Snprintf(aTmpArray, 21, "%dx%d", pstCap->stStdTiming[i].u32HorActive,
											          pstCap->stStdTiming[i].u32VerActive);
			if ((pstCap->stStdTiming[i].u32RefreshRate) != 0)
			{
				HDMI_OSAL_Snprintf(aTmpArray+n,21-n, "_%d", (pstCap->stStdTiming[i].u32RefreshRate));
			}
	...
	/*VESA Enhanced EDID Standard Release A, Rev.1 Page 16 */
	for (i = 0; (i < pstCap->u32EstabNum) && (i < HDMI_EDID_ESTABTIMG_BUTT) && (pstCap->au32EstabTiming[i] < ARRAY_SIZE(s_EstabTiming2Str)); i++)
	{
...
**/
static HI_VOID Proc_Sink3DCap(HDMI_PROC_ENTRY *pFile, HDMI_SINK_CAPABILITY_S *pstCap)
{
    HI_S32 i = 0;
...
**/
static HI_VOID Proc_SinkAudioCap(HDMI_PROC_ENTRY *pFile, HDMI_SINK_CAPABILITY_S *pstCap)
{
    HI_S32 i = 0, j = 0;
    ...
    PROC_PRINT(pFile, "------------------------------ AudioCap -------------------------------------------\n");
  	...
  	/* audio info */
	for ( i= 0; (i < pstCap->u32AudioInfoNum) && (i < HDMI_EDID_MAX_AUDIO_CAP_COUNT); i++)
	{
        /* line 1 */
		...
		for (j = 0; (j < pAudioInfo->u32SupportBitDepthNum) && (j < HDMI_EDID_MAX_BIT_DEPTH_NUM); j++)
		{
		...
		for ( j = 0; (j < pAudioInfo->u32SupportSampleRateNum) && (j < HDMI_EDID_MAX_SAMPE_RATE_NUM); j++)
		{
...
**/
static HI_VOID Proc_SinkHdrCap(HDMI_PROC_ENTRY *pFile, HDMI_SINK_CAPABILITY_S *pstCap)
{
    CHECK_POINTER(pFile);
    CHECK_POINTER(pstCap);
    ...
    PROC_PRINT(pFile,"%-20s: %-20s \n", "HdrEotfHLG", pstCap->stHdrCap.stEotf.bEotfHLG ? STR_YES: STR_NO);
    /* line 3 */
    PROC_PRINT(pFile,"%-20s: %-20d ", "MaxLum", pstCap->stHdrCap.u8MaxLuminance_CV);
    PROC_PRINT(pFile,"%-20s: %-20d \n", "AvgLum", pstCap->stHdrCap.u8AverageLumin_CV);
    /* line 4 */
...
#ifdef HDMI_FRL_SUPPORT
static HI_VOID Proc_SinkFrlCaps(HDMI_PROC_ENTRY *pFile, HDMI_SINK_CAPABILITY_S *pstCap)
{
    HI_U32 u32Index = 0;
    ...
    CHECK_MAX_VALUE(u32Index, pstCap->u8MaxFrlRate, FRL_WORK_MODE_BUTT);
    PROC_PRINT(pFile,"%-20s: %-20s ", "MaxFrlRate", s_eFrlRate2Str[u32Index]);
    PROC_PRINT(pFile,"%-20s: %-20s\n", "DSCSupport", pstCap->stDscInfo.bDSC_1p2 ? STR_YES: STR_NO);
...
**/
static HI_VOID Proc_SinkDetailTiming(HDMI_PROC_ENTRY *pFile, HDMI_SINK_CAPABILITY_S *pstCap)
{
    HI_S32 i = 0;
	...
	if (pstCap->u32PerferTimingNum > 0)
	{
		...
		for (i = 0; (i < pstCap->u32PerferTimingNum) && (i < HDMI_EDID_MAX_DETAIL_TIMING_COUNT); i++)
		{
...
#endif
#ifdef HDMI_FRL_SUPPORT
    Proc_FRLStatus(pFile, HDMI_DEVICE_ID0);
#endif
...
HI_S32 DRV_HDMI_AOReadProc(osal_proc_entry_t* pFile)
{
    HDMI_DEVICE_S *pstHdmiDev = HI_NULL;
...
HI_S32 DRV_HDMI_SinkReadProc(osal_proc_entry_t* pFile)
{
    HDMI_DEVICE_S           *pstHdmiDev = HI_NULL;
    HI_S32                  s32Ret      = HI_SUCCESS;
