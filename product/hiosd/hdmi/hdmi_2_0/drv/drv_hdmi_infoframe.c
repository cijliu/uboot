                ...
                HI_U32 size, HDMI_3D_MODE_E HDMI_3D_Mode)
{
    ...
    ptr[3] = video_code & 0xff; /*frame->video_code*/
...
{
    HDMI_DRM_INFOFRAME_S*   pstDRMInfoframe = HI_NULL;
    HDMI_DEVICE_S*          pstHdmiDev      = HI_NULL;
...
HI_S32 DRV_HDMI_AVIInfoFrameSend(HDMI_INFOFRAME_S* pstInfoFrame, HI_BOOL bEnable)
{
    ...
    switch(pstHdrAttr->enHdrMode)
    {
        ...
        case HDMI_HDR_MODE_DOLBY_NORMAL:
            ...
            HDMI_INFO("Dolby normal, avi infoframe force to change outcolorspace(%d), ycc_quantization_range(%d).\n",
...
{
    HDMI_INFOFRAME_U         uInfoframe;
    HDMI_AUDIO_INFOFRAME_S*  pstAudioInfoframe = HI_NULL;
...
{
    HDMI_INFOFRAME_U         uInfoframe;
    HDMI_GDB_PACK_S*         pstGdbPack = HI_NULL;
    HDMI_VO_ATTR_S*          pstVOAttr  = HI_NULL;
    HDMI_DEVICE_S*           pstHdmiDev = HI_NULL;
...
HI_S32 DRV_HDMI_NullPacketSend(HDMI_INFOFRAME_S* pstInfoFrame, HI_BOOL bEnable)
{
    CHECK_POINTER(pstInfoFrame);

    DRV_HDMI_InfoFrameEnableSet(pstInfoFrame, HDMI_INFOFRAME_TYPE_NULL, bEnable);
