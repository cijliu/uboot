...
static HI_S32 EdidVendorInfo(HDMI_EDID_INFO_S *pstEdidInfo)
{
	...
	for(i = 0; i < 3; i++)
	{
	    pstVendor->cMfrsName[2 - i] = ((u16Data & (EDID_VEND_NAME_CHAR_MASK << (5*i))) >> (5*i));

		/* 'A'~'Z' is refered to 1~26 */
		if((EDID_VEND_CHAR_LOW_INVALID < pstVendor->cMfrsName[2 - i])
			&& (EDID_VEND_CHAR_HIGH_INVALID > pstVendor->cMfrsName[2 - i]))
		{
			pstVendor->cMfrsName[2 - i] += 'A' - 1;
		}
		else
		{
    ...
    EDID_INFO("    %-20s : %s\n", "ID Manufacturer Name",pstVendor->cMfrsName);
									...
									HI_U8 u8Len)
{
    ...
    if ((u8Len / 3) > 10)
    {
        EDID_ERR("ADB Audio Capability Count(%d) is error!\n", u8Len);
									...
									HI_U8 u8Len )
{
    ...
    pstCap->bCCBPCI                         = (pau8Data[5] & BIT4_MASK) ? HI_TRUE : HI_FALSE ;
    ...
    pstCap->u8MaxFrlRate                    = (pau8Data[6] & EDID_UPPER_NIBBLE_MASK) >> 4;

    if (u8Len > 7)
    {
        pstCap->bFAPA_Start_Location        = (pau8Data[7] & BIT0_MASK) ? HI_TRUE : HI_FALSE ;
        pstCap->bALLM                       = (pau8Data[7] & BIT1_MASK) ? HI_TRUE : HI_FALSE ;
        pstCap->bFVA                        = (pau8Data[7] & BIT2_MASK) ? HI_TRUE : HI_FALSE ;
        pstCap->bCNMVRR                     = (pau8Data[7] & BIT3_MASK) ? HI_TRUE : HI_FALSE ;
        pstCap->bCinemaVRR                  = (pau8Data[7] & BIT4_MASK) ? HI_TRUE : HI_FALSE ;
        pstCap->bMDelta                     = (pau8Data[7] & BIT5_MASK) ? HI_TRUE : HI_FALSE ;
        pstCap->u8VRRMIN                    =  pau8Data[8] & 0x3F;
        pstCap->u16VRRMAX                   =  pau8Data[8] & 0xC0;
        pstCap->u16VRRMAX                   = (pstCap->u16VRRMAX << 2) | pau8Data[9];

        pstCap->stDscInfo.bDSC_1p2          = (pau8Data[10] & BIT7_MASK) ? HI_TRUE : HI_FALSE ;
        pstCap->stDscInfo.bDSC_Native_420   = (pau8Data[10] & BIT6_MASK) ? HI_TRUE : HI_FALSE ;
        pstCap->stDscInfo.bDSC_All_bpp      = (pau8Data[10] & BIT3_MASK) ? HI_TRUE : HI_FALSE ;
        pstCap->stDscInfo.bDSC_16bpc        = (pau8Data[10] & BIT2_MASK) ? HI_TRUE : HI_FALSE ;
        pstCap->stDscInfo.bDSC_12bpc        = (pau8Data[10] & BIT1_MASK) ? HI_TRUE : HI_FALSE ;
        pstCap->stDscInfo.bDSC_10bpc        = (pau8Data[10] & BIT0_MASK) ? HI_TRUE : HI_FALSE ;

        pstCap->stDscInfo.u8DSC_MaxSlices   = (pau8Data[11] & EDID_LOWER_NIBBLE_MASK);
        pstCap->stDscInfo.u8DSCMaxFrlRate   = (pau8Data[11] & EDID_UPPER_NIBBLE_MASK);

        pstCap->stDscInfo.u8DSC_TotalChunkKBytes = (pau8Data[12] & 0x3F);
    }
                            ...
                            HI_U8 u8HdmiVicLen, HI_U32 u8Len, HI_U8 *pu8Offset, HI_U8 *pau8Data)
{
    ...
    EDID_NULL_CHK_NO_RET(pstCap);
    EDID_NULL_CHK_NO_RET(pstStatus);
    EDID_NULL_CHK_NO_RET(pau8Data);
    EDID_NULL_CHK_NO_RET(pu8Offset);
	...
	if (1 == u8Len)
	{
		for (i = 0 ; (i < pstCap->u32SupportVICNum) && (i < HDMI_EDID_MAX_VIC_COUNT) ; i++)
		{
			if ( (pstCap->au32SupportFormat[i] <= HDMI_EDID_MAX_REAL_VIC)
                 && (pstCap->u32SupportY420VICNum < HDMI_EDID_MAX_VIC_COUNT) )
			{
...
//                EDID_INFO("    %-20s : %d-%s\n", "Short Video Descriptor Support YUV420", pstCap->au32SupportFormat[i], (pstCap->au32SupportFormat[i] >= HDMI_3840X2160P24_16_9) ? u8VideoCode4KStr[pstCap->au32SupportFormat[i]-HDMI_3840X2160P24_16_9] : u8VideoCodeStr[pstCap->au32SupportFormat[i]]);
	...
	{
		u32Loop = u8Len*8;
		for(i = 0; (i < u32Loop) && (i < pstCap->u32SupportVICNum) && (i < HDMI_EDID_MAX_VIC_COUNT); i++)
	    {
...
// BYTE 3
    ...
    pstHdrCap->stEotf.bEotfHLG            = BIT_CHECK(pau8Data[0], BIT3_MASK) ? HI_TRUE : HI_FALSE; // ET_3
    pstHdrCap->stEotf.bEotfFuture         = HI_FALSE; // ET_4 to ET_5
// BYTE 4
...
static HI_U32 EdidVideoCapDB(HDMI_SINK_CAPABILITY_S *pstCap,
											  HI_U8 *pau8Data,
											  HI_U8 u8Len)
{
    EDID_NULL_CHK(pstCap);
    ...
    pstCap->bYccQrangeSelectable = BIT_CHECK(pau8Data[0], BIT7_MASK) ? HI_TRUE : HI_FALSE;
    pstCap->bRgbQrangeSelectable = BIT_CHECK(pau8Data[0], BIT6_MASK) ? HI_TRUE : HI_FALSE;
														...
														HI_U8 u8Len)
{
    ...
    switch(u8ExtTagCode)
    {
        ...
        case	EDID_VIDEO_CAPABILITY_DB :
                u32Ret = EdidVideoCapDB(pstCap, pau8Data, u8Len);
                EDID_SUCCESS_CHK(u32Ret);
                break;
									...
									HDMI_EDID_UPDATE_MODE_E enMode)
{
    ...
    {
        pstHdmiDev = HDMI_CONTAINER_OF(pstEdidInfo, __typeof__(*pstHdmiDev), stEdidInfo);
        HDMI_HAL_CALL(HAL_HDMI_HotPlugStatusGet, &bHotplug);
        if (bHotplug)
        {
            s32Ret = HAL_CALL_RET(HAL_HDMI_EdidRawDataRead, HDMI_EDID_TOTAL_SIZE, pau8Data);
