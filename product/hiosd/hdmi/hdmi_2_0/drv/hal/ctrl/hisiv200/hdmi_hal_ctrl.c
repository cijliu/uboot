...
{
    HI_U32 u32Reg = 0;
    if (HI_TRUE == HDMI_TX_CHANNEL_reg_vid_bypass_selGet())
    {
        /* 36bit must use 2.0 channel */
        u32Reg = HDMI_TX_CTRL_read(0x1800);
        u32Reg &= 0xFFFFFFFC;
        u32Reg |= enDeepColorBit;
        HDMI_TX_CTRL_write(0x1800, u32Reg);
    }
    else
    {
...
static HI_VOID CtrlVideoColorRgbSet(HI_BOOL bRgbIn,HI_BOOL bRgbOut)
{
    ...
    HDMI_YUV_RGB_CFG_reg_yuv_rgb_cfgSet(bRgbIn);
    return ;
}
...
/* 422->420 bypass or Horizontal sampling */
static HI_VOID CtrlVideoColorDwsmVertSet(HI_BOOL bDwsmVert)
{
    ...
    if (HI_TRUE == HDMI_TX_CHANNEL_reg_vid_bypass_selGet())
    {
        ...
        u32Reg = HDMI_TX_CTRL_read(0x0980);
        u32Reg &= (~(1 << 3));
        u32Reg |= ((!bDwsmVert) << 3);
        HDMI_TX_CTRL_write(0x0980, u32Reg);
    }
    else
    {
        HDMI_DATA_ALIGN_CTRL_reg_vert_cbcr_selSet(HI_FALSE);
    }
...
/* 444->422 bypass or Vertical sampling */
static HI_VOID CtrlVideoColorDwsmHoriSet(HI_BOOL bDwsmHori)
{
    ...
    {
        HDMI_VIDEO_DWSM_CTRL_reg_hori_filter_enSet(HI_FALSE);
    }
...
{
    if ( HDMI_VIDEO_DWSM_CTRL_reg_dwsm_hori_enGet() )
    {
        return HI_TRUE;
    }
    else
...
{
    HI_S32  s32HotPlug = 0;
    HI_BOOL bHpd       = HI_FALSE;

    s32HotPlug = HDMI_TX_AON_STATE_hotplug_stateGet();

    if (HDMI_HOTPLUG_ST_CFG_hpd_polarity_ctlGet() > 0)
    {
        bHpd = (0 < s32HotPlug) ? HI_FALSE : HI_TRUE;
    }
    else
    {
        bHpd = (0 < s32HotPlug) ? HI_TRUE : HI_FALSE;
    }
...
HI_VOID CtrlMachPpdCallBack(HI_VOID *pvData)
{
    ...
    if (HDMI_TX_AON_INTR_STATE_aon_intr_stat0Get())
    {
    	...
    	/* event */
        bEvent = CtrlHpdGet();
        if (bEvent != pstCtrlInfo->bHpd)
        {
            pstCtrlInfo->pfnEventCallBack(pstCtrlInfo->pvEventData, \
                (HI_TRUE == bEvent) ? HDMI_EVENT_HOTPLUG : HDMI_EVENT_HOTUNPLUG);

            pstCtrlInfo->bHpd = bEvent;
        }
        else
        {
            HDMI_WARN("HPD event not change: %d!\n", bEvent);
        }
    ...
    if (HDMI_TX_AON_INTR_STATE_aon_intr_stat1Get())
    {
    	...
    	/* event */
        bEvent = CtrlRsenGet();
        if (bEvent != pstCtrlInfo->bRsen)
        {
            pstCtrlInfo->pfnEventCallBack(pstCtrlInfo->pvEventData, \
               (HI_TRUE == bEvent) ? HDMI_EVENT_RSEN_CONNECT : HDMI_EVENT_RSEN_DISCONNECT);

            pstCtrlInfo->bRsen = bEvent;
        }
        else
        {
            HDMI_WARN("RSEN event not change: %d!\n", bEvent);
        }
    }
...
#if 0
static HI_S32 CtrlNullPacketGet(HI_BOOL *pbEnable)
{
    CTRL_NULL_CHK(pbEnable);

    *pbEnable = HDMI_AVMIXER_CONFIG_null_pkt_enGet();
...
HI_S32 HAL_HDMI_CtrlInit(HDMI_DEVICE_ID_E enHdmi,HDMI_HAL_INIT_S *pstHalInit)
{
    ...
    if (!pstCtrlInfo->bInit)
    {
        ...
        s32Ret |= HDMI_TX_S_audio_path_reg_REGS_TYPE_Init(HI_NULL);
    ...
    {
        s32Ret |= REG_HDMI_CRG_reg_Init();
        s32Ret |= HDMI_TX_S_tx_aon_reg_REGS_TYPE_Init(pstHalInit->pcBaseAddr);
        s32Ret |= HDMI_TX_S_audio_path_reg_REGS_TYPE_Init(pstHalInit->pcBaseAddr);
        /* This functions will not be used at BVT */
#ifndef CONFIG_HDMI_BVT_SDK
        s32Ret |= HDMI_TX_S_mcu_cpu_reg_REGS_TYPE_Init(pstHalInit->pcBaseAddr);
#endif
        s32Ret |= HDMI_TX_S_tx_ctrl_reg_REGS_TYPE_Init(pstHalInit->pcBaseAddr);
        s32Ret |= HDMI_TX_S_tx_hdmi_reg_REGS_TYPE_Init(pstHalInit->pcBaseAddr);
        s32Ret |= HDMI_TX_S_video_path_reg_REGS_TYPE_Init(pstHalInit->pcBaseAddr);
...
#ifndef HDMI_HDCP_SUPPORT
    /* if not support HDCP, this register must set 0x00 */
    HDMI_CTL_TYPE_CONFIG_ctl_type_configSet(0x00);
#endif
#endif
...
HI_S32 HAL_HDMI_CtrlVideoPathSet(HDMI_DEVICE_ID_E enHdmi, HDMI_VIDEO_PATH_S *pstVideoPath)
{
    ...
    if (HDMI_DEEP_COLOR_36BIT == pstVideoPath->enOutDeepColor)
    {
        /* 36bit must use 2.0 channel */
        DRV_HDMI_ProdCrgAllResetSet(HI_TRUE);
        HDMI_TX_CHANNEL_reg_vid_bypass_selSet(HI_TRUE);
    }
    else
    {
        if (HI_TRUE == HDMI_TX_CHANNEL_reg_vid_bypass_selGet())
        ...
        HDMI_TX_CHANNEL_reg_vid_bypass_selSet(HI_FALSE);
    }
#endif
    ...
    if (HI_FALSE == HDMI_TX_CHANNEL_reg_vid_bypass_selGet())
    {
        ...
        if(HDMI_ONE_BEAT_CLK >= pstVideoPath->u32InPixelClk && HDMI_COLORSPACE_YCbCr420 == pstVideoPath->enOutColorSpace)
        {
            bFifoAutoRst = HI_FALSE;
        }
        // TODO:reg_fifo_auto_rst_en should be set 0 when pixclk <= 600M and enOutColorSpace is YCbCr420
        HDMI_TX_PACK_FIFO_CTRL_reg_fifo_auto_rst_enSet(bFifoAutoRst);
        // TODO:vidpath_dout_clk_sel should be set 1 when pixclk <= 600M and enOutColorSpace is YCbCr420
        HDMI_TX_PWD_RST_CTRL_vidpath_dout_clk_selSet(!bFifoAutoRst);
    }
...
HI_VOID HAL_HDMI_CtrlDataReset(HDMI_DEVICE_ID_E enHdmi)
{
    HI_U32 u32HTotalPre = 0;
    HI_U32 u32HTotalCur = 0;
    HI_U32 u32StartTime = 0;
    HI_U32 u32Threshold = 1;

    switch(HDMI_TX_PACK_FIFO_CTRL_tmds_pack_modeGet())
    {
        case 0:
            u32Threshold = 1;
            break;
    ...
    HDMI_FCCONTINSET0_p_contin_upd_enSet(HI_FALSE);
    /* pwd reset */
    ...
    HDMI_FCCONTINSET0_p_contin_upd_enSet(HI_TRUE);
    HDMI_MSLEEP(1);

#ifndef HDMI_BUILD_IN_BOOT
    u32StartTime = HDMI_OSAL_GetTimeInMs();

    while((HDMI_OSAL_GetTimeInMs() - u32StartTime) < 30)
    {
        /* TMDS PACK FIFO reset */
        HDMI_TX_PACK_FIFO_CTRL_reg_fifo_manu_rstSet(HI_TRUE);
        HDMI_UDELAY(2);
        HDMI_TX_PACK_FIFO_CTRL_reg_fifo_manu_rstSet(HI_FALSE);
        ...
        u32HTotalCur = HDMI_HTOTAL_HW_htotal_hwGet();
        if ((u32HTotalCur >= 300) && (osal_abs(((HI_S32)u32HTotalCur - u32HTotalPre)) < u32Threshold))
        {
            HDMI_INFO("Htotal(%d) stable, time:%d.\n", u32HTotalCur, (HDMI_OSAL_GetTimeInMs() - u32StartTime));
            break;
        }

        u32HTotalPre = u32HTotalCur;
    }
#else
    while(u32StartTime < 30)
    {
        /* TMDS PACK FIFO reset */
        HDMI_TX_PACK_FIFO_CTRL_reg_fifo_manu_rstSet(HI_TRUE);
        HDMI_UDELAY(2);
        ...
        u32HTotalCur = HDMI_HTOTAL_HW_htotal_hwGet();
        if ((abs(((HI_S32)u32HTotalCur - u32HTotalPre)) < u32Threshold))
        {
            break;
        }
