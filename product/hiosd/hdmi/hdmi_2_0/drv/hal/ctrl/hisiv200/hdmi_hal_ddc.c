    ...
    if (bReadIssue)
    {
        for (s32DataSize=0;s32DataSize < pstCfg->s32DataSize;s32DataSize++,pu8Data++)
        {
            ...
            if ((i >= u32Retry) || ((u32TimeCurr - u32TimeStart) > DDC_DEFAULT_RETRY_TIMEOUT_ISSUE))
            {
                ...
                {
                    HDMI_WARN("ddc read fifo u32Retry=%u ms,size=%d,timeout:%d!\n", u32Retry, pstCfg->s32DataSize, (u32TimeCurr - u32TimeStart));
                }
                else
                {
                    HDMI_INFO("ddc read fifo u32Retry=%u ms,size=%d,timeout:%d!\n", u32Retry, pstCfg->s32DataSize, (u32TimeCurr - u32TimeStart));
                }
                return HI_FAILURE;
            }
    ...
    else
    {
        for (s32DataSize=0;s32DataSize < pstCfg->s32DataSize;s32DataSize++,pu8Data++)
    	{
            ...
            if (i >= u32Retry || ((u32TimeCurr - u32TimeStart) > DDC_DEFAULT_RETRY_TIMEOUT_ISSUE))
            {
                if (pstCfg->enFuncType != DDC_FUNC_TYPE_SCDC)
                {
                    HDMI_ERR("ddc write fifo u32Retry=%u ms,size=%d,timeout:%d!\n", u32Retry, pstCfg->s32DataSize, (u32TimeCurr - u32TimeStart));
                }
                else
                {
                    HDMI_INFO("ddc write fifo u32Retry=%u ms,size=%d,timeout:%d!\n", u32Retry, pstCfg->s32DataSize, (u32TimeCurr - u32TimeStart));
                }
                return HI_FAILURE;
            }
...
/* Only for small amounts of data transmitting. */
static HI_S32 DdcDataPwdIssueTrim(DDC_CFG_S *pstCfg, HI_BOOL bReadIssue)
{
    HI_U32      s32DataSize = 0;
    HI_U8       *pu8Data = HI_NULL;
    HI_U32      u32TimeStart = 0;
    ...
    if (bReadIssue)
    {
        ...
        {
            u32TimeCurr = HDMI_OSAL_GetTimeInMs();
            if ((u32TimeCurr - u32TimeStart) >= (pstCfg->u32IssueTimeOut))
            {
//                HDMI_ERR("DDC read timeout:%dms!\n", pstCfg->u32IssueTimeOut);
                goto EXIT;
            }

            if (pu8Data)
            {
                if((HDMI_PWD_DATA_CNT_pwd_fifo_data_cntGet() > 0))
                {
                    *pu8Data = HDMI_PWD_FIFO_RDATA_pwd_fifo_data_outGet();
                    /* the fifo status is not refresh promptly, so re-read the fifo status and delay 1us if the fifo is empty,
                       wait the data ready. it must delay 5us(from wzg @2015.12.28) after read fifo data. */
                    HDMI_UDELAY(1);
                    pu8Data++;
                    s32DataSize++;
                }
            ...
            {
//                HDMI_ERR("ddc read ptr(%d) is null!\n", s32DataSize);
                goto EXIT;
            }

        }while((s32DataSize < pstCfg->s32DataSize));
    }
    /* write issue */
    else
    {
        do
        {
            ...
            if ((u32TimeCurr - u32TimeStart) >= pstCfg->u32IssueTimeOut)
            {
//                HDMI_ERR("DDC write timeout:%d!\n", pstCfg->u32IssueTimeOut);
                goto EXIT;
            }

            if (pu8Data)
            {
                if ((HDMI_PWD_DATA_CNT_pwd_fifo_data_cntGet() < DDC_MAX_FIFO_SIZE))
                {
                    HDMI_PWD_FIFO_WDATA_pwd_fifo_data_inSet(*pu8Data);
                    HDMI_UDELAY(1);
                    s32DataSize++;
            ...
            {
//                HDMI_ERR("ddc write ptr(%d) is null!\n", s32DataSize);
                goto EXIT;
            }
        }while((s32DataSize < pstCfg->s32DataSize));
    }
    ...
    if (bReadIssue)
    {
        for (s32DataSize=0;s32DataSize < pstCfg->s32DataSize;s32DataSize++,pu8Data++)
        {
            ...
            if (i >= u32Retry)
            {
                if (pstCfg->enFuncType != DDC_FUNC_TYPE_SCDC)
                {
                    HDMI_WARN("ddc read fifo u32Retry=%u ms,size=%d!\n", u32Retry, pstCfg->s32DataSize);
                }
                else
                {
                    HDMI_INFO("ddc read fifo u32Retry=%u ms,size=%d\n", u32Retry, pstCfg->s32DataSize);
                }
                return HI_FAILURE;
            }
    ...
    else
    {
        for (s32DataSize=0;s32DataSize < pstCfg->s32DataSize;s32DataSize++,pu8Data++)
    	{
            ...
            if (i >= u32Retry)
            {
                if (pstCfg->enFuncType != DDC_FUNC_TYPE_SCDC)
                {
                    HDMI_ERR("ddc write fifo u32Retry=%u ms,size=%d\n", u32Retry, pstCfg->s32DataSize);
                }
                else
                {
                    HDMI_INFO("ddc write fifo u32Retry=%u ms,size=%d!\n", u32Retry, pstCfg->s32DataSize);
                }
                return HI_FAILURE;
            }
...
HI_S32 HAL_HDMI_DdcIssueTrim(HDMI_DEVICE_ID_E enHdmi, DDC_CFG_S *pstCfg)
{
    HI_S32      s32Ret = HI_SUCCESS;
    HI_BOOL     bReadIssue = HI_FALSE;
    DDC_INFO_S *pstDdcInfo = DdcInfoPtrGet(enHdmi);
...
//        HDMI_ERR("ddc command issue fail!\n");
    ...
    /* issue data */
    if ((s32Ret = DdcDataPwdIssueTrim(pstCfg, bReadIssue)) <= 0)
    {
//        HDMI_INFO("ddc data issue fail!\n");
        goto RETURN;
    }
...
HI_S32 HAL_HDMI_DdcRateSet(HDMI_DEVICE_ID_E enHdmi, HI_U32 u32Rate)
{
    if (u32Rate == 0 || u32Rate > 3000)
    {
        return HI_FAILURE;
    }

    u32Rate = (6000 / u32Rate) - 1;

    HDMI_DDC_MST_CTRL_ddc_speed_cntSet(u32Rate);

    HDMI_INFO("Write ddc freq(%d)\n", u32Rate);
