...
HI_VOID HAL_HDMI_DebugHdcp14Print(HI_VOID *pvData)
{
    ...
    HDCP14_RUN_STAT_S   stRunStat = {0};
    HDMI_DEVICE_ID_E    enHdmiId = HDMI_DEVICE_ID0;

    DEBUG_NULL_CHK_NO_RET(pvData);
    enHdmiId = *(HDMI_DEVICE_ID_E *)pvData;
    HDMI_MEMSET(&stRunStat, 0, sizeof(HDCP14_RUN_STAT_S));

    pstStauts = HDMI_VMALLOC(sizeof(HDCP14_STAUTS_S));
    DEBUG_NULL_CHK_NO_RET(pstStauts);

    HDMI_MEMSET(pstStauts, 0, sizeof(HDCP14_STAUTS_S));
...
{
    HI_S32              i = 0;
    DDC_RUN_S           stDdcRun = {0};
    DDC_RECORD_ELEM_S   stElem = {0};
    HDMI_DEVICE_ID_E    enHdmiId = HDMI_DEVICE_ID0;

    DEBUG_NULL_CHK_NO_RET(pvData);
    HDMI_MEMSET(&stElem, 0, sizeof(DDC_RECORD_ELEM_S));
    HDMI_MEMSET(&stDdcRun, 0, sizeof(DDC_RUN_S));
...
HI_VOID HAL_HDMI_DebugCecPrint(HI_VOID *pvData)
{
    ...
    for (i=0;(i<stStatus.u32TxMsgCnt) && (i<CEC_MAX_MSG_NUM);i++)
    {
        ...
        if(   (pstCecData->bCecTx && i>=stStatus.u32TxMsgCnt)
           || (!pstCecData->bCecTx && i>=stStatus.u32RxMsgCnt) )
        {
            break;
        }
