...
#define MACHINE_ID_CHK(u32MachId) do{\
    ...
    {\
        HDMI_WARN("can't find machine id = %u\n!",u32MachId);\
        return HI_FAILURE;\
    }\
}while(0)
...
typedef struct
{
    HDMI_SEMAPHORE      	stMachMutex;			/* lock to avoid competition*/
...
HI_S32 HAL_HDMI_MachDeInit(HI_VOID)
{
    ...
    HDMI_DESTORY_MUTEX(pstMachInfo->stMachMutex);
...
HI_S32 HAL_HDMI_MachInvoke(HI_VOID)
{
    ...
    else
    {
...

        {
            ...
            {
                if (HDMI_OSAL_GetTimeInMs() - pstTmpElem->stMachRun.u64LastTime > pstTmpElem->stMachCtrl.u64Interval)
                {
                    ...
                    HDMI_MUTEX_LOCK(pstMachInfo->stMachMutex);
                    pstTmpElem->stMachRun.u64LastTime = HDMI_OSAL_GetTimeInMs();
                    pstTmpElem->stMachRun.au64Timestamp[pstTmpElem->stMachRun.u32StampPtr] = HDMI_OSAL_GetTimeInMs();
    ...
    else
    {
            ...
            ++i,pstTmpElem=&pstMachInfo->astMachElem[i])
        {
            ...
            if (!pstTmpElem->stMachRun.bValidId)
            {
                ...
                s32NameLen = HDMI_STRLEN(pstTmpElem->stMachCtrl.pcName);
                s32NameLen = (s32NameLen<HDMI_MACH_MAX_NAME_SIZE) ? s32NameLen : (HDMI_MACH_MAX_NAME_SIZE-1);
                HDMI_STRNCPY(pstTmpElem->stMachRun.acName, pstTmpElem->stMachCtrl.pcName, s32NameLen);
                pstTmpElem->stMachRun.acName[s32NameLen] = '\0';
                pstTmpElem->stMachCtrl.pcName = pstTmpElem->stMachRun.acName;
...
HI_S32 HAL_HDMI_MachStart(HI_U32 u32MachId)
{
    ...
    pstTmpElem->stMachRun.u64EnableTime = HDMI_OSAL_GetTimeInMs();
	HDMI_MUTEX_UNLOCK(pstMachInfo->stMachMutex);
...
HI_S32 HAL_HDMI_MachStop(HI_U32 u32MachId)
{
    ...
    pstTmpElem->stMachRun.u64DisableTime    = HDMI_OSAL_GetTimeInMs();
	HDMI_MUTEX_UNLOCK(pstMachInfo->stMachMutex);
...
{
    return (HI_U64)HDMI_OSAL_GetTimeInMs();
}
