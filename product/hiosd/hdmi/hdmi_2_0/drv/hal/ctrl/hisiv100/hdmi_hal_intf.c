...
#define INTF_INIT_CHK(pstHalCfg) do{\
    ...
    {HDMI_ERR("hal intf module didn't init!\n");\
    return HI_FAILURE;}\
}while(0)
...
HI_VOID HAL_HDMI_CtrlReset(struct hiHDMI_HAL_S *pstHdmiHal)
{
    ...
    HAL_HDMI_CtrlDataReset(pstHdmiHal->stHalCtx.u32HdmiID, HI_FALSE, 0);

    for (i = 0; (!bTmdsStable) && (i < 20); i++)
    {
        HDMI_MSLEEP(1);
        HAL_HDMI_CtrlTmdsStableGet(pstHdmiHal->stHalCtx.u32HdmiID, &bTmdsStable);
    }

    HDMI_INFO("wait %dms, bTmdsStable=%d\n", i, bTmdsStable);
