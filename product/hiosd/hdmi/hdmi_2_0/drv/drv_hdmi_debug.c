...
#define DEBUG_HAL_CALL(pfunc,param...) ({\
    if (HI_SUCCESS != HAL_CALL(pfunc, param)) \
    { \
        HDMI_DBG_ERR("null pointer!\n"); \
        return HI_FAILURE;\
    } \
})
...
#define DEFAULT_ECHO_DEVICE_HANDLE (0)
    ...
    HDMI_VSNPRINTF(buf, size, fmt, args);
...
HI_VOID DEBUG_EchoHelper(const HI_CHAR *fmt, ...)
{
    ...
    HDMI_MEMSET(pucBuf, 0, DEBUG_PROC_BUFFER_SIZE);
...
{
	HDMI_DEVICE_S   *pstHdmiDev = HI_NULL;
    HDMI_BLACK_FRAME_INFO_S stBlackInfo = {0};
    ...
    HDMI_MEMSET(&stBlackInfo, 0, sizeof(HDMI_BLACK_FRAME_INFO_S));
    ...
    if (IS_STR_EQUAL(pstCmdArg->pcArgv[1],"0"))
    {
        DEBUG_HAL_CALL(HAL_HDMI_PhyOutputEnableSet, HI_FALSE);
        HDMI_DBG_PRINT("\n oe set OFF %s! \n\n", STR_SUCC );
    }
    else if (IS_STR_EQUAL(pstCmdArg->pcArgv[1],"1"))
    {
        DEBUG_HAL_CALL(HAL_HDMI_PhyOutputEnableSet, HI_TRUE);
        HDMI_DBG_PRINT("\n oe set ON %s! \n\n", STR_SUCC );
    }
    else
    {
...
static HI_S32 DebugEdidFromFile(HDMI_DEVICE_S *pstHdmiDev, HDMI_DEBUG_CMD_ARG_S *pstCmdArg)
{
    ...
    pFile = Debugfopen(pstCmdArg->pcArgv[2], HDMI_O_RDONLY, 0);
    if (pFile == HI_NULL)
    {
    ...
    if (pu8Buf)
    {
        HDMI_MEMSET(pu8Buf, 0, DEBUG_MAX_EDIDBUF_SIZE);
...
static HI_S32 DebugEdid(HDMI_DEBUG_CMD_ARG_S *pstCmdArg)
{
    ...
    if (pu8Buf)
    {
        HDMI_MEMSET(pu8Buf, 0, HDMI_EDID_TOTAL_SIZE);
...
usage_error:
    ...
    HDMI_DBG_PRINT("[argv2  ]: You have to use at least 1 charater of 'SPACE|TAB|,|0x' between 2 hex-bytes.\n");
...
static HI_S32 DebugCbar(HDMI_DEBUG_CMD_ARG_S *pstCmdArg)
{
    ...
    {
        bEnable = HI_FALSE;
        DEBUG_HAL_CALL(HAL_HDMI_Debug, HDMI_DEBUG_CMD_COLOR_BAR, &bEnable);
        HDMI_DBG_PRINT("\n ColorBar is OFF %s! \n\n",STR_SUCC);
    }
    else if (IS_STR_EQUAL(pstCmdArg->pcArgv[1],"1"))
    {
...
static HI_S32 DebugScdc(HDMI_DEBUG_CMD_ARG_S *pstCmdArg)
{
    ...
    if (IS_STR_EQUAL(pstCmdArg->pcArgv[1],"0"))
    {
        ...
        DEBUG_HAL_CALL(HAL_HDMI_ScdcStatusSet, &stScdcStatus);
        HDMI_MSLEEP(90);
        DEBUG_HAL_CALL(HAL_HDMI_PhyOutputEnableSet, HI_TRUE);
        ...
        HDMI_MSLEEP(3000);
        DEBUG_HAL_CALL(HAL_HDMI_ScdcStatusGet, &stScdcStatus);
        HDMI_DBG_PRINT("\n scdc set OFF %s! \n\n",
                    (stScdcStatus.bSourceScrambleOn == HI_FALSE) &&
                    (stScdcStatus.bSinkScrambleOn == HI_FALSE) &&
...
static HI_S32 DebugDdc(HDMI_DEBUG_CMD_ARG_S *pstCmdArg)
{
    ...
    if (u32DdcFreq > 300 || u32DdcFreq == 0)
    {
        goto usage_error;
    }

    DEBUG_HAL_CALL(HAL_HDMI_Debug, HDMI_DEBUG_CMD_DDC_FREQ, &u32DdcFreq);
    HDMI_DBG_PRINT("\n ddc frequency set %dKHz! \n\n", u32DdcFreq);

    return HI_SUCCESS;

usage_error:
    HDMI_DBG_PRINT("[Usage  ]: echo ddc argv1 > /proc/msp/hdmi0\n");
    HDMI_DBG_PRINT("[argv1  ]: ddc frequency [1,300] kHz\n");
...
static HI_S32 DebugHdmimode(HDMI_DEBUG_CMD_ARG_S *pstCmdArg)
{
    ...
    if (IS_STR_EQUAL(pstCmdArg->pcArgv[1],"0"))
    {
        DEBUG_HAL_CALL(HAL_HDMI_TmdsModeSet, HDMI_TMDS_MODE_DVI);
        HDMI_DBG_PRINT("\n hdmi tmds mode set DVI %s! \n\n",STR_SUCC);
    }
    else if (IS_STR_EQUAL(pstCmdArg->pcArgv[1],"1"))
    {
        DEBUG_HAL_CALL(HAL_HDMI_TmdsModeSet, HDMI_TMDS_MODE_HDMI_1_4);
        HDMI_DBG_PRINT("\n hdmi tmds mode set HDMI1.4 %s! \n\n",STR_SUCC);
    }
    else if (IS_STR_EQUAL(pstCmdArg->pcArgv[1],"2"))
    {
        DEBUG_HAL_CALL(HAL_HDMI_TmdsModeSet, HDMI_TMDS_MODE_HDMI_2_0);
...
static HI_S32 DebugDither(HDMI_DEBUG_CMD_ARG_S *pstCmdArg)
{
    ...
    DEBUG_HAL_CALL(HAL_HDMI_Debug, HDMI_DEBUG_CMD_DITHER, &enDitherMode);
...
static HI_S32 DebugCmd(HDMI_DEBUG_CMD_ARG_S *pstCmdArg)
{
    ...
    else if (IS_STR_EQUAL(pstCmdArg->pcArgv[1], "2"))
    {
        ...
        DEBUG_HAL_CALL(HAL_HDMI_BlackDataSet, &stBlackInfo);
    ...
    else if (IS_STR_EQUAL(pstCmdArg->pcArgv[1], "4"))
    {
        ...
        u32Data = (u32Data > HDMI_TRACE_LEN_1) ? HDMI_TRACE_LEN_1 : u32Data;
        HDMI_DBG_PRINT("\n PCB trace len old(%d)->new(%d)! \n\n", pstHdmiDev->stModParam.enTraceLen, u32Data);

        pstHdmiDev->stModParam.enTraceLen = (HDMI_TRACE_LEN_E)u32Data;
    }
    else
    {
        HDMI_DBG_ERR("error argcs!\n");
...
static HI_S32 DebugHdr(HDMI_DEBUG_CMD_ARG_S *pstCmdArg)
{
    ...
    {
        if(pstCmdArg->u32Argc < 3 || HI_NULL == pstCmdArg->pcArgv[2])
        {
            goto usage_error;
        }
...
usage_error:
    ...
    HDMI_DBG_PRINT("           0\t--debug HDR mode set (argv2 = 1-HDR10, 2-Dolby, else-SDR).\n"
                   "           1\t--debug mode set when the HDR mode change to HDR10(argv2 = 1-OE, 2-AVMUTE, else-disable.)\n"
...
static HI_S32 DebugFrl(HDMI_DEBUG_CMD_ARG_S *pstCmdArg)
{
    ...
    DEBUG_NULL_CHK(pstFrlInfo);
    DEBUG_NULL_CHK(pstMachInfo);

    HDMI_MEMSET(&stConfig, 0, sizeof(stConfig));
    HDMI_MEMSET(&stDebug, 0, sizeof(HDMI_FRL_DEBUG_S));
    ...
    if (IS_STR_EQUAL(pstCmdArg->pcArgv[1], "0"))
    {
        ...
        HDMI_DBG_PRINT("Set FRL machine old(%s)->new(%s).\n", \
                        ...
                        (0 == u32Data) ? "stop" : "start");

        pstMachInfo->bStartMach = (0 == u32Data) ? HI_FALSE : HI_TRUE;
    }
    else if (IS_STR_EQUAL(pstCmdArg->pcArgv[1], "1"))
    {
...
usage_error:
    ...
    HDMI_DBG_PRINT("           0\t--Train machine start [argv2: 0--stop, else--start].\n"
    ...
    HDMI_DBG_PRINT("           8\t--Mode change         [argv2:(TMDS/FRL) argv3:(Rate)\n");
                                                    ...
                                                    HI_S64 *ps64Data)
{
    ...
    DEBUG_NULL_CHK(pBuf);
    DEBUG_NULL_CHK(ps64Data);
    DEBUG_NULL_CHK(pFile);
