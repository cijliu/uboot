...
static inline hi_void rgb_to_yc_full(hi_u16 r, hi_u16 g, hi_u16 b, hi_u16 *py, hi_u16 *pcb, hi_u16 *pcr)
{
    hi_u16 py_temp, pcb_temp, pcr_temp;

    py_temp = (hi_u16)(((r * 76 + g * 150 + b * 29) >> 8) * 4);
    pcb_temp = (hi_u16)(((((b * 130 - r * 44) - g * 86) >> 8) + 128) * 4);
    pcr_temp = (hi_u16)(((((r * 130 - g * 109) - b * 21) >> 8) + 128) * 4);
...
static inline hi_u32 rgb_to_yuv_full(hi_u32 rgb)
{
    hi_u16 y, u, v;
...
hi_void vo_drv_set_dev_clk(vo_hal_dev vo_dev)
{
    ...
    hi_u32 lcd_mclk_div = 0x00FFFFF;  // default value of the register
    ...
    switch (g_hal_dev_cfg[vo_dev].out_sync) {
        ...
        case VO_OUTPUT_USER: {
            ...
            if (VO_INTF_LCD_6BIT & g_hal_dev_cfg[vo_dev].intf_type ||
                ...
                if (g_user_intfsync_info.dev_div == 3) {
...
hi_void vo_drv_set_dev_dither_cfg(vo_hal_dev vo_dev, hi_u32 in_bit_widht, hi_u32 out_bit_widht)
{
    ...
    if ((in_bit_widht == 10) && (out_bit_widht == 8)) {
    ...
    } else if ((in_bit_widht == 10) && (out_bit_widht == 6)) {
        dither_cfg.dither_en = 1;
    ...
    } else if (((in_bit_widht == 10) && (out_bit_widht == 10)) ||
               ((in_bit_widht == 8) && (out_bit_widht == 8))) {
...
hi_void vo_drv_set_all_crg_clk(hi_bool clk_en)
{
    ...
    // add for mipi
    sys_hal_vo_dev_clk_en(0, vo_clk_en);
    sys_hal_vo_core_clk_en(0, vo_clk_en);
...
{
    HAL_MULTICHN_EN multi_chn_en = HAL_MULTICHN_EN_1P1C;
...
void vo_drv_set_dev_dither(hi_s32 vo_dev)
{
    ...
    if (VO_INTF_LCD_6BIT & intf_type ||
        VO_INTF_LCD_16BIT & intf_type) {
        vo_drv_set_dev_dither_cfg(vo_dev, 10, 6);
    } else {
        vo_drv_set_dev_dither_cfg(vo_dev, 10, 8);
    }
}
...
hi_s32 load_bmp(hi_u32 *bmp_addr, OSD_LOGO_T *video_logo)
{
    hi_u16 bmpf_type;
    hi_u16 bpp;
    ...
    memcpy(&bmpf_type, (char *)bmp_addr, sizeof(hi_u16));
    memcpy(&bmp_file_header, (char *)bmp_addr + sizeof(hi_u16), sizeof(OSD_BITMAPFILEHEADER));
    memcpy(&bmp_info, (char *)bmp_addr + sizeof(hi_u16) + sizeof(OSD_BITMAPFILEHEADER), sizeof(OSD_BITMAPINFO));
    video_logo->width = bmp_info.bmi_header.bi_width;
    video_logo->height = bmp_info.bmi_header.bi_height;
    video_logo->stride = bmp_info.bmi_header.bi_width * 2;
    video_logo->rgb_buffer = bmp_addr + bmp_file_header.bf_off_bits / 4 + 1;

    bpp = (bmp_info.bmi_header.bi_bit_count) / 8;
    if ((bpp < 2)) {
