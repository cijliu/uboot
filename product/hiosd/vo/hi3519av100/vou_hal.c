...
hi_s32 sys_hal_vo_dev_clk_en(hi_s32 vo_dev, hi_bool clk_en)
{
    ...
    if (vo_dev == 0) {
        ...
        hi_reg_set_bit(tmp, 4, IO_ADDRESS(VOU_CRG_PERCTL_ADDR));
        hi_reg_set_bit(tmp, 7, IO_ADDRESS(VOU_CRG_PERCTL_ADDR));
    } else {
        return -1;
    }
...
hi_s32 sys_hal_vo_hdmi_clk_en(hi_bool hdmi_clk_en)
{
    ...
    hi_reg_set_bit(tmp, 27, IO_ADDRESS(VOU_CRG_PERCTL_ADDR));
...
hi_s32 sys_hal_vo_mipi_clk_en(hi_bool mipi_clk_en)
{
    ...
    hi_reg_set_bit(tmp, 28, IO_ADDRESS(VOU_CRG_PERCTL_ADDR));
...
{
    hi_reg_write32(hd1_div_mod << 14, 0x3 << 14, IO_ADDRESS(VOU_CRG_PERCTL_ADDR));
    return 0;
}

hi_s32 sys_hal_vo_hd0_ppc_sel(hi_u32 clk_sel)
{
    hi_reg_write32(clk_sel << 16, 0x3 << 16, IO_ADDRESS(VOU_CRG_PERCTL_ADDR));
...
{
    hi_reg_write32(clk_sel << 18, 0x7 << 18, IO_ADDRESS(VOU_CRG_PERCTL_ADDR));
...
{
    hi_reg_write32(clk_sel << 21, 0x3 << 21, IO_ADDRESS(VOU_CRG_PERCTL_ADDR));
...
{
    hi_reg_write32(bt_clk_ch_sel << 25, 0x1 << 25, IO_ADDRESS(VOU_CRG_PERCTL_ADDR));
...
hi_s32 sys_hal_vo_hd_out_pctrl(hi_bool clk_reverse)
{
    ...
    hi_reg_set_bit(tmp, 29, IO_ADDRESS(VOU_CRG_PERCTL_ADDR));
...
{
    hi_reg_write32(mipi_ch_sel << 30, 0x1 << 30, IO_ADDRESS(VOU_CRG_PERCTL_ADDR));
...
{
    hi_reg_write32(data_mode << 26, 0x7 << 26, IO_ADDRESS(0x04528004));
