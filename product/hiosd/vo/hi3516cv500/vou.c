...
int start_vo(unsigned int dev, unsigned int type, unsigned int sync)
{
    ...
    if (VO_INTF_LCD & type ||
        ...
        vo_drv_set_dev_bk_grd(dev, rgb_to_yuv(g_bg_color));
    }
...
int start_videolayer(unsigned int layer, unsigned long addr, unsigned int strd, hi_vo_rect layer_rect)
{
    ...
    } else if (VO_INTF_LCD & vo_intf_type ||
        VO_INTF_LCD_6BIT & vo_intf_type ||
        VO_INTF_LCD_8BIT & vo_intf_type ||
        VO_INTF_LCD_16BIT & vo_intf_type ||
        VO_INTF_LCD_18BIT & vo_intf_type ||
        VO_INTF_LCD_24BIT & vo_intf_type) {
        csc_matrix  = VO_CSC_MATRIX_BT709_TO_RGB_PC;
    ...
    hal_video_set_multi_area_c_addr(layer, 0, addr + strd * VO_ALIGN(layer_rect.h, 16), strd);
...
int start_gx(unsigned int layer, unsigned long addr, unsigned int strd, hi_vo_rect gx_rect)
{
    ...
    if ((VO_INTF_HDMI & vo_intf_type) ||
        (VO_INTF_BT1120 & vo_intf_type)) {
        ...
        // do rgb to yuv.
        graphic_drv_set_csc_coef(hal_gfx_layer, &gfx_csc, &csc_coef_param);

        hal_layer_set_csc_en(vo_layer, HI_TRUE);
    } else if ((VO_INTF_MIPI & vo_intf_type) ||
        (VO_INTF_LCD & vo_intf_type) ||
        (VO_INTF_LCD_6BIT & vo_intf_type) ||
        (VO_INTF_LCD_8BIT & vo_intf_type) ||
        (VO_INTF_LCD_16BIT & vo_intf_type) ||
        (VO_INTF_LCD_18BIT & vo_intf_type) ||
