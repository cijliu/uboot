...
#ifdef DEBUG_ACODEC
	printf("### fun:%s line:%d ulAcodecBase :%8lx u32Offset :%x value: %8lx\n", __FUNCTION__, __LINE__, ulAcodecBase, u32Offset, u32Value);
#endif
}
...
int acodec_device_init(void)
{
	...
	// audio crg
	acodec_regsetbit(1 , 1, (unsigned long)g_acodec_crg_reg);
	acodec_regsetbit(1 , 2, (unsigned long)g_acodec_crg_reg);
...
int acodec_device_exit(void)
{
	...
	// audio crg
	acodec_regsetbit(0 , 1, (unsigned long)g_acodec_crg_reg);
	acodec_regsetbit(0 , 2, (unsigned long)g_acodec_crg_reg);
